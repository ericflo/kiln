...
## BUG 2 — TWO confirmed findings; doubling still resists (2026-06-01, full pod session)

**Finding A — capture-step compute is CORRECT.** The `BOX102DIFF` probe (Pass-1 warm/EAGER vs first captured launch, SAME capture-step input) shows byte-identical per-layer norms: `[7.1875, 15.5, 25.0, ..., 1136.0, 1376.0]` for both. So the captured graph computes correctly on its first launch == eager. This REFUTES "the per-layer captured compute is wrong." Also confirms the per-layer probe is sane (not an arena-aliasing artifact — Pass-1/eager norms are monotonic + match).

**Finding B — late layers FREEZE on replay = stale uninitialized arena buffers.** Full-precision per-replay norms: step N→N+1 only the first ~5 layers change; layers 5-31 are byte-identical to step N (61.250, 69.000, ..., 1376.000). `KILN_ARENA_FORCE_ZERO=1` (force a captured per-replay memset on EVERY arena buffer, not just `zero=true` ones — capture_alloc.rs) UN-FREEZES them (all layers now vary step-to-step). So the freeze is caused by an arena buffer alloc'd `zero=false` (uninit) that the captured kernel does NOT fully overwrite → it reads the previous replay's value on replay. (Works on first-launch because the warm pass left the correct capture-step value there.)

**BUT the doubling SURVIVES both** — output still doubles with FORCE_ZERO on (un-frozen late layers) AND with the recurrent-state fix. So either (a) the late-layer freeze is a SEPARATE bug from the doubling, or (b) force-zero is the wrong fix (zeroing the stale buffers ≠ giving them correct values). Net: capture is correct, recurrent state advances, conv/KV in-place, late-layer freeze identified+toggleable — yet the doubling persists, meaning its true cause is still not isolated.

**DEFINITIVE next step (the only thing that will settle it):** an eager-vs-replay differential on the SAME MID-GENERATION step (not just the capture step). Add a debug path in `decode_step_paged` that, on a replay step, ALSO runs the eager `model_forward_paged_with_graph_inputs` (no capture) on the identical (token, position, linear_state snapshot, KV) inputs and dumps BOTH per-layer norm vectors → the first layer where replay ≠ eager (for the same input) is the box-102 root cause. This is the clean apples-to-apples the capture-step PASS1/FIRSTLAUNCH couldn't give for replays. Then fix that op (likely: an uninit arena buffer that must be `zero=true`, or a read-before-write kernel, or an AUTO_FREE'd graph-owned buffer). Probes (`KILN_DEBUG_LAYER_NORMS`, `KILN_ARENA_FORCE_ZERO`, `BOX102DIFF`) are all on branch `wip/1082-box2-fix-validate`. Do NOT merge (output garbage).

## BUG 2 — GRAPH-SPECIFIC confirmed; replay logits differ; layer-0/input divergence (2026-06-01)

Decisive new results (full toolkit on branch `wip/1082-box2-fix-validate`):

1. **`KILN_FORCE_EAGER_DECODE=1` (bs=1 eager every step, NO capture/replay) → COHERENT output** ("Here's a thinking process that could lead to the story above:"). So the bs=1 `model_forward_paged` forward is CORRECT; the doubling is **graph-replay-specific**. (The coherent graphs-off server baseline uses the *batched* forward, so it never tested the bs=1 forward — FORCE_EAGER does, and it's fine.)

2. **Replay logits ≠ eager logits for the same step:** `SAMESTEP EAGER_LOGITS sumsq=1851392` vs `REPLAY_LOGITS sumsq=1769472` (~4.4% diff). So the captured replay produces wrong logits → wrong argmax → the doubling. (Capture-step first-launch was correct per PASS1==FIRSTLAUNCH, so this is REPLAY-specific, not capture-compute.)

3. **The divergence starts at layer 0 / the input:** same-step per-layer norms show layer 0 differs (eager 4.34 vs replay 4.75), layer 1 differs (6.25 vs 7.5), layers 2+ mostly match. Layer-0 (embedding+first block) differing is consistent with a **stale token (or position) on replay** — the captured embedding reading a stale `token_buffer`/`position_buffer` → the model re-processes a near-stale input → re-emits → doubling. (Earlier metadata probe showed slot/seqused/block_table refresh correctly, but the TOKEN buffer specifically was not independently verified to take effect in the captured embedding on replay.)

**Caveat:** the SAME-STEP harness runs `eager_forward` before the replay (snapshot/restore linear_state, but its KV write persists idempotently), which adds noise — earlier it showed identical per-layer norms, now small layer-0/1 diffs. A cleaner localization needs a harness WITHOUT the eager-baseline's side effects.

**NEXT (precise):** (a) verify the per-replay `token_buffer`/`position_buffer` updates actually reach the captured embedding/RoPE on replay — dump the token_id the captured embedding reads vs the intended one; (b) if the token IS stale, the fix is in `update_token_buffer`/the captured embedding's buffer binding. Toolkit: `KILN_DEBUG_LAYER_NORMS` (per-layer + SAMESTEP eager-vs-replay + logits sumsq), `KILN_ARENA_FORCE_ZERO`, `KILN_FORCE_EAGER_DECODE`. Do NOT merge (output garbage).
