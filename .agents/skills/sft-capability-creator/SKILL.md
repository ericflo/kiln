...
8. **Baseline — eval-design failure goes both ways.** `./capability.oracle.sh ""` (empty adapter = base model). Log as iter 0 with `slug="baseline"`. Inspect the score:
   - **≥ 0.95** — *"your eval is too easy"* (not "capability solved"). Real 4B capabilities have failure modes; an eval that catches none is wrong. Inspect 3-5 base responses; negotiate with the user for a harder eval / anchor / per-class scoring; re-baseline. *The OPD sister-skill incorrectly abandoned its first capability at 0.99 — don't repeat that.*
   - **< 0.30 on a capability the 4B should partially have** — *"your eval is too strict."* Inspect 3-5 base responses BEFORE generating iter 1. The model may be producing reasonable output that the eval rejects on a surface-form technicality (cap #5 of OPD baseline read 0.19 because the rubric rejected fenced ```diff blocks; actual baseline once fixed was 0.85). Negotiate with the user to loosen the eval *without re-opening Goodhart holes*.
   - **0.0 or 0.01** (zero floor) — model has essentially no capability. Read §11 on answer-form; the model may be failing to produce the *form* the eval wants more than the *content*. iter 1 should be heavily anchored.
   - **In (0.10, 0.95)** — healthy. Proceed to iter 1.

   **A baseline that surprises you in either direction is a signal to inspect responses, not to start training.** Iter 1 against a mis-measured baseline almost always destroys the adapter (cap #5 OPD did this — composite collapsed from actual baseline 0.85 down to 0.10 when iter 1 ran against the wrong score).
