...
fn main() {
    // ROCm path (Phase R.7): hipcc-compile csrc/*.cu when `--features rocm`.
    // Independent of the CUDA arm below — only one GPU feature is active per
    // build in practice, but both are guarded by their own CARGO_FEATURE_*.
    if env::var("CARGO_FEATURE_ROCM").is_ok() {
        build_rocm();
    }

    // CUDA path: only compile when `--features cuda` is active (the crate's
    // default). Under `--features rocm` (cuda off) this arm is skipped so we
    // don't need an nvcc toolchain on a ROCm-only host.
...
/// hipcc-compile this crate's `csrc/*.cu` kernels into a static lib that the
/// Rust side links against. `cc::Build::cuda(true)` is hardwired to nvcc, so we
/// drive `hipcc` directly (mirrors `kiln-tensor/build.rs::build_rocm`). No-op
/// (with a `cargo:warning`) when ROCm is absent so `cargo check --features
/// rocm` stays green on a toolchain-less host.
///
/// The `.cu` sources `#include <cuda_runtime.h>` and `#include <cuda_bf16.h>`;
/// those resolve to kiln-tensor's CUDA->HIP compat shim
/// (`kiln-tensor/csrc/hip_compat`) under hipcc, with `kiln-tensor/csrc` on the
/// path for the shared `kt_gpu_compat.cuh`. This crate's own `csrc` is also on
/// the path for `causal_conv1d_update.h`.
fn build_rocm() {
    let rocm_root = match find_rocm_root() {
        Some(p) => p,
        None => {
            println!(
                "cargo:warning=ROCm not found, kiln-conv1d-kernel's ROCm kernels will not be \
    ...
    let manifest_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap());
    let csrc_dir = manifest_dir.join("csrc");
    // kiln-tensor's csrc holds the CUDA->HIP compat shim + the shared
    // kt_gpu_compat.cuh. Compute it relative to this crate's manifest dir
    // (siblings under crates/).
    ...
    const ROCM_KERNELS: &[&str] = &["causal_conv1d_update.cu"];
    ...
    for kernel in ROCM_KERNELS {
        ...
        // validate under real wave64 execution even on an RDNA dev box.
    ...
    let lib = out_dir.join("libkiln_conv1d_kernel_rocm_ops.a");
    ...
    println!("cargo:rustc-link-lib=static=kiln_conv1d_kernel_rocm_ops");
    println!(
        "cargo:rustc-link-search=native={}",
