...
Running `kiln` with no subcommand starts the OpenAI-compatible server, just like `kiln serve`. Commands such as `kiln health`, `kiln openenv train`, `kiln train sft`, `kiln train grpo`, `kiln train opd`, and `kiln adapters list` talk to a running server.
...
Common next steps:
  ...
  kiln openenv train  collect live environment episodes and train with GRPO
...
const TOP_LEVEL_EXAMPLES: &str = r#"Examples:
  ...
  kiln openenv train --environment http://127.0.0.1:8000 --output-adapter game-agent
      Run grouped stateful OpenEnv episodes, preserve environment rewards and
      trajectory identity, then submit the resulting on-policy GRPO batch.
...
pub enum Commands {
    ...
    /// Inspect OpenEnv servers, collect episodes, and train from live rewards
    #[command(
        subcommand,
        long_about = OPENENV_OVERVIEW,
        after_help = OPENENV_EXAMPLES
    )]
    Openenv(OpenEnvCommands),
