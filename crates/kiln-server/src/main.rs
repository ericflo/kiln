...
async fn main() -> Result<()> {
        ...
        Some(Commands::Train(ref train)) => match train {
            TrainCommands::Sft {
                ...
                return cli::run_train_sft(
                    ...
                    *lora_rank,
                    invalid_row_policy,
                    *adapter_smoke_test,
                    adapter_smoke_prompts_file.as_deref(),
                    *detect_anomaly,
            ...
            TrainCommands::Opd {
                ...
                return cli::run_train_opd(
                    ...
                    *lora_rank,
                    *detect_anomaly,
                    sampler_segments.map(std::num::NonZeroUsize::get),
                    rollout_prompt_rendering.as_deref(),
                    checkpoint_interval.map(std::num::NonZeroUsize::get),
                    resume_checkpoint.as_deref(),
                )
                .await;
    ...
    if let Some(agent_cfg) = config.agent.clone()
        && let Some(hours) = agent_cfg.self_improve_interval_hours.filter(|&h| h > 0)
    {
            ...
            loop {
                ...
                match kiln_server::api::self_improve::submit_self_improve(&scheduler_state, req) {
                    Ok(resp) => {
                        ...
                        status.last_result = Some(format!("queued {} job(s)", resp.job_ids.len()));
