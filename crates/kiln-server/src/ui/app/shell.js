  ...
  return `
    <div class="rc-groups">
      ...
      <div class="rc-group">
        ...
        ${runtimeConfigRow('Vulkan kernel policy', `<strong>${escapeHtml(acceleratorRuntime.vulkan_kernel_policy_schema_id || '—')}</strong>`, 'Portable immutable Vulkan selection and dispatch policy compiled into this build.')}
        ...
        ${runtimeConfigRow('Strided batched matmul', `<strong>${escapeHtml(rocmStridedBatchedMatmul.effective || '—')}</strong>${srcChip(rocmStridedBatchedMatmul.source)}`, 'Portable execution uses per-row GEMMs. Strided batching is an explicit experimental route.')}
        ${runtimeConfigRow('BF16 matmul output', `<strong>${escapeHtml(rocmBf16MatmulOutput.effective || '—')}</strong>${srcChip(rocmBf16MatmulOutput.source)}`, 'Portable execution uses F32 output followed by an on-device BF16 cast. Native BF16 output is an explicit experimental route.')}
        ${runtimeConfigRow('ROCm kernel profile', `<strong>${escapeHtml(rocmKernelProfile.effective || '—')}</strong>${srcChip(rocmKernelProfile.source)}`, 'Portable fallback is the only product profile. Machine-qualified route sets are isolated from server configuration.')}
