  ...
  return `
    <div class="rc-groups">
      ...
      <div class="rc-group">
        ...
        ${runtimeConfigRow('ROCm kernel profile', `<strong>${escapeHtml(rocmKernelProfile.effective || '—')}</strong>${srcChip(rocmKernelProfile.source)}`, 'Complete immutable 75-leaf ROCm model/tensor/flash policy. Qualified enables 43 of 45 accelerated routes and keeps multi-block GDN prefill plus correctness-disabled fused RMSNorm off. Portable fallback declines all accelerated routes; experimental multiblock adds only the multi-block route and still keeps fused RMSNorm off.')}
