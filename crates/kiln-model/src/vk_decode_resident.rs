...
pub fn record_full_attn_block_batched_into(
    ...
    let qkv_combined = backend.acquire_resident_scratch(
        "nfa_b_qkv_combined",
        (batch_size * total_qkv_out * 4) as u64,
    )?;
    ...
    batch.record_shader(
        ...
        Workgroups::OneD((batch_size * total_split).div_ceil(256) as u32),
    )?;
    batch.record_shader(
        shaders::QWEN_RMSNORM_QK_COMBINED,
        &[
...
pub fn record_gdn_block_batched_into(
    ...
    let mixed_qkv =
        backend.acquire_resident_scratch("ngd_b_mixed_qkv", (batch_size * qkv_dim * 4) as u64)?;
    let conv_qkv =
        backend.acquire_resident_scratch("ngd_b_conv_qkv", (batch_size * qkv_dim * 4) as u64)?;
    ...
    let conv_total = batch_size * qkv_dim;
    batch.record_shader(
        shaders::CAUSAL_CONV1D,
        &[
            mixed_qkv.handle(),
    ...
    batch.record_shader(
        ...
        &[batch_size as u32, qk_dim as u32, v_dim as u32],
        Workgroups::OneD((batch_size * qkv_dim).div_ceil(256) as u32),
    )?;
