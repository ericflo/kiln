    ...
    pub fn validate(self) -> Result<()> {
        match self {
            ...
            Self::FullAttention { layer_idx } => {
                    ...
                    layer_idx < shapes::NUM_LAYERS,
                    "full-attention layer index {layer_idx} out of range"
                );
                ensure!(
                    shapes::is_full_attention_layer(layer_idx),
                    "layer {layer_idx} is not a Qwen3.5 full-attention layer"
                );
                Ok(())
            }
            Self::LinearAttention { layer_idx } => {
                    ...
                    layer_idx < shapes::NUM_LAYERS,
                    "linear-attention layer index {layer_idx} out of range"
                );
                ensure!(
                    !shapes::is_full_attention_layer(layer_idx),
                    "layer {layer_idx} is not a Qwen3.5 GDN layer"
                );
                Ok(())
            }
            Self::Mlp { layer_idx } => {
                    ...
                    layer_idx < shapes::NUM_LAYERS,
                    "MLP layer index {layer_idx} out of range"
                );
                Ok(())
            }
        }
...
impl GpuPackedWeightRegistry {
    ...
    pub fn require(&self, key: PackedWeightKey) -> Result<&PackedWeightStorage> {
        self.get(key)
            .with_context(|| format!("missing packed-weight registry key: {key:?}"))
    }
    ...
    pub fn from_gpu_weights(weights: &GpuWeights) -> Result<Self> {
        ...
        registry.insert_bf16(
            RegistryLayer::Embedding,
            ProjectionKind::EmbedTokens,
            &weights.embed_tokens,
            false,
        )?;
        registry.insert_bf16(
            RegistryLayer::Embedding,
            ProjectionKind::LmHeadTied,
            &weights.embed_tokens_t,
            true,
        )?;
        registry.insert_bf16(
            RegistryLayer::FinalNorm,
            ProjectionKind::FinalNorm,
            &weights.final_norm,
            false,
        )?;
        ...
        for (layer_idx, layer) in weights.layers.iter().enumerate() {
            registry.insert_bf16(
                RegistryLayer::Mlp { layer_idx },
                ProjectionKind::InputLayerNorm,
                &layer.input_layernorm,
                false,
            )?;
            registry.insert_bf16(
                RegistryLayer::Mlp { layer_idx },
                ProjectionKind::PostAttentionLayerNorm,
            ...
            match &layer.attention {
                GpuAttentionWeights::Full(full) => {
                    ...
                    registry.insert_projection_or_marlin(
                        layer_key,
                        ProjectionKind::FullQ,
                        &full.q_proj_t,
                        full.q_proj_marlin.clone(),
                    )?;
                    ...
                    registry.insert_bf16(
                        layer_key,
                        ProjectionKind::FullQNorm,
                        &full.q_norm,
                        false,
                    )?;
                    registry.insert_bf16(
                        layer_key,
                        ProjectionKind::FullKNorm,
                        &full.k_norm,
                        false,
                    )?;
                }
                GpuAttentionWeights::Linear(linear) => {
                    ...
                    registry.insert_bf16(
                        layer_key,
                        ProjectionKind::GdnInProjQkv,
                        &linear.in_proj_qkv_t,
                        true,
                    )?;
                    registry.insert_bf16(
                        layer_key,
                        ProjectionKind::GdnInProjZ,
                        &linear.in_proj_z_t,
                        true,
                    )?;
                    registry.insert_bf16(
                        layer_key,
                        ProjectionKind::GdnOutProj,
                        &linear.out_proj_t,
                        true,
                    )?;
                    registry.insert_bf16(
                        layer_key,
                        ProjectionKind::GdnInProjA,
                        &linear.in_proj_a_t,
                        true,
                    )?;
                    registry.insert_bf16(
                        layer_key,
                        ProjectionKind::GdnInProjB,
                        &linear.in_proj_b_t,
                        true,
                    )?;
                    registry.insert_bf16(
                        layer_key,
                        ProjectionKind::GdnConv1d,
                        &linear.conv1d,
                        false,
                    )?;
                    registry.insert_bf16(
                        layer_key,
                        ProjectionKind::GdnNorm,
                        &linear.norm,
                        false,
                    )?;
                    registry.insert_bf16(
                        layer_key,
                        ProjectionKind::GdnALog,
                        &linear.a_log,
                        false,
                    )?;
                    registry.insert_bf16(
                        layer_key,
                        ProjectionKind::GdnALogGates,
                        &linear.a_log_gates,
                        false,
                    )?;
                    registry.insert_bf16(
                        layer_key,
                        ProjectionKind::GdnDtBias,
                        &linear.dt_bias,
                        false,
                    )?;
                }
            ...
            registry.insert_projection_or_marlin(
                mlp_key,
                ProjectionKind::MlpGate,
                &layer.mlp.gate_proj_t,
                layer.mlp.gate_proj_marlin.clone(),
            )?;
            registry.insert_projection_or_marlin(
                mlp_key,
                ProjectionKind::MlpUp,
                &layer.mlp.up_proj_t,
                layer.mlp.up_proj_marlin.clone(),
            )?;
            registry.insert_projection_or_marlin(
                mlp_key,
                ProjectionKind::MlpDown,
                &layer.mlp.down_proj_t,
                layer.mlp.down_proj_marlin.clone(),
            )?;
        }
        Ok(registry)
    }
    ...
    fn insert_projection_or_marlin(
        ...
        let storage = if let Some(packed) = marlin {
            PackedWeightStorage::MarlinW4A16 {
                k: packed.k,
                n: packed.n,
                packed,
            }
    ...
    fn registry_rejects_duplicate_keys() {
        ...
        let key =
            PackedWeightKey::new(RegistryLayer::Mlp { layer_idx: 0 }, ProjectionKind::MlpGate)
                .unwrap();
        let storage = PackedWeightStorage::Bf16 {
            tensor,
            dims: vec![2, 2],
            transposed: true,
        };
    ...
    fn registry_returns_compile_checked_keys() {
        ...
        registry
            .insert(
                ...
                PackedWeightStorage::Bf16 {
                    tensor,
                    dims: vec![shapes::HIDDEN, shapes::MLP_HIDDEN],
                    transposed: true,
                },
            )
            .unwrap();
