...
impl CudaTrainTensor {
    ...
    pub fn sgd_step_inplace(&self, grad: &CudaTrainTensor, lr: f32) -> Result<()> {
        ...
        // Phase 7 (#1082): kt-only. Bit-exact with the previous
        // candle shell because both paths call the same FFI symbol.
        ...
        let grad_kt = kiln_kt_bridge::kt_tensor_from_candle_cuda_borrow(&grad.tensor)
            ...
            kiln_rmsnorm_kernel::supports_optimizer_step_kt(&[&param_kt, &grad_kt]),
    ...
    pub fn adamw_step_inplace(
        ...
        // Phase 7 (#1082): kt-only. The previous candle shell
        // computed bias correction internally from `step`; this call
        // passes the same values explicitly.
        ...
        let m2_kt = kiln_kt_bridge::kt_tensor_from_candle_cuda_borrow(second_moment.as_tensor())
            ...
            kiln_rmsnorm_kernel::supports_optimizer_step_kt(&[
...
pub fn cuda_flash_attn_prefill_causal_bf16(
    ...
    // Phase 7 (#1082): kt-only. Same FFI symbol as the previous
    // candle shell; both outputs keep the same shapes and feed the
    // existing backward tape slots.
    ...
    let (out_kt, lse_kt) = kiln_flash_attn::flash_attn_fwd_kt(&q_kt, &k_kt, &v_kt, scale, true)
        .map_err(|e| anyhow::anyhow!("kt flash_attn_fwd: {e}"))?;
    let out_4d = kiln_kt_bridge::kt_tensor_to_candle_cuda_copy(&out_kt)
        .context("cuda_flash_attn_prefill_causal_bf16: kt out -> candle copy")?;
    let softmax_lse = kiln_kt_bridge::kt_tensor_to_candle_cuda_copy(&lse_kt)
...
impl CudaBackwardOp for FlashAttnPrefillCausalBackward {
    ...
    fn backward(&self, grad_out: &CudaTrainTensor) -> Result<Vec<Option<CudaTrainTensor>>> {
        ...
        // Phase 7 (#1082): kt-only. The SFT/GRPO prefill backward path
        // always has dout/q/k/v/out in BF16 and softmax_lse in F32. The kt
        // shell calls the same FFI symbol as the previous candle shell; it
        // returns expanded heads_q-shaped dk/dv, so GQA still collapses here.
