...
pub fn assert_matches_config(config: &kiln_core::config::ModelConfig) -> Result<()> {
    ...
    ensure!(config.num_attention_heads == NUM_HEADS, "Qwen3.5 num_attention_heads drift");
    ensure!(config.num_kv_heads == NUM_KV_HEADS, "Qwen3.5 num_kv_heads drift");
    ensure!(config.head_dim == HEAD_DIM, "Qwen3.5 head_dim drift");
    ensure!(config.intermediate_size == MLP_HIDDEN, "Qwen3.5 intermediate_size drift");
    ensure!(config.vocab_size == VOCAB, "Qwen3.5 vocab_size drift");
    ensure!(config.max_position_embeddings == MAX_POSITION_EMBEDDINGS, "Qwen3.5 max_position_embeddings drift");
    ensure!(config.num_full_attention_layers == NUM_FULL_ATTN_LAYERS, "Qwen3.5 full-attention count drift");
    ensure!(config.full_attention_interval == FULL_ATTN_INTERVAL, "Qwen3.5 full-attention interval drift");
    ensure!(config.attn_output_gate == ATTN_OUTPUT_GATE, "Qwen3.5 output gate drift");
    ensure!(config.linear_num_key_heads == GDN_NUM_KEY_HEADS, "Qwen3.5 GDN key heads drift");
    ensure!(config.linear_key_head_dim == GDN_KEY_HEAD_DIM, "Qwen3.5 GDN key dim drift");
    ensure!(config.linear_num_value_heads == GDN_NUM_VALUE_HEADS, "Qwen3.5 GDN value heads drift");
    ensure!(config.linear_value_head_dim == GDN_VALUE_HEAD_DIM, "Qwen3.5 GDN value dim drift");
    ensure!(config.linear_conv_kernel_dim == GDN_CONV_KERNEL_DIM, "Qwen3.5 GDN conv drift");
    ensure!((config.partial_rotary_factor - PARTIAL_ROTARY).abs() < f64::EPSILON, "Qwen3.5 partial rotary drift");
    ensure!((config.rope_theta - ROPE_THETA).abs() < f64::EPSILON, "Qwen3.5 rope theta drift");
    ensure!(config.rotary_dim() == ROTARY_DIM, "Qwen3.5 rotary dim drift");
    Ok(())
}
...
mod tests {
    ...
    fn attention_layer_indexing_matches_24_8_split() {
        let full: Vec<_> = (0..NUM_LAYERS).filter(|&idx| is_full_attention_layer(idx)).collect();
        assert_eq!(full, vec![0, 4, 8, 12, 16, 20, 24, 28]);
        assert_eq!(full.len(), NUM_FULL_ATTN_LAYERS);
        assert_eq!((0..NUM_LAYERS).filter_map(gdn_layer_index).count(), NUM_GDN_LAYERS);
