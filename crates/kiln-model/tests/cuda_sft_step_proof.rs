//! Bounded single SFT-style training step on `Device::Cuda` through the kt-tape
//! substrate. CUDA twin of `rocm_sft_step_proof.rs`.
//!
//! Proves the SFT training step runs end-to-end on CUDA in three tiers:
...
fn build_lora_fixtures() -> (Tensor, Tensor, LoraProjectionWeights) {
    ...
    let a = Tensor::from_vec_on(Device::Cuda(0), a_data, vec![RANK, HIDDEN]).expect("a");
    let b = Tensor::from_vec_on(Device::Cuda(0), b_data, vec![OUT, RANK]).expect("b");
...
#[test]
fn cuda_sft_lora_linear_backprops() {
    if !cuda_enabled("cuda_sft_lora_linear_backprops") {
        return;
    }
    ...
    let (out, tape) = with_thread_local_tape(|| {
        try_tape_lora_linear_kt(&x, &weight_t, Some(&lora), LORA_SCALE)
            ...
            .expect("try_tape_lora_linear_kt returned None on CUDA")
    });
    ...
    assert_eq!(out.device(), Device::Cuda(0), "forward output left CUDA");
    ...
    eprintln!(
        "[CUDA TIER1 PASS] tape.len()={} out={out_v:?} dA={da_v:?} dB={db_v:?}",
        tape.len()
    );
}
...
fn run_sft_forward_loss_backward() -> (f32, Tensor, Tensor, LoraProjectionWeights) {
    ...
    let (loss, tape) = with_thread_local_tape(|| {
        ...
        try_tape_cross_entropy_from_logits_kt(&logits, &input_ids, &label_mask)
            ...
            .expect("CE recorder returned None on CUDA")
    });

    assert_eq!(loss.device(), Device::Cuda(0), "CE loss left CUDA");
...
#[test]
fn cuda_sft_cross_entropy_backprops() {
    if !cuda_enabled("cuda_sft_cross_entropy_backprops") {
        return;
    }
    let (loss, da, db, _lora) = run_sft_forward_loss_backward();
    eprintln!(
        "[CUDA TIER2 PASS] SFT CE loss={loss:.6} dA={:?} dB={:?}",
        read_host_f32(&da),
        read_host_f32(&db)
    );
}
...
#[test]
fn cuda_sft_one_adamw_step_changes_params() {
    if !cuda_enabled("cuda_sft_one_adamw_step_changes_params") {
        return;
    }
    let (loss, da, db, lora) = run_sft_forward_loss_backward();
    assert!(loss.is_finite(), "non-finite CE loss: {loss}");
    assert_eq!(da.device(), Device::Cuda(0), "dA left CUDA");
    assert_eq!(db.device(), Device::Cuda(0), "dB left CUDA");
    ...
    eprintln!(
        "[CUDA TIER3 PASS] one full SFT step on Device::Cuda(0): CE loss={loss:.6}\n  \
