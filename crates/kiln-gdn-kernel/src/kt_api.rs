...
use crate::{
    ...
    kiln_gdn_forward_substitution_f32, kiln_gdn_full_chunk_forward,
    kiln_gdn_full_chunk_forward_multiblock, kiln_gdn_gated_rms_norm_bf16,
    kiln_gdn_gated_rms_norm_bwd_bf16, kiln_gdn_gated_rms_norm_bwd_frozen_bf16,
    kiln_gdn_gated_rms_norm_bwd_frozen_wf32_bf16, kiln_gdn_gated_rms_norm_bwd_wf32_bf16,
    kiln_gdn_gated_rms_norm_wf32_bf16, kiln_gdn_gates_bf16, kiln_gdn_gates_bf16_f32_bf16_params,
    kiln_gdn_gates_bf16_f32_params, kiln_gdn_l2_norm_scale_bwd_bf16, kiln_gdn_recurrent_forward,
...
/// its own output's initialization. Callers must still synchronize ROCm input
/// tensor streams before launching here, because long-context training often
/// feeds tensors produced by earlier device kernels rather than host uploads.
...
#[cfg(feature = "rocm")]
fn synchronize_rocm_input_streams(
    label: &'static str,
    tensors: &[(&'static str, &KtTensor)],
) -> Result<(), GdnError> {
    for (name, tensor) in tensors {
        if matches!(tensor.device(), KtDevice::Rocm(_)) {
            kiln_tensor::rocm_synchronize_tensor_stream(tensor).map_err(|e| {
                GdnError::Msg(format!(
                    "kt-gdn: {label}: synchronize ROCm input {name}: {e}"
                ))
            })?;
        }
    }
...
fn device_synchronize_after_launch(t: &KtTensor) -> Result<(), GdnError> {
    if matches!(t.device(), KtDevice::Rocm(_)) {
        kiln_tensor::rocm_synchronize_tensor_stream(t).map_err(|e| {
            GdnError::Msg(format!("kt-gdn: synchronize ROCm output after launch: {e}"))
        })?;
    }
    Ok(())
}
...
/// F32 training/backward variant of [`gdn_forward_substitution_kt`].
///
/// Shapes:
/// - `a_strict`: F32 `[B, H, C, C]`
/// - `v_prime`:  F32 `[B, H, C, dv]`
/// - `beta`:     F32 `[B, H, C]`
///
/// Returns F32 `W` of shape `[B, H, C, dv]`.
#[cfg(any(feature = "cuda", feature = "rocm"))]
pub fn gdn_forward_substitution_f32_kt(
    a_strict: &KtTensor,
    v_prime: &KtTensor,
    beta: &KtTensor,
) -> Result<KtTensor, GdnError> {
    ...
    if a_shape.len() != 4 {
        return Err(GdnError::Msg(format!(
            "kt-gdn: f32 a_strict must be rank-4, got {a_shape:?}"
        )));
    }
    let (b, h, c, c2) = (a_shape[0], a_shape[1], a_shape[2], a_shape[3]);
    if c != c2 {
        return Err(GdnError::Msg(format!(
            "kt-gdn: f32 a_strict must be square on last two axes, got [_, _, {c}, {c2}]"
        )));
    }
    let vp_shape = v_prime.shape();
    if vp_shape.len() != 4 {
        return Err(GdnError::Msg(format!(
            "kt-gdn: f32 v_prime must be rank-4, got {vp_shape:?}"
        )));
    }
    let dv = vp_shape[3];
    if (vp_shape[0], vp_shape[1], vp_shape[2]) != (b, h, c) {
        return Err(GdnError::Msg(format!(
            "kt-gdn: f32 v_prime {vp_shape:?} mismatch with a_strict prefix [{b}, {h}, {c}]"
        )));
    }
    let bt_shape = beta.shape();
    if bt_shape.len() != 3 || (bt_shape[0], bt_shape[1], bt_shape[2]) != (b, h, c) {
        return Err(GdnError::Msg(format!(
            "kt-gdn: f32 beta {bt_shape:?} mismatch with [{b}, {h}, {c}]"
        )));
    }
    if c > 128 {
        return Err(GdnError::Msg(format!(
            "kt-gdn: f32 chunk_size must be <= 128, got {c}"
        )));
    }

    let a_ptr = kiln_kt_bridge::device_input_ptr(a_strict, KtDType::F32, "a_strict")?;
    let (a_st, _) = cuda_storage_and_byte_offset(a_strict, KtDType::F32, "a_strict")?;
    let vp_ptr = kiln_kt_bridge::device_input_ptr(v_prime, KtDType::F32, "v_prime")?;
    let _ = cuda_storage_and_byte_offset(v_prime, KtDType::F32, "v_prime")?;
    let bt_ptr = kiln_kt_bridge::device_input_ptr(beta, KtDType::F32, "beta")?;
    let _ = cuda_storage_and_byte_offset(beta, KtDType::F32, "beta")?;

    let w_out = alloc_cuda_tensor(a_st, KtDType::F32, vec![b, h, c, dv])?;
    let w_ptr = kiln_kt_bridge::device_output_ptr(&w_out);
...
/// F32 adjoint triangular solve used by the analytic GDN backward.
///
...
/// - `dw`:       F32 `[B, H, C, dv]`
///
/// Returns F32 `dr` of shape `[B, H, C, dv]`.
#[cfg(any(feature = "cuda", feature = "rocm"))]
pub fn gdn_solve_tri_transpose_f32_kt(
    a_strict: &KtTensor,
    beta: &KtTensor,
    dw: &KtTensor,
) -> Result<KtTensor, GdnError> {
    ...
    if a_shape.len() != 4 {
        return Err(GdnError::Msg(format!(
            "kt-gdn: solve_tri_transpose a_strict must be rank-4, got {a_shape:?}"
        )));
    }
    let (b, h, c, c2) = (a_shape[0], a_shape[1], a_shape[2], a_shape[3]);
    if c != c2 {
        return Err(GdnError::Msg(format!(
            "kt-gdn: solve_tri_transpose a_strict must be square on last two axes, got [_, _, {c}, {c2}]"
        )));
    }
    let dw_shape = dw.shape();
    if dw_shape.len() != 4 {
        return Err(GdnError::Msg(format!(
            "kt-gdn: solve_tri_transpose dw must be rank-4, got {dw_shape:?}"
        )));
    }
    let dv = dw_shape[3];
    if (dw_shape[0], dw_shape[1], dw_shape[2]) != (b, h, c) {
        return Err(GdnError::Msg(format!(
            "kt-gdn: solve_tri_transpose dw {dw_shape:?} mismatch with a_strict prefix [{b}, {h}, {c}]"
        )));
    }
    let beta_shape = beta.shape();
    if beta_shape.len() != 3 || (beta_shape[0], beta_shape[1], beta_shape[2]) != (b, h, c) {
        return Err(GdnError::Msg(format!(
            "kt-gdn: solve_tri_transpose beta {beta_shape:?} mismatch with [{b}, {h}, {c}]"
        )));
    }
    if c > 128 {
        return Err(GdnError::Msg(format!(
            "kt-gdn: solve_tri_transpose chunk_size must be <= 128, got {c}"
        )));
    }

    let a_ptr = kiln_kt_bridge::device_input_ptr(a_strict, KtDType::F32, "a_strict")?;
    let (a_st, _) = cuda_storage_and_byte_offset(a_strict, KtDType::F32, "a_strict")?;
    let beta_ptr = kiln_kt_bridge::device_input_ptr(beta, KtDType::F32, "beta")?;
    let _ = cuda_storage_and_byte_offset(beta, KtDType::F32, "beta")?;
    let dw_ptr = kiln_kt_bridge::device_input_ptr(dw, KtDType::F32, "dw")?;
    let _ = cuda_storage_and_byte_offset(dw, KtDType::F32, "dw")?;

    let dr_out = alloc_cuda_tensor(a_st, KtDType::F32, vec![b, h, c, dv])?;
...
/// for `weight`. Returns BF16 `dx`/`dz` and F32 `dw` accumulated across rows.
#[cfg(any(feature = "cuda", feature = "rocm"))]
pub struct GdnGatedRmsNormBwdKt {
    pub dx: KtTensor,
    pub dz: KtTensor,
    pub dw: KtTensor,
}

/// Fused gated RMSNorm gradients when the normalization weight is frozen.
///
/// Only activation gradients are exposed. The corresponding GPU launch does
/// not allocate a weight-gradient tensor and uses a kernel instantiation with
/// the cross-row dWeight atomic reduction compiled out.
#[cfg(any(feature = "cuda", feature = "rocm"))]
pub struct GdnGatedRmsNormFrozenWeightBwdKt {
    pub dx: KtTensor,
    pub dz: KtTensor,
}

#[cfg(any(feature = "cuda", feature = "rocm"))]
struct GdnGatedRmsNormBwdLaunchKt {
    dx: KtTensor,
    dz: KtTensor,
    dw: Option<KtTensor>,
}
...
#[cfg(any(feature = "cuda", feature = "rocm"))]
pub fn gdn_gated_rms_norm_bwd_bf16_kt(
    grad_out: &KtTensor,
    x: &KtTensor,
    z: &KtTensor,
    weight: &KtTensor,
    ...
    let launch = gdn_gated_rms_norm_bwd_impl(grad_out, x, z, weight, eps, KtDType::BF16, true)?;
    let GdnGatedRmsNormBwdLaunchKt { dx, dz, dw } = launch;
    let dw = dw.ok_or_else(|| {
        GdnError::Msg("kt-gdn: trainable gated-rmsnorm backward omitted dWeight".to_string())
    })?;
    Ok(GdnGatedRmsNormBwdKt { dx, dz, dw })
}

/// Fused BF16 gated RMSNorm backward with a frozen BF16 weight.
///
/// Returns only BF16 `dx` and `dz`. No dWeight buffer is allocated, and the
/// selected kernel instantiation performs no dWeight atomic reduction.
#[cfg(any(feature = "cuda", feature = "rocm"))]
pub fn gdn_gated_rms_norm_bwd_bf16_frozen_weight_kt(
    grad_out: &KtTensor,
    x: &KtTensor,
    z: &KtTensor,
    weight: &KtTensor,
    eps: f32,
) -> Result<GdnGatedRmsNormFrozenWeightBwdKt, GdnError> {
    let launch = gdn_gated_rms_norm_bwd_impl(grad_out, x, z, weight, eps, KtDType::BF16, false)?;
    debug_assert!(launch.dw.is_none());
    Ok(GdnGatedRmsNormFrozenWeightBwdKt {
        dx: launch.dx,
        dz: launch.dz,
    })
}

/// Fused gated RMSNorm backward with BF16 activations and a trainable F32
/// weight. Returns BF16 `dx`/`dz` and an F32 `dw` accumulated across rows.
#[cfg(any(feature = "cuda", feature = "rocm"))]
pub fn gdn_gated_rms_norm_bwd_bf16_f32_weight_kt(
    ...
    let launch = gdn_gated_rms_norm_bwd_impl(grad_out, x, z, weight, eps, KtDType::F32, true)?;
    let GdnGatedRmsNormBwdLaunchKt { dx, dz, dw } = launch;
    let dw = dw.ok_or_else(|| {
        GdnError::Msg("kt-gdn: trainable gated-rmsnorm backward omitted dWeight".to_string())
    })?;
...
/// Fused BF16 gated RMSNorm backward with a frozen F32 weight.
///
/// Returns only BF16 `dx` and `dz`. No dWeight buffer is allocated, and the
/// selected kernel instantiation performs no dWeight atomic reduction.
#[cfg(any(feature = "cuda", feature = "rocm"))]
pub fn gdn_gated_rms_norm_bwd_bf16_f32_weight_frozen_kt(
    grad_out: &KtTensor,
    x: &KtTensor,
    z: &KtTensor,
    weight: &KtTensor,
    ...
    let launch = gdn_gated_rms_norm_bwd_impl(grad_out, x, z, weight, eps, KtDType::F32, false)?;
    debug_assert!(launch.dw.is_none());
    Ok(GdnGatedRmsNormFrozenWeightBwdKt {
        dx: launch.dx,
        dz: launch.dz,
...
#[cfg(any(feature = "cuda", feature = "rocm"))]
fn gdn_gated_rms_norm_bwd_impl(
    grad_out: &KtTensor,
    x: &KtTensor,
    z: &KtTensor,
    weight: &KtTensor,
    eps: f32,
    weight_dtype: KtDType,
    compute_weight_grad: bool,
) -> Result<GdnGatedRmsNormBwdLaunchKt, GdnError> {
    let x_shape = x.shape();
    if x_shape.len() != 2 {
        return Err(GdnError::Msg(format!(
            "kt-gdn: gated-rmsnorm-bwd x must be [rows, hidden], got {x_shape:?}"
        )));
    }
    let (rows, hidden) = (x_shape[0], x_shape[1]);
    if z.shape() != x_shape {
        return Err(GdnError::Msg(format!(
            "kt-gdn: gated-rmsnorm-bwd z {:?} != x {x_shape:?}",
            z.shape()
        )));
    }
    if grad_out.shape() != x_shape {
        return Err(GdnError::Msg(format!(
            "kt-gdn: gated-rmsnorm-bwd grad_out {:?} != x {x_shape:?}",
            grad_out.shape()
        )));
    }
    if weight.shape() != [hidden] {
        return Err(GdnError::Msg(format!(
            "kt-gdn: gated-rmsnorm-bwd weight {:?} != [{hidden}]",
            weight.shape()
        )));
    }
    if grad_out.device() != x.device() || z.device() != x.device() || weight.device() != x.device()
    {
        return Err(GdnError::Msg(format!(
            "kt-gdn: gated-rmsnorm-bwd inputs must share one device, got grad_out={:?}, x={:?}, z={:?}, weight={:?}",
            grad_out.device(),
            x.device(),
            z.device(),
            weight.device()
    ...
    let z_ptr = kiln_kt_bridge::device_input_ptr(z, KtDType::BF16, "z")?;
    let _ = cuda_storage_and_byte_offset(z, KtDType::BF16, "z")?;
    let w_ptr = kiln_kt_bridge::device_input_ptr(weight, weight_dtype, "weight")?;
    let _ = cuda_storage_and_byte_offset(weight, weight_dtype, "weight")?;

    let dx = alloc_cuda_tensor(go_st, KtDType::BF16, vec![rows, hidden])?;
    let dz = alloc_cuda_tensor(go_st, KtDType::BF16, vec![rows, hidden])?;
    let dw = if compute_weight_grad {
        Some(alloc_cuda_tensor(go_st, KtDType::F32, vec![hidden])?)
    } else {
        None
    };
    ...
    let status = unsafe {
        match (weight_dtype, compute_weight_grad) {
            ...
            (KtDType::BF16, false) => kiln_gdn_gated_rms_norm_bwd_frozen_bf16(
                go_ptr as *const _,
                x_ptr as *const _,
                z_ptr as *const _,
                w_ptr as *const _,
            ...
            (KtDType::F32, true) => kiln_gdn_gated_rms_norm_bwd_wf32_bf16(
                go_ptr as *const _,
                x_ptr as *const _,
                z_ptr as *const _,
                w_ptr as *const _,
            ...
            (KtDType::F32, false) => kiln_gdn_gated_rms_norm_bwd_frozen_wf32_bf16(
                go_ptr as *const _,
                x_ptr as *const _,
                z_ptr as *const _,
                w_ptr as *const _,
            ...
            _ => unreachable!("gated-rmsnorm backward weight dtype is BF16 or F32"),
        }
    };
    if status != 0 {
        stream_submission.quarantine();
...
/// Fused BF16 backward for the GDN q/k L2 norm scale op.
///
/// Inputs are BF16 `[rows, 128]` for `grad_out` and `x`. Returns BF16
/// `[rows, 128]` for `dx`.
#[cfg(any(feature = "cuda", feature = "rocm"))]
pub fn gdn_l2_norm_scale_bwd_bf16_kt(
    grad_out: &KtTensor,
    x: &KtTensor,
    scale: f32,
    eps: f32,
    ...
    if x_shape.len() != 2 {
        return Err(GdnError::Msg(format!(
            "kt-gdn: l2-norm-scale-bwd x must be [rows, hidden], got {x_shape:?}"
        )));
    }
    let (rows, hidden) = (x_shape[0], x_shape[1]);
    if grad_out.shape() != x_shape {
        return Err(GdnError::Msg(format!(
            "kt-gdn: l2-norm-scale-bwd grad_out {:?} != x {x_shape:?}",
            grad_out.shape()
        )));
    }
...
pub fn gdn_chunk_scan_kt(
    ...
    let stream_submission = rocm_launch_stream(
        ...
        &[
            ...
            ("b_mask", b_mask),
            ("v_prime", v_prime),
            ("q_s_scaled", q_s_scaled),
            ("beta", beta),
            ("decay_last_col", decay_last_col),
        ],
    )?;
...
#[cfg(any(feature = "cuda", feature = "rocm"))]
pub fn gdn_l2_norm_scale_bwd_supports_kt(grad_out: &KtTensor, x: &KtTensor) -> bool {
    let shape = x.shape();
    x.dtype() == KtDType::BF16
        && grad_out.dtype() == KtDType::BF16
        && grad_out.shape() == shape
        && grad_out.device() == x.device()
        && shape.len() == 2
        && shape[1] == 128
}
