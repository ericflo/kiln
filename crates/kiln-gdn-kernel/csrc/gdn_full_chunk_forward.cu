...
// Multi-block dv-tiled variant of `gdn_full_chunk_forward_kernel`.
//
// The single-block kernel above launches one block per `(B, H)` pair. For
// Qwen3.5-4B with batch=1 that is only 32 blocks — well below the 76 SMs on
// an RTX 4090 Laptop. Because each thread `tid` in the chunk-body owns one
// dv column and forward-substitution only reads `s_w[i*dv + tid]` (its own
// column), the work along dv decomposes across blocks without any cross-block
// data dependency. The arithmetic per output column is unchanged, so the
// kernel is bit-exact with the single-block path.
//
// `DV_TILE` is the number of dv columns each block owns; each block has
// `DV_TILE` threads and handles one (B, H, dv_tile_lo .. dv_tile_lo+DV_TILE)
// slice. `dv` must be a multiple of `DV_TILE`.
template <int DV_TILE>
__global__ void gdn_full_chunk_forward_mb_kernel(
    const __nv_bfloat16 *__restrict__ g,
    const __nv_bfloat16 *__restrict__ v,
    const __nv_bfloat16 *__restrict__ kkt,
    const __nv_bfloat16 *__restrict__ qkt,
    ...
    const int bh = blockIdx.x / dv_tiles;
    const int tile = blockIdx.x % dv_tiles;
    const int dv_lo = tile * DV_TILE;
    const int tid = threadIdx.x;
    if (tid >= DV_TILE) {
        return;
    }
    const int j = dv_lo + tid; // global dv column owned by this thread

    extern __shared__ __nv_bfloat16 smem[];
    __nv_bfloat16 *s_a = smem;                                       // [C, C]
    __nv_bfloat16 *s_b = s_a + (size_t)kChunkSize * kChunkSize;      // [C, C]
    __nv_bfloat16 *s_w = s_b + (size_t)kChunkSize * kChunkSize;      // [C, DV_TILE]
    ...
    for (int t = 0; t < kChunkSize; ++t) {
        ...
        const __nv_bfloat16 *a_row = s_a + (size_t)t * kChunkSize;
        const __nv_bfloat16 *b_row = s_b + (size_t)t * kChunkSize;
        __nv_bfloat16 *w_row = s_w + (size_t)t * DV_TILE;

        float acc_a = 0.0f;
        #pragma unroll
        for (int i = 0; i < kChunkSize; ++i) {
            if (i < t) {
                acc_a += bf16_to_f32(a_row[i]) * bf16_to_f32(s_w[(size_t)i * DV_TILE + tid]);
            }
        }

        const size_t td = (size_t)t * dv + j;
        const float vp = bf16_to_f32(f32_to_bf16(
            bf16_to_f32(v_base[td]) - bf16_to_f32(ks_base[td]) * p_t
        ));
        const float w_val = beta_t * (vp - acc_a);
        ...
        for (int i = 0; i < kChunkSize; ++i) {
            if (i <= t) {
                acc_b += bf16_to_f32(b_row[i]) * bf16_to_f32(s_w[(size_t)i * DV_TILE + tid]);
            }
        }
    ...
    for (int k_idx = 0; k_idx < dk; ++k_idx) {
        ...
        for (int t = 0; t < kChunkSize; ++t) {
            const float kt = bf16_to_f32(kt_base[(size_t)k_idx * kChunkSize + t]);
            const float w = bf16_to_f32(s_w[(size_t)t * DV_TILE + tid]);
            const float decay_last = bf16_to_f32(f32_to_bf16(s_decay_last[t]));
            const float w_weighted = bf16_to_f32(f32_to_bf16(w * decay_last));
            delta += kt * w_weighted;
        }
        const size_t state_idx = (size_t)k_idx * dv + j;
        const float prev = bf16_to_f32(state_base[state_idx]);
        state_base[state_idx] = f32_to_bf16(prev * p_last + delta);
    }
}
...
extern "C" kiln_gdn_full_chunk_forward_status_t kiln_gdn_full_chunk_forward_multiblock(
    const void *g,
    const void *v,
    const void *kkt,
    const void *qkt,
    ...
    // Only support DV_TILE=32 for now. The kernel is templated so adding
    // other tile sizes is a one-line instantiation; keep the surface area
    // small until we have a use for more.
    if (dv_tile != 32) {
        return -3;
    }
    if (dv % dv_tile != 0) {
    ...
    const int dv_tiles = dv / dv_tile;
    const int blocks = batch_heads * dv_tiles;
    const int threads = dv_tile;
    const size_t smem_bytes =
        ((size_t)kChunkSize * kChunkSize * 2 + (size_t)kChunkSize * dv_tile) *
        sizeof(__nv_bfloat16);

    cudaStream_t s = reinterpret_cast<cudaStream_t>(stream);
    gdn_full_chunk_forward_mb_kernel<32><<<blocks, threads, smem_bytes, s>>>(
        reinterpret_cast<const __nv_bfloat16 *>(g),
        reinterpret_cast<const __nv_bfloat16 *>(v),
        reinterpret_cast<const __nv_bfloat16 *>(kkt),
        reinterpret_cast<const __nv_bfloat16 *>(qkt),
