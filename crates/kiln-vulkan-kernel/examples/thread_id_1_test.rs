//! Test thread ID shader with std430 buffer.
...
fn main() -> Result<()> {
    ...
    // Compile shader
    let spirv_bytes = kiln_vulkan_kernel::pipeline::ShaderPipeline::compile_shader("/tmp/thread_id_1.comp")?;
    ...
    // Buffer view (R32_SFLOAT for std430 float buffer)
    ...
    println!("result = {:?}", data);
    println!("expected [1,2,3,4] = {:?}", data == vec![1.0, 2.0, 3.0, 4.0]);
