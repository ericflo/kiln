//! Test storage buffer (not texel buffer).
...
fn main() -> Result<()> {
    ...
    // Compile shader
    let spirv_bytes = kiln_vulkan_kernel::pipeline::ShaderPipeline::compile_shader("/tmp/thread_id_storage.comp")?;
    ...
    // DSL - using STORAGE_BUFFER (not texel buffer)
    ...
    let dps = vk::DescriptorPoolSize::builder()
        .ty(vk::DescriptorType::STORAGE_BUFFER).descriptor_count(1).build();
    ...
    // Update descriptor with buffer info (not texel buffer view)
    let bui = vk::DescriptorBufferInfo::builder()
        .buffer(buf).offset(0).range(buf_size).build();
    ...
    unsafe {
        ...
        (*p).descriptor_type = vk::DescriptorType::STORAGE_BUFFER;
        (*p).p_buffer_info = &bui as *const _;
    }
