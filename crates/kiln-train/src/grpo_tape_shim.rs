...
use crate::trainer::{
    GrpoLossParams, entropy_aware_kl_mask_kt, grpo_loss_with_kl_auxiliary_route,
    selected_logits_from_chunk_sparse, token_log_probs,
};
#[cfg(any(
    feature = "cuda",
    feature = "metal",
...
struct GrpoPgLossFromLogitsBackward {
    ...
    /// Backend-owned GRPO auxiliary route for KL threshold and coeff fast paths.
...
pub(crate) fn grpo_scalar_seed_or_unit_kt(
    ...
    grad_output
        ...
        .first()
        .copied()
        .ok_or_else(|| kiln_tensor::Error::Msg(format!("{op_name}: empty grad_output")))
        .map(|v| v as f64)
}
...
fn prepare_vulkan_grpo_active_hidden(
    ...
    let (active_positions_usize, active_labels) =
        active_positions_and_labels(input_ids, action_mask)
    ...
    let active_positions: Vec<u32> = active_positions_usize
        ...
        .map(|&pos| {
            u32::try_from(pos).map_err(|_| {
                vulkan_tensor_err(format!("vulkan GRPO: active position {pos} exceeds u32"))
            })
        })
...
> {
    ...
    let Some(kernel_params) = vulkan_grpo_fused_kernel_params(loss_params, num_active) else {
        return Ok(None);
    };
...
fn grpo_loss_coeff_col_device_fast_path_kt(
    ...
    let device_supported = matches!(
        ...
        {
            matches!(device, Device::Cuda(_) | Device::Rocm(_))
        }
    ...
    {
        let mask = entropy_aware_kl_mask_kt(grpo_kl_auxiliary_route, &policy, loss_params, device)?
...
struct GrpoPgLossFromNormedHiddenBackward {
    ...
    /// §5.5 verifier-free mode: the policy-gradient term is masked — the
    /// backward emits ONLY the env-CE rows (validation guarantees
    /// `echo_env` is present when this is set).
    no_pg: bool,
}
...
impl BackwardOp for GrpoPgLossFromNormedHiddenBackward {
    ...
    fn apply(
        ...
        // §5.5 verifier-free mode: the PG term is masked — only the env-CE
        // rows drive. Build a zero base instead of the PG hidden-grad.
        let grad_hidden = if self.no_pg {
            let dims = self.normed_hidden.dims().to_vec();
            kiln_tensor::Tensor::zeros(dims, kiln_tensor::DType::F32, self.device)
                .and_then(|t| t.to_dtype(self.normed_hidden.dtype()))
                .map_err(|e| {
                    kiln_tensor::Error::Msg(format!(
                        "GrpoPgLossFromNormedHiddenBackward: no_pg zero grad: {e}"
                    ))
                })?
        } else {
            grpo_pg_loss_from_normed_hidden_grad_with_logsum_kt(
...
fn grpo_pg_loss_from_normed_hidden_grad_with_logsum_kt(
    ...
    if chunk_size == 0 {
        anyhow::bail!(
            "grpo_pg_loss_from_normed_hidden_grad_with_logsum_kt: chunk_size must be > 0"
        );
    }
    let dims = normed_hidden.dims().to_vec();
    anyhow::ensure!(
    ...
    let mut grad_active_hidden =
        KtTensor::zeros(vec![num_active, hidden_size], KtDType::F32, *device).context(
            "grpo_pg_loss_from_normed_hidden_grad_with_logsum_kt: grad_active_hidden zeros",
        )?;
    ...
    ) && matches!(device, Device::Cuda(_))
    {
        ...
        while chunk_start < vocab {
            ...
            .context(
                "grpo_pg_loss_from_normed_hidden_grad_with_logsum_kt: cuda grad logits chunk",
            )?;
        ...
        let grad_hidden_2d =
            scatter_add(&grad_active_hidden.contiguous()?, 0, active_idx, seq_len)?;
    ...
    while chunk_start < vocab {
        ...
        let chunk_contrib = if row_hits.is_empty() {
            ...
            let selected_contrib =
                scatter_add(&selected_rows.contiguous()?, 0, &row_idx, num_active)?;
    ...
    let grad_hidden_2d = scatter_add(&grad_active_hidden.contiguous()?, 0, active_idx, seq_len)?;
    let grad_hidden = grad_hidden_2d.unsqueeze(0)?.to_dtype(hidden_dtype)?;
    Ok(grad_hidden)
}
...
pub(crate) fn grpo_pg_loss_from_normed_hidden_loss_and_grad_kt(
    ...
    anyhow::ensure!(
        !no_pg || echo_state.is_some(),
        "grpo_pg_loss_from_normed_hidden_loss_and_grad_kt: no_policy_loss with no \
         env-CE rows in this completion — nothing to train on"
    );
    let (loss_kt, grad_hidden, env_ce) = match echo_state {
        ...
        Some((node_state, env_ce_kt, env_ce_val)) => {
            ...
            // §5.5 verifier-free mode: env rows are the WHOLE gradient.
            let grad_hidden = if no_pg {
                env_grad
            } else {
                (&grad_hidden + &env_grad).map_err(|e| {
                    anyhow::anyhow!(
...
fn grpo_pg_loss_from_logits_grad_kt(
    ...
    let grad_2d_neg_softmax = grad_2d.index_add(&active_idx, &rows, 0).map_err(|e| {
        anyhow::anyhow!("grpo_pg_loss_from_logits_grad_kt: scatter softmax rows: {e}")
    })?;
    ...
    let grad_2d = grad_2d_neg_softmax
        ...
        .map_err(|e| {
            anyhow::anyhow!("grpo_pg_loss_from_logits_grad_kt: scatter label coeffs: {e}")
        })?;
...
pub(crate) fn try_tape_grpo_pg_loss_from_normed_hidden_kt(
    ...
    anyhow::ensure!(
        !no_pg || echo_state.is_some(),
        "try_tape_grpo_pg_loss_from_normed_hidden_kt: no_policy_loss with no env-CE \
         rows in this completion — validation admits no_policy_loss only with ECHO \
         enabled, and a rollout with zero env rows has nothing to train on"
    );
    let (loss_kt_forward, echo_node, env_ce) = match echo_state {
        ...
        Some((node_state, env_ce_kt, env_ce_val)) => {
            let lambda = echo_env.map(|s| s.lambda).unwrap_or(0.0);
            let weighted = env_ce_kt.affine(lambda, 0.0).map_err(|e| {
                anyhow::anyhow!("try_tape_grpo_pg_loss_from_normed_hidden_kt: echo scale: {e}")
            })?;
            // §5.5 verifier-free mode: the scalar IS the env-CE term; the
            // PG value is masked out entirely.
            let composed = if no_pg {
                weighted
            } else {
                (&loss_kt_forward + &weighted).map_err(|e| {
                    anyhow::anyhow!(
...
mod tests {
    ...
    #[cfg(any(feature = "cuda", feature = "vulkan"))]
    #[test]
    fn grpo_no_policy_loss_trains_on_env_rows_alone() {
        let device = kiln_tensor::Device::Cpu;
        let (seq_len, hidden_size, vocab) = (6usize, 5usize, 13usize);
        let input_ids: Vec<u32> = vec![1, 5, 10, 3, 7, 2];
        let action_mask = vec![false, false, true, true, false, true];
        let env_mask = vec![false, true, false, false, true, false];
        let total_obs_len = 3usize;
        let lambda = 0.05f64;
        ...
        assert!(
            (loss_np_v - lambda * env_ce).abs() < 1e-6,
            "no_pg loss {loss_np_v} vs λ·env_ce {}",
            lambda * env_ce
        );
        assert!(
            (read(&loss_full)[0] as f64 - loss_np_v).abs() > 1e-9,
            "the PG term must actually be absent (full loss differs)"
        );
        ...
        for i in 0..g_np.len() {
            let env_only = g_e[i] - g_ne[i];
            assert!(
                (g_np[i] - env_only).abs() < 1e-5,
                "no_pg grad must be the env rows alone at {i}: {} vs {}",
                g_np[i],
                env_only
            );
        }
    ...
    fn grpo_normed_hidden_chunked_matches_full_logits_cpu() {
        ...
        for (name, params) in variants {
            ...
            assert_eq!(
                actual.len(),
                cached.len(),
                "{name}: cached grad length drift"
            );
            for (i, ((e, a), c)) in expected
                .iter()
                .zip(actual.iter())
                .zip(cached.iter())
                .enumerate()
    ...
    fn grpo_normed_hidden_sparse_backward_cuda_chunk_runs() {
        ...
        let normed_hidden =
            KtTensor::from_vec_on(device, hidden_host, vec![1, seq_len, hidden_size])
                .expect("cuda hidden");
        let head_t =
            KtTensor::from_vec_on(device, head_host, vec![hidden_size, vocab]).expect("cuda head");
        let logits = normed_hidden
            .squeeze(0)
            .unwrap()
            .matmul(&head_t)
