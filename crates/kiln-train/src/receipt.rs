...
pub struct EchoDiagnosticSummary {
    ...
    /// Env-token cross-entropy on a held-out trajectory set generated by
    /// a stronger teacher, sampled at the start of training. Paper §5.2
    /// uses this as the dynamics test — proves ECHO *learned* terminal
    /// structure rather than memorizing the training rollouts.
    /// Set by `pi-terminal-bench-lite/calibration/dynamics_holdout.py`
    /// (or similar) when the cap supplies a teacher-generated holdout.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub dynamics_holdout_ce_initial: Option<f64>,
    /// Env-token cross-entropy on the held-out trajectory set at the end
    /// of training. Paper §5.2: ECHO drops this sharply; GRPO-only barely
    /// moves it. The dynamics-holdout drop is the headline evidence that
    /// the model generalizes the terminal dynamics it learned.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub dynamics_holdout_ce_final: Option<f64>,
}
...
mod tests {
    ...
    fn echo_diagnostic_summary_skips_none_fields() {
        ...
        let summary = EchoDiagnosticSummary {
            ...
            dynamics_holdout_ce_initial: None,
            dynamics_holdout_ce_final: None,
        };
        let json = serde_json::to_string(&summary).unwrap();
        ...
        assert!(!json.contains("dynamics_holdout_ce_initial"));
        assert!(!json.contains("dynamics_holdout_ce_final"));
    ...
    #[test]
    fn echo_legacy_receipt_without_dynamics_holdout_fields_parses() {
        // Receipts written before dynamics_holdout_ce_{initial,final}
        // landed should still deserialize cleanly — the new fields are
        // Option<f64> with serde defaults.
        let json = r#"{
            "lambda": 0.05,
            "env_ce_initial": 4.21,
            "env_ce_final": 0.83,
        ...
        let parsed: EchoDiagnosticSummary = serde_json::from_str(json).unwrap();
        assert_eq!(parsed.dynamics_holdout_ce_initial, None);
        assert_eq!(parsed.dynamics_holdout_ce_final, None);
    }
}
