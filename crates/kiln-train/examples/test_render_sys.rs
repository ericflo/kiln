...
fn main() {
    ...
    let combined = vec![
        ChatMessage {
            ...
            content: "Few-shot exemplars.".to_string(),
            ..Default::default()
        },
        ChatMessage {
            role: "system".to_string(),
            content: "You emit JSON.".to_string(),
            ..Default::default()
        },
        ChatMessage {
            role: "user".to_string(),
            content: "Read x.rs".to_string(),
            ..Default::default()
        },
    ];
    let user_only = vec![
    ...
    let opts = ChatTemplateOptions {
        template_kwargs: kwargs.clone(),
    };

    println!("=== combined (extras + user_only) ===");
    let t2 = tok
        .apply_chat_template_full_with_options(combined.as_slice(), None, None, opts.clone())
        .unwrap();
    ...
    println!("=== user_only ===");
    let t3 = tok
        .apply_chat_template_full_with_options(user_only.as_slice(), None, None, opts)
        .unwrap();
    ...
    println!(
        "Is user_only a SUFFIX of combined? {}",
        &i2[i2.len() - i3.len()..] == i3.as_slice()
    );
}
