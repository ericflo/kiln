...
mod tests {
    ...
    // ----- Backward (Phase 10) -----
    //
    // The candle-typed `rmsnorm_backward_fallback` / `fused_rmsnorm_backward`
    // entries and their parity tests were deleted in (#1082). The kt-typed
    // backward path (`fused_rmsnorm_backward_kt`) is exercised by the
    // kt_v2 smoke test in `tests/kt_v2_smoke.rs` and via the kt_api unit
    // tests.
