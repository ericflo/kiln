...
/// Metal resident `a @ b^T` path used by matmul backward and chunked loss
/// gradients. This verifies the transposed-B tile loader against the CPU
...
fn matmul_rhs_transposed_matrix_core_parity() {
    ...
    let cases = [
        ...
        (vec![3usize], 17usize, 129usize, 31usize),
        (vec![2usize, 2usize], 9usize, 64usize, 13usize),
    ];
    for (bdims, m, k, n) in cases {
        let batch: usize = bdims.iter().product::<usize>().max(1);
        let a = pattern(batch * m * k, 700 + k as u64);
        let b = pattern(batch * n * k, 800 + n as u64);
        let mut a_shape = bdims.clone();
        a_shape.extend_from_slice(&[m, k]);
        let mut b_shape = bdims.clone();
        ...
        assert!(
            d < 0.02 * mref.max(1.0),
            "rhs-transposed matmul {bdims:?} [{m},{k}] x [{n},{k}]^T max|Δ|={d} (ref {mref})"
        );
    }
}
