...
use kiln_tensor::{
    DType, rocm_pool_stats as stats, rocm_synchronize_default_stream as sync,
    rocm_zeros_ctx as alloc,
};

fn mb(bytes: u64) -> f64 {
    bytes as f64 / (1024.0 * 1024.0)
}
fn mb_elems(m: usize) -> usize {
    (m * 1024 * 1024) / 2
} // bf16
...
fn pool_reuses_freed_blocks_without_growing_reserved() {
    ...
    let mut a = Vec::new();
    for _ in 0..8 {
        a.push(alloc(0, DType::BF16, mb_elems(256)).unwrap());
    }
    sync(0).ok();
    ...
    eprintln!(
        "[pool] reserved: start={:.0} afterAlloc={:.0} afterFree={:.0} MB",
        mb(r0),
        mb(r1),
        mb(r2)
    );
    eprintln!(
        "[pool] used:     start={:.0} afterAlloc={:.0} afterFree={:.0} MB",
        mb(u0),
        mb(u1),
        mb(u2)
    );
    ...
    assert!(
        u1 - u0 > mb_elems(8 * 256) as u64 / 2 || mb(u1) - mb(u0) > 1500.0,
        "used must rise ~2GB on alloc (start {:.0} -> {:.0})",
        mb(u0),
        mb(u1)
    );
    assert!(
        mb(u2) - mb(u0) < 300.0,
        "used must drop back ~baseline after free (got {:.0})",
        mb(u2) - mb(u0)
    );
    ...
    let mut b = Vec::new();
    for _ in 0..10 {
        b.push(alloc(0, DType::BF16, mb_elems(205)).unwrap());
    }
    sync(0).ok();
    let (r3, u3) = stats(0).unwrap();
    eprintln!(
        "[pool] after 10x205MB realloc: reserved={:.0} used={:.0} MB (reserved growth vs hwm {:.0})",
        mb(r3),
        mb(u3),
        mb(r3) - mb(r1.max(r2))
    );
