...
unsafe extern "C" {
    ...
    fn kiln_w8a8_gemv_gumbel_sample_bf16_async(
        x_q: *const core::ffi::c_void,
        w_q: *const core::ffi::c_void,
        x_scales: *const core::ffi::c_void,
        w_scales: *const core::ffi::c_void,
...
/// Decode-only fused W8A8 full-distribution Gumbel sample for one BF16
/// activation row and row-wise W8 weights.
#[allow(clippy::too_many_arguments)]
pub fn rocm_w8a8_gemv_gumbel_sample_bf16(
    x: &Tensor,
    w_q: &Tensor,
    scales: &Tensor,
    history_indices: &[u32],
    ...
    if history_indices.len() != history_counts.len() {
        return Err(Error::Msg(format!(
            "rocm_w8a8_gemv_gumbel_sample_bf16: history_indices len {} != history_counts len {}",
            history_indices.len(),
            history_counts.len()
        )));
    }
    if !(temperature.is_finite() && temperature > 0.0) {
        return Err(Error::Msg(format!(
            "rocm_w8a8_gemv_gumbel_sample_bf16: temperature must be finite > 0, got {temperature}"
        )));
    }
    if !(repetition_penalty.is_finite() && repetition_penalty > 0.0) {
        return Err(Error::Msg(format!(
            "rocm_w8a8_gemv_gumbel_sample_bf16: repetition_penalty must be finite > 0, got {repetition_penalty}"
        )));
    }
    if x.dtype() != DType::BF16 {
        return Err(Error::Msg(format!(
            "rocm_w8a8_gemv_gumbel_sample_bf16: x must be BF16, got {}",
            x.dtype()
        )));
    }
    if w_q.dtype() != DType::U8 {
        return Err(Error::Msg(format!(
            "rocm_w8a8_gemv_gumbel_sample_bf16: w_q must be U8, got {}",
            w_q.dtype()
        )));
    }
    if scales.dtype() != DType::F32 {
        return Err(Error::Msg(format!(
            "rocm_w8a8_gemv_gumbel_sample_bf16: scales must be F32, got {}",
            scales.dtype()
        )));
    }
    if !x.is_contiguous() || !w_q.is_contiguous() || !scales.is_contiguous() {
        return Err(Error::Msg(
            "rocm_w8a8_gemv_gumbel_sample_bf16: contiguous inputs required".to_string(),
        ));
    }
    let device = match x.device() {
        Device::Rocm(i) => Device::Rocm(i),
    ...
    if w_q.device() != device || scales.device() != device {
        return Err(Error::Msg(format!(
            "rocm_w8a8_gemv_gumbel_sample_bf16: device mismatch x={} w_q={} scales={}",
            x.device(),
            w_q.device(),
            scales.device()
        )));
    ...
    if x_shape.len() < 2 {
        return Err(Error::Msg(format!(
            "rocm_w8a8_gemv_gumbel_sample_bf16: x rank must be >=2, got {x_shape:?}"
        )));
    }
    let k = *x_shape.last().unwrap();
    let m: usize = x_shape[..x_shape.len() - 1].iter().product();
    if m != 1 {
        return Err(Error::Msg(format!(
            "rocm_w8a8_gemv_gumbel_sample_bf16: x must have one leading row, got {m}"
        )));
    }
    let w_shape = w_q.shape();
    if w_shape.len() != 2 {
        return Err(Error::Msg(format!(
            "rocm_w8a8_gemv_gumbel_sample_bf16: w_q must be [n, k], got {w_shape:?}"
        )));
    }
    let (n, wk) = (w_shape[0], w_shape[1]);
    if wk != k {
        return Err(Error::Msg(format!(
            "rocm_w8a8_gemv_gumbel_sample_bf16: x k={k} but w_q k={wk}"
        )));
    }
    if scales.shape() != [n] {
        return Err(Error::Msg(format!(
            "rocm_w8a8_gemv_gumbel_sample_bf16: scales shape {:?} != [{n}]",
            scales.shape()
        )));
    }
    ...
    let history_indices_t = if history_indices.is_empty() {
        ...
        Some(
            Tensor::from_vec(history_indices.to_vec(), (history_indices.len(),))
                .map_err(|e| {
                    Error::Msg(format!(
                        "rocm_w8a8_gemv_gumbel_sample_bf16: history indices tensor: {e}"
                    ))
                })?
                .to_device(device)
                .map_err(|e| {
                    Error::Msg(format!(
                        "rocm_w8a8_gemv_gumbel_sample_bf16: history indices upload: {e}"
                    ))
                })?,
        )
    };
    let history_counts_t = if history_counts.is_empty() {
        ...
        Some(
            Tensor::from_vec(history_counts.to_vec(), (history_counts.len(),))
                .map_err(|e| {
                    Error::Msg(format!(
                        "rocm_w8a8_gemv_gumbel_sample_bf16: history counts tensor: {e}"
                    ))
                })?
                .to_device(device)
                .map_err(|e| {
                    Error::Msg(format!(
                        "rocm_w8a8_gemv_gumbel_sample_bf16: history counts upload: {e}"
                    ))
                })?,
        )
    };
    ...
    let x_q_storage = RocmStorage::alloc_uninit_ctx(&ctx, device_index, DType::U8, k)?;
    let x_scales_storage = RocmStorage::alloc_uninit_ctx(&ctx, device_index, DType::F32, 1)?;
    let scores_storage = RocmStorage::alloc_uninit_ctx(&ctx, device_index, DType::F32, n)?;
    let out_storage = RocmStorage::alloc_uninit_ctx(&ctx, device_index, DType::I64, 1)?;
