//! Concrete `DeviceOp` implementations.
//!
//! Each submodule ships one logical op family and provides:
//!
//! - A struct implementing one of `DeviceOp1` / `DeviceOp2` / `DeviceOp3`.
//! - A `fn op_name(&Tensor, ...) -> Result<Tensor>` convenience that
//!   wraps `dispatch{1,2,3}` so call sites read like regular functions.
//! - A CPU forward (the canonical reference; backend-fallthrough hook).
//! - GPU forwards default to `Ok(None)` until the per-backend impl
//!   lands in Phase 2+.
//! - Parity tests in `#[cfg(test)]` that exercise the public API.
//!
//! Today: `embedding`. Phase 4 fills out the rest of the "glue" ops
//! (RMSNorm, residual add, final norm + LM head, sampling).

pub mod activation;
pub mod addmm;
pub mod argmax;
pub mod binary_minmax;
pub mod bincount;
pub mod bool_reduce;
pub mod broadcast;
pub mod cast;
pub mod categorical;
pub mod causal_conv1d_bwd;
pub mod chunk_split;
pub mod clamp_pow;
pub mod clip_grad_value;
pub mod compare;
pub mod concat;
pub mod cosine_sim;
pub mod cross_entropy;
pub mod cumprod;
pub mod cumsum;
pub mod diag;
pub mod dot;
pub mod dropout;
pub mod einsum;
pub mod elementwise;
pub mod embedding;
pub mod expand;
pub mod eye;
pub mod flip;
pub mod gather;
pub mod glu;
pub mod grad_clip;
pub mod gumbel_sample;
pub mod hyperbolic;
pub mod index_select;
pub mod init;
pub mod interpolate_1d;
pub mod l2norm;
pub mod layernorm;
pub mod leaky_activations;
pub mod lerp;
pub mod like;
pub mod linear;
pub mod log_softmax;
pub mod log_variants;
pub mod logit_dry;
pub mod logit_mirostat;
pub mod logit_misc;
pub mod logit_modern;
pub mod logit_penalties;
pub mod logit_processor;
pub mod logit_xtc;
pub mod losses;
pub mod mask;
pub mod masked_select;
pub mod matmul;
pub mod max_min_axis;
pub mod meshgrid;
pub mod mha;
pub mod nonzero;
pub mod normalize;
pub mod norms;
pub mod one_hot;
pub mod outer;
pub mod pad;
pub mod precision;
pub mod random;
pub mod range_ctors;
pub mod reduce;
pub mod repeat;
pub mod repeat_interleave;
pub mod rmsnorm;
pub mod roll;
pub mod rope;
pub mod rope_init;
pub mod rope_split_half;
pub mod scalar;
pub mod scatter_add;
pub mod sdpa;
pub mod searchsorted;
pub mod sign_and_round;
pub mod silu_mul;
pub mod softmax;
pub mod sort;
pub mod stack;
pub mod stats;
pub mod tensor_norm;
pub mod tile;
pub mod top_k;
pub mod trace;
pub mod triangular;
pub mod trig;
pub mod unary_arith;
pub mod unbind;
pub mod unique;
pub mod where_select;

pub use activation::{ActivationOp, UnaryKind, gelu, relu, sigmoid, silu, tanh};
pub use addmm::addmm;
pub use argmax::{ArgmaxLastDimOp, argmax_last_dim};
pub use binary_minmax::{maximum, minimum};
pub use bincount::bincount;
pub use bool_reduce::{BoolReduce, all_axis, all_reduce, any_axis, any_reduce};
pub use broadcast::broadcast_to;
pub use cast::{CastOp, cast};
pub use categorical::Multinomial;
pub use causal_conv1d_bwd::causal_depthwise_conv1d_bwd_input_composite;
pub use chunk_split::{ChunkOp, SplitWithSizesOp, chunk, split_with_sizes};
pub use clamp_pow::{clamp, pow};
pub use clip_grad_value::clip_grad_value;
pub use compare::{CmpKind, eq, ge, gt, le, lt, ne};
pub use concat::{ConcatOp, concat};
pub use cosine_sim::cosine_similarity;
pub use cross_entropy::{CrossEntropyOp, cross_entropy};
pub use cumprod::cumprod;
pub use cumsum::cumsum;
pub use diag::{diag, diagonal};
pub use dot::dot;
pub use dropout::dropout;
pub use einsum::einsum;
pub use elementwise::{BinaryKind, ElementwiseOp, add, div, mul, sub};
pub use embedding::{EmbeddingOp, embedding};
pub use expand::expand_dims;
pub use eye::{eye, eye_on_device};
pub use flip::flip;
pub use gather::gather;
pub use glu::{geglu, glu, reglu, swiglu};
pub use grad_clip::clip_grad_norm;
pub use gumbel_sample::{GumbelSampler, gumbel_softmax_sample};
pub use hyperbolic::{atanh, cosh, sinh};
pub use index_select::{IndexSelectOp, index_select};
pub use init::{kaiming_normal, kaiming_uniform, xavier_normal, xavier_uniform};
pub use interpolate_1d::{AlignCorners, interpolate_1d};
pub use l2norm::{L2NormOp, l2_norm};
pub use layernorm::{LayerNormOp, layer_norm};
pub use leaky_activations::{elu, leaky_relu, mish, softplus};
pub use lerp::lerp;
pub use like::{full_like, ones_like, zeros_like};
pub use linear::linear;
pub use log_softmax::{log_softmax_last_dim, log_softmax_last_dim_f32};
pub use log_variants::{exp2, expm1, log1p, log2, log10};
pub use losses::{
    bce_with_logits, hinge_loss, huber_loss, info_nce, kl_div_log_probs, l1_loss, margin_ranking,
    mse_loss, nll_loss,
};
pub use mask::{MaskedFillOp, causal_mask, masked_fill};
pub use masked_select::masked_select;
pub use matmul::{
    MatmulLhsTransposedOp, MatmulOp, MatmulRhsTransposedOp, matmul, matmul_lhs_transposed,
    matmul_rhs_transposed,
};
pub use max_min_axis::{MinMaxKind, max_axis, min_axis};
pub use meshgrid::meshgrid;
pub use mha::multi_head_attention;
pub use nonzero::nonzero;
pub use normalize::normalize;
pub use norms::{frobenius_norm, mean_squared, vector_norm};
pub use one_hot::one_hot;
pub use outer::outer;
pub use pad::pad;
pub use precision::{to_bf16, to_f16, to_f32};
pub use random::{rand_normal, rand_uniform};
pub use range_ctors::{arange, linspace};
pub use reduce::{ReduceOp, ReductionKind, ReductionScope, mean_all, mean_axis, sum_all, sum_axis};
pub use repeat::repeat;
pub use repeat_interleave::repeat_interleave;
pub use rmsnorm::{RmsNormOp, rms_norm};
pub use roll::roll;
pub use rope::{RopeOp, rope};
pub use rope_init::precompute_rope_freqs;
pub use rope_split_half::rope_split_half;
pub use scalar::{ScalarKind, ScalarOp, add_scalar, div_scalar, mul_scalar, sub_scalar};
pub use scatter_add::{ScatterAddOp, scatter_add};
pub use sdpa::{causal_scaled_dot_product_attention, scaled_dot_product_attention};
pub use searchsorted::searchsorted;
pub use sign_and_round::{ceil, floor, reciprocal, round, sign, trunc};
pub use silu_mul::{MulSigmoidGateOp, mul_sigmoid_gate};
pub use softmax::{SoftmaxLastDimOp, softmax_last_dim};
pub use sort::{argsort, sort};
pub use stack::stack;
pub use stats::{mean_variance, median, quantile, std_dev, variance};
pub use tensor_norm::{l1_norm, l2_norm_scalar, linf_norm, lp_norm};
pub use tile::tile;
pub use top_k::top_k;
pub use trace::trace;
pub use triangular::{tril, tril_mask, triu, triu_mask};
pub use trig::{TrigKind, acos, asin, atan, cos, sin, tan};
pub use unary_arith::{UnaryArithKind, abs, exp, ln, neg, sqrt};
pub use unbind::unbind;
pub use unique::unique;
pub use where_select::where_select;
