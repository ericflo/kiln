...
impl DeviceOp1 for L2NormOp {
    ...
    fn vulkan_fwd(&self, x: &Tensor) -> Result<Option<Tensor>> {
        ...
        //   - F32 only on Vulkan (the
        //     `vk_l2_norm_lastdim_no_grad` shader is F32-only; BF16/F16
        //     fall through to CPU until a cast wrapper or BF16/F16
        //     shader lands).
        //   - rank ≥ 1
        //   - contiguous input
        //   - hidden dim <= 256 (shader cap; see `check_l2norm_shape`
        //     in `vk_ops/l2norm.rs`)
        //   - storage must be Vulkan-backed (otherwise fall back to CPU
        //     gracefully rather than have the wrapper raise)
        //
        // Phase 4 substrate-op landing: dispatch through
        // `crate::vulkan_l2norm_last_axis` which wraps the production
        // `vk_l2_norm_lastdim_no_grad` shader. Pure L2-norm passes
        // `scale = 1.0` (the shader supports fused-scale for QK-norm
        // call sites). Current bridge D2H+H2D round-trips bytes through
        // the host; see `vulkan_storage.rs::vulkan_l2norm_last_axis`
        // rustdoc for the zero-copy follow-up plan.
        ...
        Ok(Some(crate::vulkan_l2norm_last_axis(x, self.eps)?))
    }
