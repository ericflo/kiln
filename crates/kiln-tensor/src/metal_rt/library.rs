//! Vendored from candle-metal-kernels 0.10.2 `src/metal/library.rs`
//! (MIT/Apache-2.0). objc2 wrapper over `MTLLibrary` / `MTLFunction` +
//! the function-constant-value machinery. Only crate-internal paths are
//! renamed (`MetalKernelError` -> `MetalRtError`). (#1082)
    ...
    pub fn get_function(
        ...
        let function = match constant_values {
            Some(constant_values) => self
                ...
                .map_err(|e| MetalRtError::LoadFunctionError(e.to_string()))?,
            None => self
                .raw
                .newFunctionWithName(&NSString::from_str(name))
                .ok_or(MetalRtError::LoadFunctionError(name.to_string()))?,
        };
...
impl std::fmt::Debug for FunctionConstantValues {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FunctionConstantValues")
            .field("raw", &"<MTLFunctionConstantValues>")
            .finish()
    }
}
