...
//! (MIT/Apache-2.0). The command-buffer pool: semaphore-coordinated
//! selection, batch-50 recycling, and the commit/wait lifecycle that is
//! the host-read synchronization point on Apple Silicon UMA. Ported
//! faithfully — only crate-internal paths are renamed
...
impl std::fmt::Debug for CommandBufferEntry {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CommandBufferEntry")
...
impl Commands {
    ...
    fn create_pool_entry(
        command_queue: &CommandQueue,
    ) -> Result<Arc<CommandBufferEntry>, MetalRtError> {
    ...
    fn select_entry(&self) -> Result<Arc<CommandBufferEntry>, MetalRtError> {
