//! Vendored from candle-metal-kernels 0.10.2 `src/metal/encoder.rs`
//! (MIT/Apache-2.0). objc2 wrappers over `MTLComputeCommandEncoder` and
//! `MTLBlitCommandEncoder`. The `Drop` impl ends encoding and releases the
...
impl std::fmt::Debug for ComputeCommandEncoder {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ComputeCommandEncoder")
            .field("raw", &"<MTLComputeCommandEncoder>")
...
impl std::fmt::Debug for BlitCommandEncoder {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BlitCommandEncoder")
            .field("raw", &"<MTLBlitCommandEncoder>")
            .field("semaphore", &self.semaphore)
            .finish()
    }
}
