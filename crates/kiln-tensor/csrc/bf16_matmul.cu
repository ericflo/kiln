...
//
// Computes batched C[M, N] = A[M, K] @ B[K, N] for row-major contiguous BF16
// operands, accumulating in FP32 and storing either BF16 or F32. This is
// intentionally small and shape-generic; production uses it only for
// explicitly admitted hipBLASLt gaps on gfx115x.
...
__global__ void kiln_bf16_matmul_f32_out_kernel(
    const __nv_bfloat16* __restrict__ a,
    const __nv_bfloat16* __restrict__ b,
...
extern "C" int kiln_rocm_bf16_matmul_bf16_out_async(
    ...
    cudaStream_t stream) {
    if (!a || !b || !c) return 1;
    if (m < 0 || n < 0 || k < 0 || batch < 0) return 2;
    if (m == 0 || n == 0 || k == 0 || batch == 0) return 0;
    if (m > INT32_MAX || n > INT32_MAX || k > INT32_MAX ||
        batch > 65535) return 3;
...
extern "C" int kiln_rocm_bf16_matmul_f32_out_async(
    const void* a,
    const void* b,
    void* c,
    int64_t m,
    ...
    kiln_bf16_matmul_f32_out_kernel<<<grid, block, 0, stream>>>(
        static_cast<const __nv_bfloat16*>(a),
        static_cast<const __nv_bfloat16*>(b),
        static_cast<float*>(c),
        static_cast<int>(m),
