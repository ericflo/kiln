...
| D-15 | desktop/README.md Apple secret list contradicts CI + signing.md (R161) | desktop/README.md:136-141 — documents the Tauri app-specific-password secret set (`APPLE_ID`, `APPLE_TEAM_ID`, `APPLE_PASSWORD`, plus `APPLE_CERTIFICATE`, `APPLE_SIGNING_IDENTITY`); `.github/workflows/desktop-build.yml` consumes the App Store Connect API key set (`APPLE_API_KEY_BASE64` :83; `APPLE_CERTIFICATE` :116; `APPLE_CERTIFICATE_PASSWORD` :117; `APPLE_SIGNING_IDENTITY` :118; `APPLE_API_ISSUER` :119; `APPLE_API_KEY` :120), matching `docs/desktop/signing.md` §3; `APPLE_ID`/`APPLE_TEAM_ID`/`APPLE_PASSWORD` have zero workflow references (all re-verified today) | (a) align the README list with the API-key flow (replace the 3 stale names with the 3 missing ones) · (b) document both flows explicitly · (c) keep | "align desktop/README.md's Apple secret list with desktop-build.yml + signing.md" |
| D-16 | `docs/desktop/about.png` generated but unreferenced (R161) | docs/desktop/about.png — generated by `scripts/capture-desktop-screenshots.mjs:43`; embedded nowhere (README.md:1673-1681 and desktop/README.md:54-62 embed only dashboard/settings/logs; no site page, no smoke expectation) (re-verified today) | (a) embed it in an about-window doc section · (b) drop the capture target + delete the PNG (net removal) | "embed about.png in the desktop docs" / "stop generating about.png" |
...
## 161. Retired env prefix in site benchmarks reproduce block; pin soundness audit; two desktop owner items (round 161)

Steering (4 candidates): (1) verify round-160 legacy-receipt validator pin
soundness; (2) one consistency pass between `docs/desktop/` and
desktop/README.md (owner-managed content report-only); (3) check stale
flags/env names in fenced code blocks of the largest live docs against
`kiln.example.toml` + config schema self-test; (4) fresh eyes for any unclosed
class completable in one bounded round.

**Executed (the single committed cleanup).** Removed the retired
`KILN_BENCH_LOG_TOKENS=1` env prefix from the "Reproduce the latest result"
command in `docs/public/BENCHMARKS.md` (the site's benchmarks page; net −1).
Commit: `1a3efd9dc`. Evidence:
- Retired deliberately: `99a4e5809` "Make benchmark controls explicit"
  (2026-07-18) removed the `KILN_BENCH_LOG_TOKENS` / `KILN_BENCH_LOG_ITL` env
  reads from `crates/kiln-server/src/bench.rs`; the runtime-env inventory
  classified the reads as experimental/debug ("runtime access outside the
  typed startup configuration boundary").
- `git grep KILN_BENCH_LOG_TOKENS` over tracked code: zero hits; remaining
  mentions are only frozen surfaces (docs/audits, docs/archive) and the fixed
  line.
- All 7 surviving flags in the block (`--model-path` :388, `--paged` :403,
  `--latency-only` :406, `--latency-warmup-runs` :409, `--prompt-tokens`
  :394, `--max-output-tokens` :391, `--seed` :412, `--quiet` :461) verified
  live in `crates/kiln-server/src/bench.rs` arg parsing.
- No script or smoke assertion references the env (the
  `scripts/check_docs_site_smoke.mjs` retired-asset list names the retired
  `docs/site/assets/desktop-*.png` files, a different set).
- Gates: `python3 scripts/check_repository_artifacts.py` pass (4564 tracked
  paths); `python3 scripts/check_production_file_budget.py` pass (646 files);
  `node scripts/docs-site/build.mjs --validate-only` → 59 documents, 0 copied
  assets; `KILN_DOCS_SMOKE_STATIC_ONLY=true node scripts/check_docs_site_smoke.mjs`
  rc=0.

**Verified (no action).**
- Round-160 pin soundness (candidate 1): checkpoint `9371035bf` exists ("Retain
  portable CUDA parity failure", 2026-07-27); `git diff --quiet 9371035bf --
  benchmarks/receipts/rocm/strix-halo qualification/oracle-results/rocm/strix-halo`
  clean (both legacy roots byte-identical to the checkpoint); disk split = 10
  current + 51 legacy receipts and 17 legacy oracle results (matches the gate
  report); `scripts/qualification/check_oracle_results.py` byte-identical to
  checkpoint (md5 `3f2b6591…`). Working-tree `scripts/bench-concurrent-batch.py`
  md5 `86fd085b…` ≠ checkpoint `009e4c05…` — expected: the gate runs the
  *checkpoint* driver from a detached worktree, while the working-tree file is
  the live v26/27 driver. `docs/policies/ci-policy.md` describes this pin
  accurately and never claims working-tree byte-identity. Sound; no change.
- Candidate-3 sweep: TOML key blocks across all live tracked `.md` validated
  against the schema `x-kiln-path` field set (120 canonical fields) — the only
  stale hits are the already-queued A-1 `batching.*` sites; `openenv.credentials.<id>.*`
  / `teachers.credentials.<id>.*` are the schema's legitimate dynamic template
  fields. KILN_* env sweep of fenced code blocks: only `KILN_BENCH_LOG_TOKENS`
  (fixed above) and the A-1 `KILN_W4A16` / `KILN_CUDA_GRAPHS` sites have zero
  code references; every other KILN_* name in live-doc code blocks resolves to
  live code (46–65 non-doc files) or a canonical schema env.
- Candidate-2 pass: all image/link paths between `docs/desktop/` and
  desktop/README.md resolve (dashboard/settings/logs embedded in both
  README.md:1673-1681 and desktop/README.md:54-62; the signing.md link
  resolves). Two content findings → owner queue below (D-15, D-16).

**Owner-queue additions (report-only; added to the DECISION table as D-15/D-16).**
- D-15: desktop/README.md:136-141 documents the Tauri app-specific-password
  Apple secret set (`APPLE_ID`, `APPLE_TEAM_ID`, `APPLE_PASSWORD`), while the
  workflow consumes the App Store Connect API key set (6 secrets matching
  `docs/desktop/signing.md` §3); the 3 stale names have zero workflow
  references. Owner-managed desktop doc — not edited this round.
- D-16: `docs/desktop/about.png` is generated by
  `scripts/capture-desktop-screenshots.mjs` but embedded in no doc or site
