#!/usr/bin/env bash
# run_iter.sh — full iter recipe for pi-faithful-completion.
#
...
OUT_ROOT="${OUT_ROOT:-/tmp/pi-faithful-completion-iter-${SLUG}}"
ROLLOUT_DIR="$OUT_ROOT/rollouts"
ADAPTER_NAME="${ADAPTER_NAME:-pi-faithful-completion-${SLUG}}"
...
echo "=== run_iter $SLUG (iter $ITER_NUM) for pi-faithful-completion ==="
