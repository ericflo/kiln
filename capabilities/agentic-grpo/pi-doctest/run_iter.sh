#!/usr/bin/env bash
# run_iter.sh — full iter recipe for pi-doctest.
#
...
OUT_ROOT="${OUT_ROOT:-/tmp/pi-doctest-iter-${SLUG}}"
ROLLOUT_DIR="$OUT_ROOT/rollouts"
ADAPTER_NAME="${ADAPTER_NAME:-pi-doctest-${SLUG}}"
...
echo "=== run_iter $SLUG (iter $ITER_NUM) for pi-doctest ==="
