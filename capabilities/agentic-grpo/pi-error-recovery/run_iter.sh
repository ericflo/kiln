#!/usr/bin/env bash
# run_iter.sh — full iter recipe for pi-error-recovery (round 2).
#
# Pipeline:
...
OUT_ROOT="${OUT_ROOT:-/tmp/pi-error-recovery-iter-${SLUG}}"
ROLLOUT_DIR="$OUT_ROOT/rollouts"
ADAPTER_NAME="${ADAPTER_NAME:-pi-error-recovery-${SLUG}}"
...
echo "=== run_iter $SLUG (iter $ITER_NUM) for pi-error-recovery ==="
