#!/usr/bin/env bash
# run_iter.sh — full iter recipe for pi-script-fixup.
#
...
OUT_ROOT="${OUT_ROOT:-/tmp/pi-script-fixup-iter-${SLUG}}"
ROLLOUT_DIR="$OUT_ROOT/rollouts"
ADAPTER_NAME="${ADAPTER_NAME:-pi-script-fixup-${SLUG}}"
...
echo "=== run_iter $SLUG (iter $ITER_NUM) for pi-script-fixup ==="
