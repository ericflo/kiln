#!/usr/bin/env bash
# run_iter.sh — full iter recipe for pi-tool-call-efficiency.
#
...
OUT_ROOT="${OUT_ROOT:-/tmp/pi-tool-call-efficiency-iter-${SLUG}}"
ROLLOUT_DIR="$OUT_ROOT/rollouts"
ADAPTER_NAME="${ADAPTER_NAME:-pi-tool-call-efficiency-${SLUG}}"
...
echo "=== run_iter $SLUG (iter $ITER_NUM) for pi-tool-call-efficiency ==="
