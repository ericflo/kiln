#!/usr/bin/env bash
# run_iter.sh — full iter recipe for pi-incremental-progress (round 2).
#
# Pipeline:
...
OUT_ROOT="${OUT_ROOT:-/tmp/pi-incremental-progress-iter-${SLUG}}"
ROLLOUT_DIR="$OUT_ROOT/rollouts"
ADAPTER_NAME="${ADAPTER_NAME:-pi-incremental-progress-${SLUG}}"
...
echo "=== run_iter $SLUG (iter $ITER_NUM) for pi-incremental-progress ==="
