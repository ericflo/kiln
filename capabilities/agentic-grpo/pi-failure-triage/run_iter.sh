#!/usr/bin/env bash
# run_iter.sh — full iter recipe for pi-failure-triage.
#
...
OUT_ROOT="${OUT_ROOT:-/tmp/pi-failure-triage-iter-${SLUG}}"
ROLLOUT_DIR="$OUT_ROOT/rollouts"
ADAPTER_NAME="${ADAPTER_NAME:-pi-failure-triage-${SLUG}}"
...
echo "=== run_iter $SLUG (iter $ITER_NUM) for pi-failure-triage ==="
