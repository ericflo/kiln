#!/usr/bin/env bash
# run_iter.sh — full iter recipe for pi-shell-hygiene.
#
...
OUT_ROOT="${OUT_ROOT:-/tmp/pi-shell-hygiene-iter-${SLUG}}"
ROLLOUT_DIR="$OUT_ROOT/rollouts"
ADAPTER_NAME="${ADAPTER_NAME:-pi-shell-hygiene-${SLUG}}"
...
echo "=== run_iter $SLUG (iter $ITER_NUM) for pi-shell-hygiene ==="
