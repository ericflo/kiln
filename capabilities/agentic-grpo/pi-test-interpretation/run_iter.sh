#!/usr/bin/env bash
# run_iter.sh — full iter recipe for pi-test-interpretation.
#
...
OUT_ROOT="${OUT_ROOT:-/tmp/pi-test-interpretation-iter-${SLUG}}"
ROLLOUT_DIR="$OUT_ROOT/rollouts"
ADAPTER_NAME="${ADAPTER_NAME:-pi-test-interpretation-${SLUG}}"
...
echo "=== run_iter $SLUG (iter $ITER_NUM) for pi-test-interpretation ==="
