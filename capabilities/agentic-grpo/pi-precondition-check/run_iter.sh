#!/usr/bin/env bash
# run_iter.sh — full iter recipe for pi-precondition-check.
#
...
OUT_ROOT="${OUT_ROOT:-/tmp/pi-precondition-check-iter-${SLUG}}"
ROLLOUT_DIR="$OUT_ROOT/rollouts"
ADAPTER_NAME="${ADAPTER_NAME:-pi-precondition-check-${SLUG}}"
...
echo "=== run_iter $SLUG (iter $ITER_NUM) for pi-precondition-check ==="
