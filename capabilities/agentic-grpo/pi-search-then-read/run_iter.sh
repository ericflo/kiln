#!/usr/bin/env bash
# run_iter.sh — full iter recipe for pi-search-then-read (round 2).
#
# Pipeline:
...
OUT_ROOT="${OUT_ROOT:-/tmp/pi-search-then-read-iter-${SLUG}}"
ROLLOUT_DIR="$OUT_ROOT/rollouts"
ADAPTER_NAME="${ADAPTER_NAME:-pi-search-then-read-${SLUG}}"
...
echo "=== run_iter $SLUG (iter $ITER_NUM) for pi-search-then-read ==="
