...
| **OpenEnv availability** | does the capability already exist as a stateful OpenEnv server with an enforceable action schema, observations, `done`, and reward? Inspect it with `kiln openenv inspect` or Training → OpenEnv in the dashboard. | Prefer the native environment loop over duplicating state transitions or reward logic in `rollout.py`. Inspection binds a canonical complete-discovery digest and compiles the self-contained schema; external references are not resolved, and every sampled action is validated before `step`. |
