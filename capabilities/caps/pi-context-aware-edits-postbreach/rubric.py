...
def _check_naming_case(edit: str, target_case: str) -> float:
    ...
    if "def " in edit:
        ...
        names = re.findall(r"\bfunction\s+(\w+)\s*\(", edit)
        names.extend(re.findall(r"\b(?:const|let|var)\s+(\w+)\s*=\s*(?:\([^)]*\)|\w+)\s*=>", edit))
    elif "func " in edit:
...
def _check_logging_style(edit: str, target: str) -> float:
    ...
    if target == "console_warn":
        return 1.0 if "console.warn" in edit else (0.0 if "console.log" in edit else 0.5)
    return 0.5
...
def _check_error_handling(edit: str, target: str) -> float:
    ...
    if target == "throw_error":
        return 1.0 if re.search(r"\bthrow\s+new\s+Error\b|\bthrow\s+\w+", edit) else 0.5
    return 0.5
...
def _check_comment_style(edit: str, target: str) -> float:
    ...
    if target == "jsdoc":
        return 1.0 if "/**" in edit and "*/" in edit else 0.0
    if target == "minimal":
