#!/usr/bin/env bash
# run_iter.sh — full OPD iter recipe for tool-call-arg-fidelity.
#
# Pipeline:
...
OUT_ROOT="${OUT_ROOT:-/tmp/tool-call-arg-fidelity-iter-${SLUG}}"
ADAPTER_NAME="${ADAPTER_NAME:-tool-call-arg-fidelity-${SLUG}}"
...
echo "=== OPD run_iter $SLUG (iter $ITER_NUM) for tool-call-arg-fidelity ==="
