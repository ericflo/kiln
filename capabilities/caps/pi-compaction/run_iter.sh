#!/usr/bin/env bash
# run_iter.sh — full iter recipe for pi-compaction.
#
...
OUT_ROOT="${OUT_ROOT:-/tmp/pi-compaction-iter-${SLUG}}"
ROLLOUT_DIR="$OUT_ROOT/rollouts"
ADAPTER_NAME="${ADAPTER_NAME:-pi-compaction-${SLUG}}"
...
echo "=== run_iter $SLUG (iter $ITER_NUM) for pi-compaction ==="
