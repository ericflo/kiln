#!/usr/bin/env bash
# capability.oracle.sh — blind eval driver for pi-search-then-read (round 2).
# Wraps `kiln eval-adapter` (#33).
set -euo pipefail
cd "$(dirname "$0")"
...
OUT_FILE="${OUT_FILE:-/tmp/pi-search-then-read-eval-${ADAPTER:-base}.json}"
