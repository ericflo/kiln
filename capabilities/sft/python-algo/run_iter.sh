#!/usr/bin/env bash
# run_iter.sh — full SFT iter recipe for python-algo.
set -euo pipefail
cd "$(dirname "$0")"

SLUG="${1:-h1-default}"
ITER_NUM="${ITER_NUM:-1}"
ADAPTER_NAME="${ADAPTER_NAME:-python-algo-${SLUG}}"
OUT_ROOT="${OUT_ROOT:-/tmp/python-algo-iter-${SLUG}}"
...
echo "=== SFT run_iter $SLUG (iter $ITER_NUM) for python-algo ==="
