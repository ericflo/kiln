...
const expectedTroubleshootingSections = [
  ...
  { label: 'ROCm false-drain soak rejection', terms: ['development-soak attempt rejected a false-drained snapshot', 'all 29 warmup responses were valid', 'all 39 entries', 'terminal model cleanup still retained one ROCm graph owner', 'resource-ownership boundary', 'accelerator telemetry correctly remain zero', 'Require conservative terminal cleanup publication'] },
