...
const expectedCliSections = [
  ...
  { label: 'native and HF/TRL training path', terms: ['run native jobs or export a verified hf/trl handoff', 'kiln train sft', 'kiln train grpo', 'kiln train opd', 'kiln train hf export-sft'] },
  ...
  { label: 'verified HF/TRL handoff', terms: ['verified hf/trl sft handoff', '--file is read by the server process', 'exactly one of --file or --dataset is required', 'refuses redirects and existing outputs', 'duplicate paths', 'trailing gzip data', '{name}.kiln-hf', '--keep-server-copy', 'identity-conditional if-match', '--export-sha256', 'omit that flag only for deliberate operator cleanup'] },
...
const expectedCliCodeExamples = [
  ...
  { label: 'HF/TRL export commands', terms: ['kiln train hf export-sft', '--file /data/corrections.jsonl', '--dataset corrections:active', '--name support-hf-01', 'kiln train hf list --json', 'kiln train hf delete', '--name retained-export', '--export-sha256'] },
