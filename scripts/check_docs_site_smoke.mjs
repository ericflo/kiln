...
const generatedDocsPages = [
  ...
  },
  {
    ...
    terms: [
      ...
      '115 operations',
      'complete',
      'DELETE 12, GET 54, POST 48, PUT 1',
  ...
  },
  {
    ...
    terms: [
      ...
      'Historical serving benchmark driver v9 retained the v8 provenance containment',
      ...
      'now retired and deleted',
      'current source always constructs the actor for a real backend',
...
const expectedArchitectureSections = [
  ...
  { label: 'portable Vulkan policy', terms: ['Vulkan kernel selection is an immutable implementation contract', 'kiln.vulkan-kernel-policy.v4', 'VulkanKernelPolicy::portable_fallback()', 'device-name', 'Machine qualification data does not alter product dispatch', 'Device-tuned GDN', 'Rows-four/rows-eight selection', '20,000,000,000', '2048-row tile', '10,000,000-element row-work budget', '128-row tiles', '1,048,576 elements per dispatch', 'Kernel-stage and resident-decode profiling are disabled', 'historical binary', 'cannot qualify v4'] },
...
const expectedTroubleshootingSections = [
  ...
  { label: 'ROCm streaming-prefill pause/OOM diagnosis', terms: ['rocm long prefill pauses or runs out of memory', 'prefill_runtime.streaming_prefill', 'base/tape tiles of 256', 'server.max_prefill_tokens_per_cycle=256', 'server.max_batch_tokens=512', 'startup rejects a mismatched actor-prefill contract before loading model weights', 'detached/boundary/replay tiles of 8192', 'mode="disabled"', 'startup rejects either before loading weights', 'output throughput fell from 13.194 to 7.108 tokens/second', 'Actor-prefill slow-phase events increased from 612 to 3,548', 'same-binary A/B before any final ROCm endurance promotion', 'multiples of 64', 'do not broadly raise the timeout', 'hosted gpu ci is not hardware evidence'] },
