...
const generatedDocsPages = [
  ...
  },
  {
    ...
    terms: [
      ...
      '15 top-level sections and 114 fixed leaf fields',
...
const expectedArchitectureSections = [
  ...
  { label: 'prefix-cache route invariance', terms: ['prefix-cache route invariance', 'cache admission changes storage and reuse, not numerical prefill geometry', 'block-aligned final prompt boundary', 'no lookup', 'recurrent-state snapshot', 'prompt registration', 'rolling snapshot', 'Every resumable prompt-chunk boundary', 'first token', 'following decode token', 'Accelerator qualification remains required'] },
...
const expectedTroubleshootingSections = [
  ...
  { label: 'ROCm multi-row graph-route diagnosis', terms: ['Current ROCm capture supports contiguous BF16 multi-row decode', 'multi_row_batch_unsupported', 'active shared width slot', 'not proof of VRAM rebalancing', 'graph-performance qualification must still fail'] },
