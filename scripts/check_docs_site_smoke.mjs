...
const expectedApiSections = [
  ...
  { label: 'ROCm multi-row graph-route accounting', terms: ['all 14 reason counters', 'multi_row_batch_unsupported', 'Native ROCm capture is currently single-row only', 'without claiming capture or replay'] },
...
const expectedTroubleshootingSections = [
  ...
  { label: 'ROCm multi-row graph-route diagnosis', terms: ['Native ROCm capture currently supports only single-row decode', 'multi_row_batch_unsupported', 'once per completed forward', 'not proof of VRAM rebalancing', 'graph-performance qualification must still fail'] },
