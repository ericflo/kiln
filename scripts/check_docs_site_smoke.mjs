...
const expectedCliSections = [
  ...
  { label: 'native and HF/TRL training path', terms: ['run native jobs or move a verified hf/trl handoff', 'kiln train sft', 'kiln train grpo', 'kiln train opd', 'kiln train hf export-sft', 'kiln train hf import-peft'] },
  ...
  { label: 'verified HF/TRL handoff', terms: ['verified hf/trl sft handoff', '--file is read by the server process', 'exactly one of --file or --dataset is required', 'refuses redirects and existing outputs', 'duplicate paths', 'trailing gzip data', '{name}.kiln-hf', '--keep-server-copy', 'identity-conditional if-match', '--export-sha256', 'omit that flag only for deliberate operator cleanup', 'completed extracted .kiln-hf directory', 'fails local verification before connecting', 'deterministic ten-file envelope', 'locally predicted receipt', 'never changes or removes the source bundle', 'rejected without replacement'] },
