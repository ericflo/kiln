...
def _rust_read_sites(
    ...
    *,
    ...
    for match in read_pattern.finditer(masked):
        opening = masked.find("(", match.start())
        expression = _balanced_call(body, masked, opening)
        contextual_targets = sorted(
            {
                target
                for target in _quoted_strings(body)
                if _implementation_target(target)
            }
        )
        expression_literals = _quoted_strings(expression)
        expression_identifiers = set(
            re.findall(r"\b[A-Za-z_][A-Za-z0-9_]*\b", expression)
        )
        reads_dynamic_source_path = bool(
            expression_identifiers
            & {"path", "relative_path", "source_path", "source_file"}
        )
        helper_reads_workspace_path = (
            not expression_literals and "workspace_root" in expression.casefold()
        )
        helper_walks_rust_sources = (
            function_name is not None
            and (
                "source" in function_name.casefold()
                or function_name == "parse_functions"
                or (
                    "read_dir" in body
                    and "extension" in body
                    and any(value == "rs" for value in _quoted_strings(body))
                )
            )
        )
        if (
            _source_expression(expression)
            or (contextual_targets and reads_dynamic_source_path)
            or helper_reads_workspace_path
            or helper_walks_rust_sources
        ):
            target = ", ".join(contextual_targets) or _compact(expression)
            read_sites.append({"api": "read_to_string", "target": target})
...
def render_report(inventory: dict[str, object]) -> str:
    ...
    if owners:
        lines.extend(
            [
                "Owners are ordered by the number of migration obligations.",
                "",
                "| Test owner | Tests |",
                "| --- | ---: |",
            ]
        ...
        lines.append("None. No test currently enters the source-parsing migration queue.")
    lines.extend(
        [
            "",
            "## Migration queue",
    ...
    if entries:
        lines.extend(
            [
                "Each row is a replacement obligation, not accepted evidence.",
                "",
        ...
        lines.append("Empty. Any future row is technical debt, not correctness evidence.")
    lines.extend(
        [
            "",
            "## Gate",
