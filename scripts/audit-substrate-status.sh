...
ROWS = [
    ...
    ("1.44", "docs/guides/SUBSTRATE_QUICKSTART.md — contributor entry point",
                                    ["docs/guides/SUBSTRATE_QUICKSTART.md"]),
    ...
    ("1.46", "tied-weight integration test (anti-pattern 17)",
                                    ["crates/kiln-optim/tests/tied_weights.rs"]),
    ("1.47", "kiln-param ReplayBuffer (off-policy RL data plumbing)",
                                    ["crates/kiln-param/src/replay_buffer.rs"]),
    ("1.48", "LogitProcessor chain skeleton (temp + top-K + top-P)",
                                    ["crates/kiln-tensor/src/ops/logit_processor.rs"]),
    ("1.49", "penalty LogitProcessors (rep / freq / presence)",
                                    ["crates/kiln-tensor/src/ops/logit_penalties.rs"]),
    ("1.50", "sampler chain integration test",
                                    ["crates/kiln-tensor/tests/sampler_chain.rs"]),
