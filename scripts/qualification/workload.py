...
RUNNER_RESULT_METRICS = {"case_pass", "exit_code", "output_assertion_failures"}
COMPARISON_MODES = {
    "same_environment_performance",
    "declared_ab_variants",
    "cross_backend_correctness",
}
METRIC_SCOPES = {"receipt", "result"}
...
VARIANT_KEYS = {"id", "description", "backend", "device_requirement", "skip_policy", "cases"}
CASE_KEYS = {
    "id",
    "description",
    "required",
...
VARIANT_PAIR_KEYS = {"baseline_variant_id", "candidate_variant_id"}
BACKEND_PAIR_KEYS = {"backend_a", "variant_a_id", "backend_b", "variant_b_id"}
METRIC_RULE_KEYS = {
    "scope",
    "result_id",
    "metric",
...
def _validate_variable(errors: list[str], value: Any, context: str) -> tuple[str, bool | None]:
    ...
    if name == "seed":
        errors.append(f"{context}.name 'seed' is reserved by determinism.seed")
    ...
    if not isinstance(allowed, list):
        ...
        keys = [_scalar_key(item) for item in allowed]
...
def _placeholder(
    ...
    if match:
        name = match.group(1)
        if name != "seed" and name not in declared_variables:
...
) -> tuple[str, bool | None, set[str], set[str], set[str], set[str]]:
...
def _validate_comparison(
    ...
    *,
    ...
    backend_pairs: list[tuple[tuple[str, str], tuple[str, str]]] = []
    if not isinstance(raw_backend_pairs, list):
        ...
        for index, raw_pair in enumerate(raw_backend_pairs):
            ...
            backend_pairs.append(pair_key)

    raw_config_paths = comparison.get("allowed_effective_config_differences")
    config_paths: list[str] = []
    if not isinstance(raw_config_paths, list):
        errors.append(f"{context}.allowed_effective_config_differences must be an array")
    else:
        for index, path in enumerate(raw_config_paths):
            path_text = _check_string(
                errors, path, f"{context}.allowed_effective_config_differences[{index}]"
            )
            if path_text and not CONFIG_PATH_RE.fullmatch(path_text):
                errors.append(
                    f"{context}.allowed_effective_config_differences[{index}] has invalid dot-path syntax"
                )
            config_paths.append(path_text)
        if len(config_paths) != len(set(config_paths)):
            errors.append(f"{context}.allowed_effective_config_differences contains duplicates")
        if config_paths != sorted(config_paths):
            errors.append(
                f"{context}.allowed_effective_config_differences must use ascending order"
            )
    ...
    for index, raw_rule in enumerate(rules):
        ...
        if scope == "receipt" and result_id is not None:
            errors.append(f"{rule_context}.result_id must be null for receipt scope")
        elif scope == "result":
    ...
    if mode == "same_environment_performance":
        ...
        if variant_pairs or backend_pairs or config_paths:
            errors.append(
                f"{context}.mode same_environment_performance cannot declare pairs or config differences"
            )
        ...
        if not config_paths:
            errors.append(
                f"{context}.mode declared_ab_variants requires exact allowed effective-config differences"
            )
        if not validated_rules:
        ...
        if config_paths:
            errors.append(
                f"{context}.mode cross_backend_correctness cannot allow effective-config differences"
            )
        if not validated_rules:
    ...
    if mode == "declared_ab_variants":
        comparison_variant_ids = {item for pair in variant_pairs for item in pair}
    elif mode == "cross_backend_correctness":
        comparison_variant_ids = {endpoint[1] for pair in backend_pairs for endpoint in pair}
    else:
...
def validate_workload(workload: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(variants, list) or not variants:
        ...
        for variant_index, raw_variant in enumerate(variants):
            ...
            non_seed_references = references - {"seed"}
            missing_required = sorted(required_variables - non_seed_references)
            if missing_required:
                errors.append(
                    f"{variant_context} does not consume required variables: {', '.join(missing_required)}"
                )
            all_references.update(non_seed_references)
