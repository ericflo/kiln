...
async function startServer({
  ...
  const server = http.createServer(async (req, res) => {
      ...
      json(res, {
        ...
        vram: {
          ...
          governor: {
            ...
            reclaim_mode_effective: 'automatic',
        ...
        accelerator_runtime: {
          ...
          rocm_graph_mode: { configured: 'profile', effective: 'lazy_capture_replay', source: 'default' },
    ...
    if (url.pathname === '/v1/openenv/runs' && req.method === 'GET') {
      json(res, { schema: 'kiln.openenv-run-list.v5', runs: openEnvRuns });
      return;
    }
    ...
    if (url.pathname === '/v1/openenv/runs' && req.method === 'POST') {
      ...
      const status = {
        ...
        training_contract: {
          ...
          effective_config: {
            optimizer: request.training_config?.optimizer || { kind: 'muon' },
            lora_rank: request.training_config?.lora_rank || 8,
            base_adapter: null,
            output_name: request.output_adapter,
            auto_load: false,
