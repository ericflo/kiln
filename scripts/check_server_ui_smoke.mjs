...
async function expectRocmMatmulRuntimeConfig(page, scenarioLabel) {
  ...
  const selector = '#runtime-config-body';
  await waitForPanelText(page, selector, /Accelerator execution[\s\S]*Policy schema[\s\S]*kiln\.accelerator-runtime-policy\.v10[\s\S]*v10/, `${prefix} should render policy schema v10`);
