...
| `training_data` | Source kind, optional path, source-specific content digest, and optional validated `kiln.openenv-training-data.v1` corpus identity. | A path alone does not bind mutable bytes. OpenEnv's group-plan digest describes endpoint/reset/seed semantics, its behavior-policy identity binds the exact policy revision, and the ordinary digest still binds every input byte. |
