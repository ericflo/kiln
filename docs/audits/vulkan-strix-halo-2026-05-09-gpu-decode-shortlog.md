...
| A078 | Prefer cached Vulkan host-visible staging memory. | Changed Vulkan device init to prefer `HOST_VISIBLE | HOST_COHERENT | HOST_CACHED` staging memory, falling back to the previous non-cached host-visible type. On Strix Halo this selects memory type `5` instead of the previous first matching uncached type. Same-seed no-profile A/B kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]` and improved prefill `2060.805ms -> 1667.561ms`, mean ITL `97.134ms -> 86.451ms`, and p99 ITL `99.784ms -> 89.955ms`. Profile confirmed the mechanism: A077 `batch=64 read_host_visible` `246.909ms` dropped to A078 `9.733ms`, and GDN `in_proj seq_len=64` `364.616ms -> 121.046ms`. Server smoke selected Vulkan, logged `cached=true`, and returned direct chat `content == "blue green"` with `chat_template_kwargs: {"enable_thinking": false}`. | Keep. This is a Vulkan-only staging-memory win; CUDA/Metal source paths are untouched and local CUDA/Metal feature checks remain host-blocked. Next GDN in-proj work should target `record_submit_wait` or adjacent-stage residency/fusion, not readback memory type. |
| A079 | Enable packed-BF16 Vulkan MLP weights for large prefill batches without row-pair BF16. | Removed the backend `row_count < 8` gate for packed-BF16 MLP weights, while keeping BF16 on the existing non-row-pair batched shaders (`rows2=false`) instead of reviving A029's rejected BF16 row-pair shader. Added batch-9 BF16 MLP parity coverage. Cross-build seed-79 A/B kept token IDs and improved prefill `1641.175ms -> 1451.345ms`; same-binary rollback-env seed-80 A/B also kept token IDs and improved prefill `1630.605ms -> 1570.237ms`, mean ITL `102.947ms -> 89.671ms`, and p99 `104.134ms -> 96.883ms`. Profile showed MLP `batch=64` inner total `812.985ms -> 678.129ms`: `gate_up_dispatch` `664.670ms -> 426.340ms`, with down dispatch slower (`130.466ms -> 234.627ms`) but net positive. Server smoke selected Vulkan and returned direct chat `content == "blue green"` with `chat_template_kwargs: {"enable_thinking": false}`. | Keep. This is a Vulkan-only prefill win using existing BF16 kernels; CUDA/Metal source paths are untouched. Do not reinterpret this as approval for A029's BF16 row-pair shader; that remains rejected. |
| A080 | Hybridize large-batch Vulkan MLP as BF16 gate/up plus F32 row-pair down. | Added a guarded MLP dispatcher that uses packed-BF16 gate/up weights but keeps the down projection on the cached F32 row-pair shader for `row_count >= 8`, with rollback via `KILN_DISABLE_VULKAN_MLP_BF16_GATE_UP_F32_DOWN=1`. Same-binary seed-81 A/B kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]` and improved prefill `1470.047ms -> 1384.763ms`, mean ITL `87.129ms -> 86.617ms`, and p99 `87.891ms -> 87.867ms`. Profile confirmed the intended route (`bf16_weights=true down_bf16_weights=false rows2=false down_rows2=true`) and improved A079 MLP `batch=64` inner total `678.129ms -> 558.304ms`, mostly by restoring down dispatch `234.627ms -> 127.721ms`. Server smoke selected Vulkan and returned direct chat `content == "blue green"` with `chat_template_kwargs: {"enable_thinking": false}`. | Keep. This is a Vulkan-only prefill win that preserves A079's BF16 gate/up gain while avoiding its BF16 down-projection regression. CUDA/Metal source paths are untouched; local CUDA/Metal feature checks remain host-blocked. |
| A081 | Add packed-BF16 Vulkan MLP gate/up rows4 shader for the A080 hybrid path. | Added `mlp_gate_up_decode_batched_rows4_bf16w.comp`, which computes four adjacent rows per workgroup and reuses each packed-BF16 gate/up weight load across those rows. It is only used for the large-batch hybrid path (`bf16_weights=true down_bf16_weights=false row_count>=8`), with rollback via `KILN_DISABLE_VULKAN_MLP_BF16_GATE_UP_ROWS4=1`. Same-binary seed-82 A/B kept token IDs and improved prefill `1379.325ms -> 1118.678ms`; seed-83 repeated the win `1372.707ms -> 1128.404ms` and improved mean ITL `87.919ms -> 85.485ms`. Profile confirmed `gate_up_rows4=true` and improved A080 MLP `batch=64` inner total `558.304ms -> 279.313ms`, with gate/up dispatch `413.934ms -> 132.253ms`. Server smoke selected Vulkan and returned direct chat `content == "blue green"` with `chat_template_kwargs: {"enable_thinking": false}`. | Keep. This is a Vulkan-only prefill win inspired by Metal row-quad gate/up work, but implemented as a Strix Halo-specific BF16 rows4 shader on the existing hybrid path. CUDA/Metal source paths are untouched; local CUDA/Metal feature checks remain host-blocked. |
| A082 | Add F32 Vulkan MLP down rows4 shader for the A080/A081 hybrid path. | Added `linear_decode_batched_rows4.comp`, selected only for the large-batch hybrid path (`bf16_weights=true down_bf16_weights=false row_count>=8`), with rollback via `KILN_DISABLE_VULKAN_MLP_F32_DOWN_ROWS4=1`. Same-binary seed-84 A/B stayed on backend `vulkan`, kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, and measured a small no-profile win: prefill `1099.699ms -> 1097.205ms`, mean ITL `87.059ms -> 86.810ms`, p99 `93.162ms -> 89.402ms`. Profile confirmed `down_rows4=true down_rows2=false` and improved A081 MLP `batch=64` inner total `279.313ms -> 232.591ms`, with down dispatch `128.874ms -> 85.732ms`. Server smoke returned direct chat `content == "blue green"` with `chat_template_kwargs: {"enable_thinking": false}`. | Keep. This is a tightly scoped Vulkan-only prefill win; CUDA/Metal source paths are untouched and local CUDA/Metal feature checks remain host-blocked. |
| A083 | Void misapplied Vulkan conv1d fused-state trial. | Temporarily added a `causal_conv1d_prefill_k4` shader intended to test the Metal-style one-workgroup-per-channel prefill shape, with rollback via `KILN_DISABLE_VULKAN_CONV1D_PREFILL_FUSED_STATE=1`. Follow-up inspection found the dispatcher branch was accidentally inserted into `dispatch_causal_conv1d_update`, not `dispatch_causal_conv1d_prefill`, so the same-binary seed-85 A/B is not valid evidence about the GDN conv prefill bucket. The temporary run stayed on backend `vulkan` and kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, but its timing should not be used to accept or reject the intended prefill optimization. | Voided and removed. Final source has no A083 code and CUDA/Metal paths are untouched; a correctly wired prefill experiment is still needed before judging this idea. |
| A084 | Reject correctly wired Metal-style Vulkan conv1d prefill fused-state shader. | Temporarily wired `causal_conv1d_prefill_k4` into `dispatch_causal_conv1d_prefill`, one workgroup per `(batch, channel)` stream with state advance after a workgroup barrier, rollback via `KILN_DISABLE_VULKAN_CONV1D_PREFILL_FUSED_STATE=1`. Temporary source passed fmt, Vulkan compile checks, focused conv prefill parity, and release bench build. Same-binary seed-86 A/B stayed on backend `vulkan` and kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, but regressed prefill `1050.457ms -> 1090.345ms`, mean ITL `86.436ms -> 87.868ms`, and p99 `89.999ms -> 91.913ms`. | Rejected and removed. The Metal one-workgroup-per-channel conv prefill shape does not transfer as a default Vulkan win on Strix Halo; final source has no A084 code and CUDA/Metal paths are untouched. |
| A085 | Reject Metal-inspired Vulkan MLP F32 down rows4 tile8 shader. | Temporarily added `linear_decode_batched_rows4_tile8.comp` and wired it only into the large-batch hybrid MLP down path before A082's accepted rows4 shader, with rollback via `KILN_DISABLE_VULKAN_MLP_F32_DOWN_ROWS4_TILE8=1`. Temporary source passed fmt, Vulkan compile checks, focused BF16-gate/up F32-down MLP parity, and release bench build. Same-binary seed-87 A/B stayed on backend `vulkan` and kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, but regressed prefill `1057.323ms -> 1393.836ms`. Mean ITL was effectively flat/noisy at `85.741ms -> 85.508ms`, p99 `87.497ms -> 87.012ms`. | Rejected and removed. Direct Metal rowquad tile8 down-projection mirroring is not a Vulkan F32 MLP down win on Strix Halo; final source has no A085 code and CUDA/Metal paths are untouched. |
| A086 | Reject Vulkan packed-BF16 GDN input-projection rows8 shader. | Temporarily added `gdn_in_proj_decode_batched_pair_qkv_z_rows8_bf16w.comp`, selected only for packed-BF16 paired-QKV/Z GDN input projection at `batch >= 8`, with rollback via `KILN_DISABLE_VULKAN_GDN_IN_PROJ_BATCH_ROW_OCT=1`. Temporary source passed fmt, Vulkan compile checks, focused GDN in-proj parity, and release bench build. Same-binary seed-88 A/B stayed on backend `vulkan` and kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, but regressed prefill `1066.390ms -> 1142.299ms`, mean ITL `84.926ms -> 87.498ms`, and p99 `86.301ms -> 93.231ms`. | Rejected and removed. Larger GDN in-proj row grouping increases pressure enough to lose against rows4 on Strix Halo; final source has no A086 code and CUDA/Metal paths are untouched. |
| A087 | Reject Vulkan single-row BF16 linear `32x8` shader. | Temporarily added `linear_decode_bf16w_x32y8.comp` and selected it for single-row packed-BF16 generic linear decode and MLP down projection, with rollback via `KILN_DISABLE_VULKAN_LINEAR_BF16W_X32Y8=1`. Temporary source passed fmt, Vulkan compile checks, focused generic linear BF16 parity, focused MLP BF16 parity, and release bench build. Same-binary seed-89 A/B stayed on backend `vulkan` and kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, but regressed prefill `1065.126ms -> 1086.950ms`, mean ITL `87.013ms -> 94.090ms`, and p99 `94.390ms -> 94.935ms`. | Rejected and removed. The direct `16x16` to `32x8` serial BF16 linear trade is slower on Strix Halo; final source has no A087 code and CUDA/Metal paths are untouched. |
| A088 | Reject broad Vulkan conv1d batched transfers. | Temporarily batched x/weight/state uploads and out/state readbacks in both `dispatch_causal_conv1d_update` and `dispatch_causal_conv1d_prefill`, with rollback via `KILN_DISABLE_VULKAN_CONV1D_BATCHED_TRANSFERS=1`. Temporary source passed fmt, Vulkan compile checks, focused conv update/prefill parity, and release bench build. Same-binary seed-90 A/B stayed on backend `vulkan` and kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`. Candidate improved prefill `1077.433ms -> 1063.865ms`, but regressed mean ITL `85.479ms -> 88.239ms` and p99 `86.885ms -> 93.994ms`. | Rejected and removed. Do not batch the single-token conv update transfer path by default; a prefill-only transfer batching follow-up remains worth testing. Final source has no A088 code and CUDA/Metal paths are untouched. |
| A089 | Reject Vulkan conv1d prefill-only batched transfers. | Temporarily batched x/weight/state uploads and out/state readbacks only in `dispatch_causal_conv1d_prefill`, leaving the single-token update path unchanged, with rollback via `KILN_DISABLE_VULKAN_CONV1D_PREFILL_BATCHED_TRANSFERS=1`. Temporary source passed fmt, Vulkan compile checks, focused conv update/prefill parity, and release bench build. Same-binary seed-91 A/B stayed on backend `vulkan` and kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, but regressed prefill `1089.390ms -> 1112.361ms`, mean ITL `85.817ms -> 88.812ms`, and p99 `87.059ms -> 90.721ms`. | Rejected and removed. Do not retry conv1d transfer batching by only grouping copy submissions; final source has no A089 code and CUDA/Metal paths are untouched. |
| A090 | Refresh current Vulkan profile after A089 and before post-PR1008 follow-up. | Release profile stayed on backend `vulkan` with token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, measured `1110.497ms` prefill, `89.517ms` mean ITL, and `91.070ms` p99. Measured-pass prefill buckets were GDN recurrent `253.580ms`, MLP fused `231.666ms`, GDN conv `175.091ms`, GDN in-proj `123.833ms`, and full-attn QKV `97.001ms`; decode buckets were MLP fused `241.639ms`, GDN recurrent `109.122ms`, and GDN in-proj `101.946ms`. | Keep as target-selection evidence. Next work should target GDN recurrent/conv or MLP/GDN boundary/fusion/residency; avoid repeating direct conv transfer batching or simple BF16/row-group retile attempts already rejected. |
| A091 | Re-test opt-in Vulkan `gdn_full_chunk_forward` on current main after cached staging changes. | Current-main default seed-92 run stayed on backend `vulkan`, kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, and measured `1108.174ms` prefill / `85.617ms` mean ITL. `KILN_ENABLE_VULKAN_GDN_FULL_CHUNK_FORWARD=1` kept the same token IDs and measured `1160.651ms` prefill / `85.418ms` mean ITL. | Rejected. Keep the current full-chunk shader opt-in only; A078's cached staging win does not make this fused chunk shape a default prefill win. |
| A092 | Add profile-only GDN recurrent inner-stage logging. | New `KILN_PROFILE_GDN_RECURRENT_INNER_STAGES=1` profile stayed on backend `vulkan` with token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`. Measured-pass prefill recurrent split: outer `293.854ms`, `matmul_prep` `101.787ms`, `chunk_scan` `86.077ms`, `chunk_prep` `56.893ms`, `state_update` `44.123ms`. Decode recurrent split: outer `118.076ms`, `single_token_fallback` `117.203ms`; no `single_token_backend_step` appeared. No-profile sanity remained coherent at `1110.633ms` prefill / `86.877ms` mean ITL. | Keep profile instrumentation. Next decode experiment should test a Vulkan-only F32 recurrent-step route with rollback; prefill work should target matmul prep or chunk scan, not slicing/cat overhead. |
| A093 | Enable the existing Vulkan GDN recurrent-step backend for single-token F32 decode tensors. | Same-binary short and longer A/Bs stayed on backend `vulkan` and kept identical token IDs. Longer seed-95 candidate improved mean ITL from rollback `86.111ms` to `81.145ms` (`5.8%`) with prefill `1141.733ms -> 1040.197ms`; p99 was noisy at `88.974ms -> 90.309ms`. Focused profile showed decode recurrent now reaches `single_token_backend_step` (`71.461ms` of `72.815ms`) instead of A092's `single_token_fallback` (`117.203ms` of `118.076ms`). | Keep. Vulkan-only F32 route, BF16 behavior unchanged, CUDA/Metal F32 routing unchanged. Rollback via `KILN_DISABLE_VULKAN_GDN_RECURRENT_STEP_F32=1`. Re-profile before picking the next serial decode target. |
| A094 | Refresh current serial Vulkan profile after A093. | Combined profile stayed on backend `vulkan`, kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, and measured `1040.499ms` prefill / `83.173ms` mean ITL. Prefill buckets: MLP fused `231.962ms`, GDN recurrent `217.895ms`, GDN conv `154.415ms`, GDN in-proj `122.372ms`, full-attn QKV `96.645ms`. Decode buckets: MLP fused `240.587ms`, GDN in-proj `99.970ms`, GDN recurrent `72.924ms`, GDN out-proj `44.351ms`. Metal E441-E443 row-policy notes were reviewed and do not justify a direct Vulkan rowwise exception. | Keep as target-selection evidence. Next serial decode target is MLP fused, then GDN in-proj; next prefill targets remain MLP fused and GDN recurrent inner work. |
| A095 | Reject single-row packed-BF16 GDN in-proj `32x8` retile. | Temporary shader passed Vulkan compile checks and focused packed-BF16 GDN in-proj parity, and same-token A/Bs stayed on backend `vulkan`. No-profile seed-98 candidate regressed mean ITL `81.312ms` rollback to `83.231ms` candidate. Focused seed-99 profile showed the target bucket also regressed: batch=1 GDN in-proj total `97.650ms` rollback to `102.044ms` candidate, record/submit/wait `85.375ms` to `89.353ms`. | Rejected and reverted. Do not retry this direct `16x16 -> 32x8` lane trade for GDN in-proj; it matches A087's generic BF16 single-row rejection. |
| A096 | Default-enable Vulkan causal conv1d prefill while keeping single-token update opt-in. | Current-main retest showed `KILN_ENABLE_VULKAN_FUSED_CONV1D=1` kept token IDs but improved prefill `1138.956ms -> 948.826ms` while regressing decode `82.150ms -> 88.527ms`, so the gate was split. Same-binary new-default seed-101 A/B kept backend `vulkan` and token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`; candidate improved prefill `1088.396ms` rollback to `990.743ms` with flat mean ITL (`82.425ms` rollback, `82.368ms` candidate). Profile confirmed `seq_len=64 stage=conv` dropped `166.418ms -> 76.519ms` while `seq_len=1 stage=conv` stayed flat. Server smoke with `chat_template_kwargs: {"enable_thinking": false}` returned `content == "blue green"`. | Keep. Vulkan-only prefill route, single-token update still opt-in. Rollback via `KILN_DISABLE_VULKAN_FUSED_CONV1D_PREFILL=1`; update opt-in via `KILN_ENABLE_VULKAN_FUSED_CONV1D=1` or `KILN_ENABLE_VULKAN_FUSED_CONV1D_UPDATE=1`. CUDA/Metal checks remain host-blocked (`nvcc` missing; `objc2` Apple-only). |
| A097 | Refresh current Vulkan profile after A096 on merged `main`. | Final measured pass stayed on backend `vulkan`, kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, and measured `1028.621ms` prefill / `84.416ms` mean ITL / `94.471ms` p99. Prefill buckets: GDN recurrent `276.795ms`, MLP fused `234.900ms`, GDN in-proj `122.603ms`, full-attn QKV `98.255ms`, GDN conv `69.332ms`. Decode buckets: MLP fused `244.138ms`, GDN in-proj `102.437ms`, GDN recurrent `72.857ms`, GDN out-proj `44.729ms`, GDN gated-norm `41.984ms`. | Keep as post-merge target-selection evidence. A096 conv prefill remains active; next Vulkan work should target MLP fused / GDN recurrent internals or boundary/residency changes, not repeat direct GDN in-proj `32x8` or QKV/Z-pairing retiles. |
| A098 | Reject one-workgroup-per-head shared-prefix Vulkan GDN chunk-prep shader. | Temporary shader compiled and focused chunk prep/scan parity passed. All benches stayed on backend `vulkan` with token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`. No-profile pairs were mildly favorable but noisy: seed-104 candidate `982.057ms` prefill vs rollback `1011.186ms`; seed-106 counter-order candidate `961.678ms` vs rollback `970.076ms`. Focused seed-105 profile rejected the mechanism: candidate recurrent `chunk_prep` was worse (`60.043ms` vs rollback `51.086ms`), with adjacent `chunk_scan` (`80.667ms` vs `69.796ms`), `matmul_prep` (`68.216ms` vs `56.184ms`), and `state_update` (`42.110ms` vs `30.479ms`) also worse. | Rejected and reverted. Do not retry this reduced-workgroup shared-prefix shape without a different scheduling model; final source has no Vulkan/CUDA/Metal runtime changes from A098. |
| A099 | Reject native head-last Vulkan GDN recurrent prefill. | Temporary Metal-inspired short-prefill shader compiled, passed focused parity, and stayed on backend `vulkan` with token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`. Same-binary seed-107 A/B regressed candidate prefill `969.531ms` vs rollback `933.872ms`, mean ITL `82.470ms` vs `81.869ms`, and p99 `89.514ms` vs `88.484ms`. | Rejected and reverted. Do not directly port the Metal native head-last recurrent shape to Vulkan without a different scheduling/data-residency model; final source has no Vulkan/CUDA/Metal runtime changes from A099. |
| A100 | Chain Vulkan MLP gate/up and down projection into one command buffer. | Same-binary A/Bs stayed on backend `vulkan` and kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`. Seed-109 candidate improved prefill `1009.780ms -> 974.825ms` and mean ITL `81.414ms -> 80.388ms`; seed-110 counter-order improved `965.251ms -> 952.166ms` and `81.298ms -> 81.043ms`, though p99 was mixed (`88.605ms -> 96.078ms`). Focused profile confirmed the decode mechanism: outer MLP `seq_len=1` improved `246.136ms -> 234.216ms`, and kernel batch-1 total improved `240.425ms -> 228.764ms`. Server smoke returned visible `content == "Red Blue"`. | Keep. Vulkan-only boundary/submission win with rollback via `KILN_DISABLE_VULKAN_MLP_CHAINED_DISPATCH=1`; CUDA/Metal source paths untouched and local feature checks remain host-blocked (`nvcc` missing, `objc2` Apple-only). |
| A101 | Refresh current Vulkan profile after A100. | Profile-only run stayed on backend `vulkan`, kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, and measured `948.269ms` prefill / `83.326ms` mean ITL / `94.805ms` p99. Prefill buckets: MLP fused `231.337ms`, GDN recurrent `230.385ms`, GDN in-proj `123.588ms`, full-attn QKV `99.372ms`, GDN conv `52.679ms`. Decode buckets: MLP fused `233.406ms`, GDN in-proj `101.511ms`, GDN recurrent `73.549ms`, GDN out-proj `44.486ms`, GDN gated-norm `43.106ms`. | Keep as post-A100 target-selection evidence. Next work should target GDN recurrent prefill internals or a materially different live-batch policy inspired by Metal E453, not repeat rejected direct tile/native-head-last ports. |
| A102 | Batch Vulkan GDN chunk prep/scan transfers. | Batches the six GDN chunk-prep uploads/readbacks and the GDN chunk-scan uploads/readbacks through transient command-pool transfer helpers, guarded by `KILN_DISABLE_VULKAN_GDN_CHUNK_BATCHED_TRANSFERS=1`. Focused chunk parity, full Vulkan GDN parity (`34 passed`), Vulkan model check, release bench/server builds, and server visible-text smoke passed. Three unprofiled pairs stayed on backend `vulkan` with identical token IDs; average was candidate `995.380ms` prefill / `81.315ms` mean ITL / `90.491ms` p99 vs rollback `1003.290ms` / `81.724ms` / `92.293ms`. Profile showed target stages down: `chunk_prep` `61.509ms -> 52.540ms`, `chunk_scan` `81.817ms -> 73.896ms`. | Keep. Narrow Vulkan-only boundary win, CUDA/Metal source paths untouched; local CUDA/Metal checks remain host-blocked (`nvcc` missing, `objc2` Apple-only). |
| A103 | Refresh current Vulkan profile after A102. | Profile-only run stayed on backend `vulkan`, kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, and measured `990.023ms` prefill / `82.617ms` mean ITL / `92.073ms` p99. Prefill buckets: GDN recurrent `247.035ms`, MLP fused `232.175ms`, GDN in-proj `124.230ms`, full-attn QKV `98.913ms`, GDN conv `68.834ms`. Decode buckets: MLP fused `232.694ms`, GDN in-proj `100.500ms`, GDN recurrent `74.277ms`, GDN out-proj `44.544ms`, GDN gated-norm `40.264ms`. | Keep as post-A102 target-selection evidence. Current next targets remain MLP/GDN boundary or residency/submission work; avoid repeating rejected direct retile/native-head-last/shared-prefix ports. |
| A104 | Reject Vulkan GDN gates single-submit. | Temporary Vulkan-only candidate folded GDN gates `a`/`b` uploads, compute, and beta/g readbacks into one command buffer behind `KILN_DISABLE_VULKAN_GDN_GATES_SINGLE_SUBMIT=1`. Candidate and rollback focused parity passed; Vulkan model check and release bench build passed. Three same-token pairs stayed on backend `vulkan`; average was candidate `993.832ms` prefill / `79.934ms` mean ITL / `89.762ms` p99 vs rollback `981.042ms` / `80.741ms` / `91.346ms`. Approximate 64 prompt + 8 ITL total lost: candidate `1633.307ms` vs rollback `1626.970ms`. | Rejected and removed. Do not retry standalone GDN gates single-submit folding; final source has no A104 runtime changes and CUDA/Metal paths are untouched. |
| A105 | Fold Vulkan MLP upload, chained compute, and readback into one submit. | Added default-enabled MLP transfer/readback folding behind `KILN_DISABLE_VULKAN_MLP_CHAINED_TRANSFER_SUBMIT=1`, active only when the existing chained dispatch path is active. Focused MLP parity passed in candidate and rollback modes, full Vulkan parity passed (`34 passed`), Vulkan model check and release bench/server builds passed, and server visible-text smoke returned `content: "Red Blue"` on backend Vulkan. Three no-profile same-token pairs averaged candidate `945.605ms` prefill / `79.271ms` mean ITL / `86.741ms` p99 vs rollback `1021.432ms` / `81.049ms` / `91.701ms`; approximate 64 prompt + 8 ITL total improved `1669.825ms -> 1579.774ms`. | Keep. Stable Vulkan-only submit-boundary win; CUDA/Metal source paths untouched, with local CUDA/Metal checks still host-blocked by missing `nvcc` and Apple-only `objc2`. |
...
| A113 | Refresh current serial Vulkan profile after A110/A112. | No source change. Release Vulkan bench rebuild passed. Profile stayed on backend `vulkan` with token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, `995.890ms` prefill, `81.107ms` mean ITL, and `89.525ms` p99. Prefill buckets: GDN recurrent `248.884ms`, MLP fused `230.460ms`, GDN in-proj `126.143ms`, full-attn QKV `98.506ms`. Decode buckets: MLP fused `215.996ms`, GDN in-proj `104.535ms`, GDN recurrent `71.995ms`. | Keep as target-selection evidence. Serial bench does not exercise A110's batch-3 threshold; next Vulkan work should target MLP/GDN boundary, residency, or submission shape instead of direct Metal row-triple ports. |
| A114 | Confirm current live Vulkan correctness and capture valid batch-3 profile. | No source change. Release Vulkan server rebuild passed. Identical-prompt probes returned coherent visible text but only reached max batch `1`, so they are retained as cache-shape evidence. Distinct eight-prompt probe returned 8/8 HTTP 200 with visible text `"eight, nine, ten, eleven, twelve, thirteen, fourteen, fifteen,"`, `128` generated tokens, `120` submitted decode jobs, `41` worker batches, `120` rows, `0` failed jobs, and max batch `3`. Valid live `seq_len=1` totals: GDN recurrent `5296.912ms`, MLP fused `1564.591ms`, GDN in-proj `1083.287ms`, gated norm `782.558ms`. Batch-3 MLP kernel total was `1443.743ms`; batch-3 GDN in-proj total was `978.831ms`; batch-3 recurrent backend step was `5152.400ms`. | Keep as current live correctness/profile evidence. The endpoint is producing non-empty, sensible text on Vulkan and exercising GPU decode kernels at batch 3; next target is GDN recurrent state movement/residency or a broader MLP/GDN boundary change, not another direct small-row retile. |
| A115 | Use device-local Vulkan GDN recurrent state for multi-row batches. | Vulkan-only dispatcher policy change: batch `1` keeps host-visible state, while batch `>1` now uses device-local state plus staging copies by default. Three same-shape live batch-3 pairs all returned 8/8 correct visible streams with identical counters (`128` generated tokens, `120` jobs, `41` worker batches, max batch `3`). Device-local average was `16.230023s` vs host-visible `16.869020s`, a `3.94%` wall-time win. Full Vulkan GDN parity passed (`34 passed`), Vulkan model/server checks and release server build passed. CUDA and Metal feature checks remain environment-blocked on this Linux host (`nvcc` missing; `objc2` requires Apple target). | Accepted. Rollback via `KILN_ENABLE_VULKAN_GDN_RECURRENT_HOST_VISIBLE_BATCH_STATE=1`; existing all-batch device-local force env still works. CUDA/Metal source paths and batch-1 Vulkan behavior are unchanged. |
| A116 | Reject Vulkan parallel gates+recurrent decode fusion. | Temporary Vulkan-only candidate added a `gdn_decode_gates_recurrent_parallel` shader that folded gate math into the accepted parallel recurrent reduction shape while leaving gated RMSNorm split. Focused parity and Vulkan checks passed, and two live batch-3 A/B pairs were correct with identical counters (`128` generated tokens, `120` jobs, `41` worker batches, max batch `3`). Candidate averaged `16.947676s` vs rollback `16.565287s`, a `2.31%` regression. | Rejected and reverted. Do not retry gates+recurrent fusion by duplicating gate math in per-output-column recurrent workgroups; it needs a different layout or broader residency change. |
| A117 | Recheck Vulkan recurrent parallel-reduce policy at live batch 3. | No source change. Release Vulkan server build passed. Default parallel reduce and rollback `KILN_DISABLE_VULKAN_GDN_RECURRENT_PARALLEL_REDUCE=1` both returned 8/8 HTTP 200 streams with exact visible text `"eight, nine, ten, eleven, twelve, thirteen, fourteen, fifteen,"` and identical counters (`128` generated tokens, `120` jobs, `41` worker batches, max batch `3`). Default wall was `18.736848s`; rollback wall was `18.711553s`, only `0.135%` faster. | No change. Keep the existing default; the post-A115 live batch-3 shape is effectively tied, and earlier A059/A067 evidence still supports the guarded default in other decode shapes. Next recurrent work needs broader residency/layout changes, not just disabling the parallel shader. |
| A118 | Replace extra-copy batched `LinearAttentionState` scatter in scheduler-owned rows. | Added rollback-guarded `scatter_batch_rows_replace` for the batch-size >1 greedy contiguous decode path. Focused state assembly/scatter test, Vulkan model/server checks, and release Vulkan server build passed. Two live batch-3 A/B pairs were correct with identical counters (`128` generated tokens, `120` jobs, `41` worker batches, max batch `3`) and exact visible text `"eight, nine, ten, eleven, twelve, thirteen, fourteen, fifteen,"`. Candidate averaged `17.721158s` vs rollback `18.779328s`, a `5.97%` speedup. CUDA/Metal feature checks remain host-blocked (`nvcc` missing; `objc2` Apple-only). | Accepted. Rollback via `KILN_DISABLE_FAST_BATCHED_LINEAR_STATE_SCATTER=1`. Batch-1 decode, pointer-stable `scatter_batch_rows`, and CUDA/Metal kernels are unchanged. |
| A119 | Refresh current live Vulkan profile after A118 fast scatter. | No source change. Release Vulkan server build passed. Eight distinct streaming prompts with `chat_template_kwargs: {"enable_thinking": false}` returned 8/8 HTTP 200, exact visible text `"eight, nine, ten, eleven, twelve, thirteen, fourteen, fifteen,"`, empty reasoning, `128` generated tokens, `120` submitted jobs, `41` worker batches, `120` rows, and max batch `3`. Wall was `15.597403s`. Decode `seq_len=1` totals: GDN recurrent `4552.681ms`, MLP fused `1560.486ms`, GDN in-proj `1091.727ms`, gated norm `1032.909ms`; batch-3 recurrent backend step `4401.754ms`. | Profile only. A118 remains correct and the visible-text issue stays resolved with real template config. Next source work should target GDN recurrent residency/layout or a broader boundary change, not another direct Metal row-triple port. |
| A120 | Add default-off decode-batcher stage profiling and measure scheduler-side costs. | `KILN_PROFILE_DECODE_BATCHER_STAGES=1` now emits `job_metadata`, `runner_call`, `worker_total`, state assembly, forward, scatter, and total decode timings. CPU/no-feature and Vulkan model checks, focused decode-batcher policy test, Vulkan server check, release Vulkan server build, and live probe passed; CUDA/Metal checks remain host-blocked (`nvcc` missing; `objc2` Apple-only). Eight distinct streaming prompts returned exact visible text `"eight, nine, ten, eleven, twelve, thirteen, fourteen, fifteen,"`, empty reasoning, `128` generated tokens, `120` jobs, `41` worker batches, `120` rows, max batch `3`, and `15.082519s` wall. Batch-3 stage totals: state assembly `372.297ms`, batched forward `9711.218ms`, scatter replace `3.576ms`, decode total `10089.406ms`. | Keep as profile instrumentation. Scatter is no longer a target, state assembly is measurable but small, and the next Vulkan work should target GDN recurrent residency/layout or a broader boundary change. |
