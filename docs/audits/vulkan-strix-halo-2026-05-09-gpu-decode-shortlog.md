...
| A083 | Trial Metal-style one-workgroup-per-channel Vulkan conv1d prefill. | Temporarily added a `causal_conv1d_prefill_k4` shader that computed all timesteps for one `(batch, channel)` stream and advanced state after a workgroup barrier, with rollback via `KILN_DISABLE_VULKAN_CONV1D_PREFILL_FUSED_STATE=1`. Temporary source passed fmt, Vulkan compile checks, focused conv prefill parity, and release bench build. Same-binary seed-85 A/B stayed on backend `vulkan` and kept token IDs `[271,1206,1423,680,1204,1691,51864,3520,506]`, but regressed prefill `1083.188ms -> 1149.654ms`, mean ITL `85.966ms -> 87.241ms`, and p99 `87.387ms -> 88.507ms`. | Rejected and removed. Do not retry this by directly mirroring the Metal conv prefill workgroup shape; final source has no A083 code and CUDA/Metal paths are untouched. |
