...
### E112: Test One-Block Batch-Local Prefix Reuse Threshold

Change:
- Temporarily lowered `BATCH_LOCAL_PREFIX_MIN_BLOCKS` from 8 blocks to 1 block.
- Reverted after measurement.

Reasoning:
- E069 kept batch-local common-prefix reuse for long shared prompts but thresholded it to at least eight shared blocks because a shorter shared-prefix fixture regressed at the time.
- The current batch decode path is materially faster after E091-E108, so this retested whether short chat-template-style common prefixes now pay for themselves on the current four-prompt all-miss fixture.

Evidence:
- `cargo fmt --all --check` passed.
- `git diff --check` passed.
- As expected for the trial, `cargo test -p kiln-model batch_local_prefix -- --nocapture` failed because `batch_local_prefix_reuse_requires_minimum_shared_blocks` encodes the current 8-block threshold.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed for the trial.
- Fresh release-server no-profile same-request checks on the same four-prompt all-miss `/v1/completions/batch` fixture as E105-E111, `temperature=0`, `top_p=1`, `max_tokens=16`, 88 prompt tokens / 64 completion tokens:
  - E108 current kept baseline with 8-block threshold: 11.988s / 5.339 tok/s.
  - E112 one-block threshold trial: 11.983s / 5.341 tok/s by corrected wrapper; server logs for two trial requests were 12.030s and 11.975s.

Verdict:
- Reject and revert.
- The measured delta is effectively neutral and below noise, so it is not enough to justify weakening the 8-block threshold or rewriting the guard test.

### E113: Test Subgroup-Clustered Reduction For Batched Generic Linear Decode

Change:
- Temporarily transposed `linear_decode_batched.comp` from 32 output columns x 8 reduction lanes to 8 reduction lanes x 32 output columns.
- Replaced the shared-memory eight-lane reduction with `subgroupClusteredAdd(sum, 8u)`, following the clustered subgroup reduction pattern used by llama.cpp Vulkan kernels.
- Reverted after measurement.

Reasoning:
- The current generic batched linear path still spends about 60ms/step in the LM head and also covers other generic projections, so removing a shared-memory reduction and barrier looked like a plausible small win.
- With Vulkan local invocation order, the transposed 8x32 layout makes each column's eight reduction lanes contiguous, allowing a direct clustered subgroup sum.
- This was intentionally measured as an A/B trial because the transpose also sacrifices the original shader's more coalesced output-column weight loads.

Evidence:
- `cargo test -p kiln-vulkan-kernel --test gdn_parity linear_decode_batched -- --nocapture` passed.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed for the trial.
- Fresh release-server no-profile same-request checks on the same four-prompt all-miss `/v1/completions/batch` fixture as E105-E112, `temperature=0`, `top_p=1`, `max_tokens=16`, 88 prompt tokens / 64 completion tokens:
  - E108 current kept baseline with 32x8 shared-memory generic batched linear: 11.988s / 5.339 tok/s.
  - E113 subgroup-clustered 8x32 generic batched linear trial: 12.493s / 5.123 tok/s, then 12.293s / 5.206 tok/s on repeat.

Verdict:
- Reject and revert.
- The subgroup reduction removes the shared-memory barrier, but the transposed lane layout appears to lose enough coalesced weight-load behavior to regress the whole request.
- Keep E094's 32x8 shared-memory generic batched linear shape.

### E114: Batched Fused Vulkan LM-Head Argmax

Change:
- Added `linear_decode_argmax_batched_blocks.comp` and `linear_decode_argmax_batched_reduce.comp`.
- The block shader keeps the E094-style 32 output columns x 8 reduction lanes layout for coalesced vocab-weight loads, but writes one `(score, index)` maximum per batch row and vocab block instead of materializing logits.
- The reduce shader collapses those block maxima to one greedy token id per batch row.
- Added `dispatch_linear_decode_argmax_batched_cached`, Vulkan backend wiring through `linear_decode_argmax_batch`, native batch LM-head routing, focused parity coverage, and rollback `KILN_DISABLE_VULKAN_LINEAR_ARGMAX_BATCH=1`.

Reasoning:
- E106 showed that fanning out to existing per-row argmax dispatches was effectively neutral, and E113 showed that a subgroup-clustered transpose loses too much memory coalescing.
- A fused batched argmax can preserve the 32x8 coalesced matvec shape while avoiding `batch * vocab` logits readback and CPU greedy sampling.
- The extra block-reduce pass is small relative to the LM-head output transfer it removes.

Evidence:
- `cargo fmt --all --check` passed.
- `git diff --check` passed.
- `cargo test -p kiln-vulkan-kernel --test gdn_parity linear_decode_argmax -- --nocapture` passed, covering both single-row and batched argmax parity.
- `cargo test -p kiln-model native_batch_greedy -- --nocapture` passed.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed.
- Fresh release-server no-profile same-request checks on the same four-prompt all-miss `/v1/completions/batch` fixture as E105-E113, `temperature=0`, `top_p=1`, `max_tokens=16`, 88 prompt tokens / 64 completion tokens:
  - E108 current kept baseline with materialized batched logits: 11.988s / 5.339 tok/s.
  - E114 batched fused LM-head argmax before adding the rollback gate: 11.946s / 5.357 tok/s, then 11.982s / 5.341 tok/s.
  - E114 same-binary default after fixing `KILN_DISABLE_VULKAN_LINEAR_ARGMAX_BATCH=1` to gate only batched argmax: 12.349s / 5.183 tok/s, then 11.757s / 5.444 tok/s; two-run mean 12.053s / 5.310 tok/s.
  - E114 same-binary rollback `KILN_DISABLE_VULKAN_LINEAR_ARGMAX_BATCH=1`: 12.846s / 4.982 tok/s, then 12.290s / 5.208 tok/s; two-run mean 12.568s / 5.093 tok/s.

Verdict:
- Keep.
- The win is modest versus the earlier E108 anchor; fixed-gate same-binary first requests were noisy, but the repeat and two-run mean favored the device-side batched reduction, and the implementation has a direct env rollback.
- This replaces the native-batch greedy LM-head path's materialized logits plus CPU sampling with a device-side batched token-id reduction.

### E115: Retile Batched Fused LM-Head Argmax Blocks To 64x4

Change:
- Retiled `linear_decode_argmax_batched_blocks.comp` from 32 output columns x 8 reduction lanes to 64 output columns x 4 reduction lanes.
- Updated the batched argmax block count from `ceil(out_dim / 32)` to `ceil(out_dim / 64)`.

Reasoning:
- E114's fused batched argmax preserved coalesced column loads and avoided materialized-logit readback, but still wrote and reduced one block maximum per 32 vocab columns.
- A 64-column block halves the intermediate block-score/block-index buffers and the reduce pass work.
- This trades away some hidden-dimension reduction parallelism, so it needed a direct A/B rather than promotion by inspection.

Evidence:
- `cargo test -p kiln-vulkan-kernel --test gdn_parity linear_decode_argmax_batched -- --nocapture` passed for the 64x4 trial.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed for the 64x4 trial.
- Fresh release-server no-profile same-request checks on the same four-prompt all-miss `/v1/completions/batch` fixture as E105-E114, `temperature=0`, `top_p=1`, `max_tokens=16`, 88 prompt tokens / 64 completion tokens:
  - E114 fixed-gate 32x8 default: 12.349s / 5.183 tok/s, then 11.757s / 5.444 tok/s.
  - E115 64x4 batched LM-head argmax blocks: 11.858s / 5.397 tok/s, then 11.729s / 5.456 tok/s.

Verdict:
- Keep.
- The second run is the best current four-prompt all-miss result on this fixture, and the two-run shape is better than E114's 32x8 block version despite lower reduction parallelism.
- E115 keeps E114's rollback: `KILN_DISABLE_VULKAN_LINEAR_ARGMAX_BATCH=1`.

### E116: Test Batched Fused LM-Head Argmax Blocks At 128x2

Change:
- Temporarily retiled `linear_decode_argmax_batched_blocks.comp` from 64 output columns x 4 reduction lanes to 128 output columns x 2 reduction lanes.
- Temporarily updated the batched argmax block count from `ceil(out_dim / 64)` to `ceil(out_dim / 128)`.
- Reverted after measurement.

Reasoning:
- E115 improved E114 by halving the block-score/block-index intermediates and reduce-pass work.
- A 128-column block halves that work again, but reduces hidden-dimension parallelism to only two lanes per output column and doubles the serial in-workgroup max scan.
- This checks whether the LM-head argmax is still bottlenecked by block-reduce overhead or has crossed back into insufficient matvec reduction parallelism.

Evidence:
- `cargo test -p kiln-vulkan-kernel --test gdn_parity linear_decode_argmax_batched -- --nocapture` passed for the 128x2 trial.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed for the 128x2 trial.
- Fresh release-server no-profile same-request checks on the same four-prompt all-miss `/v1/completions/batch` fixture as E105-E115, `temperature=0`, `top_p=1`, `max_tokens=16`, 88 prompt tokens / 64 completion tokens:
  - E115 64x4 batched LM-head argmax blocks: 11.858s / 5.397 tok/s, then 11.729s / 5.456 tok/s.
  - E116 128x2 batched LM-head argmax blocks: 11.726s / 5.458 tok/s, then 11.905s / 5.376 tok/s.

Verdict:
- Reject and revert.
- The first run tied E115's best, but the repeat regressed and the two-run shape is worse than E115.
- Keep E115's 64x4 batched LM-head argmax block shape.

### E117: Refresh Batch Greedy Profile After Batched LM-Head Argmax

Change:
- No code change.
- Rebuilt and ran the current E115 kept source with `KILN_PROFILE_VULKAN_BATCH_GREEDY=1` on the same four-prompt all-miss `/v1/completions/batch` fixture, changed to `max_tokens=4`.

Reasoning:
- E114/E115 materially changed the LM-head path, so the previous E108 profile was stale.
- The next experiment should target the actual largest remaining bucket instead of continuing to tune the same LM-head block shape.

Evidence:
- Request: 88 prompt tokens / 16 completion tokens, 10.818s wall time, server logged 10.799s.
- Serial independent prefill remained dominant:
  - row 0: 21 prompt tokens, 2101.7ms.
  - row 1: 23 prompt tokens, 2408.2ms.
  - row 2: 22 prompt tokens, 2723.2ms.
  - row 3: 22 prompt tokens, 2770.7ms.
  - stacking linear state: 23.9ms.
- Decode step 0 was 299.0ms under profiling:
  - `linear.gdn=154.1ms/24`, `full.block=54.1ms/8`, `lm_head=43.7ms/1`, `linear.mlp=41.0ms/24`.
  - Subops: `gdn.fused_gates_recur_norm=72.5ms/24`, `gdn.in_proj=57.5ms/24`, `mlp.fused=54.0ms/32`, `gdn.out_proj=13.0ms/24`, `gdn.conv=9.7ms/24`.
- Later decode steps were steadier at 228.4ms and 226.0ms:
  - step 1 totals: `linear.gdn=88.4ms/24`, `full.block=53.9ms/8`, `lm_head=43.6ms/1`, `linear.mlp=41.2ms/24`.
  - step 2 totals: `linear.gdn=86.4ms/24`, `full.block=53.9ms/8`, `lm_head=43.3ms/1`, `linear.mlp=40.9ms/24`.
  - later subops: `mlp.fused=53.8-54.3ms/32`, `gdn.in_proj=43.8-47.6ms/24`, `gdn.out_proj=15.9-16.5ms/24`, `gdn.fused_gates_recur_norm=15.2-15.4ms/24`, `gdn.conv=8.5-9.8ms/24`.

Verdict:
- Keep profile finding.
- For short all-miss batches, independent prefill is now the dominant wall-time problem.
- In decode, LM-head is no longer uniquely dominant; useful next targets are true batched/full-attention work, GDN input projection, MLP fused work, or removing remaining transfer/materialization boundaries.

### E118: Test Batched Vulkan GDN Input Projection At 88x3

Change:
- Temporarily retiled `gdn_in_proj_decode_batched.comp` from 80 output columns x 3 reduction lanes to 88 output columns x 3 reduction lanes.
- Temporarily updated the batched GDN input-projection dispatch count from `ceil(total_out / 80)` to `ceil(total_out / 88)`.
- Reverted after measurement.

Reasoning:
- E099 established 80x3 as the current best batched GDN input-projection shape, while E100's 96x3 and E109's 80x4 both regressed.
- 88x3 tests the midpoint between 80x3 and 96x3 while preserving the same three reduction lanes, checking whether fewer column groups can help without paying the full 96-column cost.

Evidence:
- `cargo test -p kiln-vulkan-kernel --test gdn_parity gdn_in_proj -- --nocapture` passed for the 88x3 trial.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed for the 88x3 trial.
- Fresh release-server no-profile same-request checks on the same four-prompt all-miss `/v1/completions/batch` fixture as E105-E117, `temperature=0`, `top_p=1`, `max_tokens=16`, 88 prompt tokens / 64 completion tokens:
  - E115 current kept source with 80x3 batched GDN input projection and 64x4 batched LM-head argmax blocks: 11.858s / 5.397 tok/s, then 11.729s / 5.456 tok/s.
  - E118 88x3 batched GDN input-projection trial: 13.281s / 4.819 tok/s, then 12.068s / 5.303 tok/s on a fresh-server confirmation.
  - Server request logs for the two E118 trial runs were 13.260s and 12.049s.

Verdict:
- Reject and revert.
- The confirmation improved over the first run but still trailed the kept source, and the two-run shape is materially worse than E115.
- Keep E099's 80x3 batched GDN input-projection shape.

### E119: Test Batched Vulkan MLP Gate/Up At 144x2

Change:
- Temporarily retiled `mlp_gate_up_decode_batched.comp` from 128 output columns x 2 reduction lanes to 144 output columns x 2 reduction lanes.
- Temporarily updated the batched MLP gate/up dispatch count from `ceil(intermediate / 128)` to `ceil(intermediate / 144)`.
- Reverted after measurement.

Reasoning:
- E095 established 128x2 as the current best batched MLP gate/up shape, while E110 showed 160x2 goes too wide and regresses.
- 144x2 tests the midpoint between those shapes while keeping the same two reduction lanes, checking whether reducing the intermediate-column group count slightly can help without paying the full 160-column cost.

Evidence:
- `cargo test -p kiln-vulkan-kernel --test gdn_parity mlp_decode -- --nocapture` passed for the 144x2 trial.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed for the 144x2 trial.
- Fresh release-server no-profile same-request checks on the same four-prompt all-miss `/v1/completions/batch` fixture as E105-E118, `temperature=0`, `top_p=1`, `max_tokens=16`, 88 prompt tokens / 64 completion tokens:
  - E115 current kept source with 128x2 batched MLP gate/up and 64x4 batched LM-head argmax blocks: 11.858s / 5.397 tok/s, then 11.729s / 5.456 tok/s.
  - E119 144x2 batched MLP gate/up trial: 12.043s / 5.314 tok/s, then 11.944s / 5.358 tok/s on a fresh-server confirmation.
  - Server request logs for the two E119 trial runs were 12.025s and 11.924s.

Verdict:
- Reject and revert.
- The confirmation improved over the first run but remained slower than both E115 kept-source runs.
- Keep E095's 128x2 batched MLP gate/up shape.

### E120: Test Batched LM-Head Argmax Reduce Shader At 128 Lanes

Change:
- Temporarily changed `linear_decode_argmax_batched_reduce.comp` from 256 local lanes to 128 local lanes.
- Temporarily reduced the shared partial arrays from 256 to 128 entries, changed the block scan stride from 256 to 128, and changed the in-workgroup reduction start from 128 to 64.
- Reverted after measurement.

Reasoning:
- E115 made the block shader cheaper by reducing the number of block maxima, but the reduce pass still used 256 lanes per batch row.
- 128 lanes could reduce shared memory and one reduction barrier if the second-stage reduction was overprovisioned for roughly 3.9k vocab blocks.

Evidence:
- `cargo test -p kiln-vulkan-kernel --test gdn_parity linear_decode_argmax_batched -- --nocapture` passed for the 128-lane reduce trial.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed for the 128-lane reduce trial.
- Fresh release-server no-profile same-request check on the same four-prompt all-miss `/v1/completions/batch` fixture as E105-E119, `temperature=0`, `top_p=1`, `max_tokens=16`, 88 prompt tokens / 64 completion tokens:
  - E115 current kept source with 256-lane batched LM-head argmax reduce: 11.858s / 5.397 tok/s, then 11.729s / 5.456 tok/s.
  - E120 128-lane reduce trial: 12.479s / 5.129 tok/s.
  - Server request log for the E120 trial was 12.461s.

Verdict:
- Reject and revert.
- The 128-lane reducer regressed clearly, so the reduce pass benefits more from the extra scan parallelism than from the smaller workgroup.
- Keep E115's 64x4 block shader and 256-lane reduce shader.

### E121: Use Batched Vulkan MLP Decode For Prefill Rows

Change:
- Extended the Vulkan backend `mlp_decode` and `mlp_gate_up_decode` wrappers to accept `seq_len > 1` tensors by flattening `[batch, seq_len, hidden]` to `[batch * seq_len, 1, hidden]`, dispatching the existing batched Vulkan kernels, and reshaping the output back.
- Added opt-in prefill profiling with `KILN_PROFILE_PREFILL=1` / `KILN_PROFILE_VULKAN_BATCH_PREFILL=1`, reusing the existing decode-profile rows for `seq_len > 1`.
- Kept the existing rollback guard: `KILN_DISABLE_VULKAN_MLP_DECODE=1`.

Reasoning:
- E117 showed short all-miss native batch latency was dominated by serial independent prefill rows.
- E121's prefill profile found the biggest per-row prefill buckets were unfused MLP projections, not registration overhead:
  - Representative all-miss rows spent about 2.30-2.40s profiled each.
  - Per row, MLP gate/up/down together were about 1.10-1.13s across 32 layers.
  - Full-attention layers were about 450-468ms/8 and GDN input projection was about 415-436ms/24.
- The existing Vulkan MLP kernels were already batch-capable for `[rows, 1, hidden]`; the missing piece was letting prefill rows reuse them by flattening the sequence dimension.
...
- `cargo test -p kiln-model native_batch_greedy -- --nocapture` passed.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed.
- Fresh release-server no-profile same-request checks on the same four-prompt all-miss `/v1/completions/batch` fixture as E105-E120, `temperature=0`, `top_p=1`, 88 prompt tokens:
  - E115 current kept source at `max_tokens=16`: 11.858s / 5.397 tok/s, then 11.729s / 5.456 tok/s.
  - E121 default at `max_tokens=1`: 6.491s wall, server 6.472s, 4 completion tokens.
  - E121 same-binary rollback `KILN_DISABLE_VULKAN_MLP_DECODE=1` at `max_tokens=1`: 8.617s wall, server 8.595s.
  - E121 default at `max_tokens=16`: 10.512s / 6.088 tok/s, then 9.497s / 6.739 tok/s on a fresh-server confirmation.
  - E121 same-binary rollback `KILN_DISABLE_VULKAN_MLP_DECODE=1` at `max_tokens=16`: 13.671s / 4.681 tok/s, server 13.663s. This rollback disables decode MLP too, so it is not a prefill-only isolation, but it confirms the widened Vulkan MLP route is material to the request.

Verdict:
- Keep.
- This is the first large post-E115 all-miss batch win, improving the best full fixture from 11.729s to 9.497s on confirmation.
- The biggest remaining prefill buckets after this direction are likely full-attention prefill and GDN input projection; the added prefill profiler should be used to confirm the new post-E121 profile before the next larger change.

### E122: Use Row-Pair Vulkan Matvec For Larger Prefill MLP Batches

Change:
- Added llama.cpp-inspired row-pair variants of the batched MLP gate/up and generic linear decode shaders:
  - `mlp_gate_up_decode_batched_rows2.comp`
  - `linear_decode_batched_rows2.comp`
- For flattened MLP batches with `batch >= 8`, dispatch the row-pair shaders instead of the decode-tuned batched shaders.
- Kept the existing bs=4 decode-tuned shaders untouched for smaller batches.
- Added parity coverage by extending the MLP decode batched test to include batch 9.
- Added rollback guard: `KILN_DISABLE_VULKAN_PREFILL_ROW_PAIR_MATMUL=1`.

Reasoning:
- E121 made prefill rows use the existing batched MLP kernels, but the post-E121 profile still showed `mlp.fused` at about 450-508ms per row.
- llama.cpp's Vulkan matvec family uses multi-row work per group to reuse shared weight loads across adjacent input rows.
- For short flattened prefill rows, computing two rows per output-column tile can reuse gate/up/down weights across adjacent token rows without changing the surrounding backend contract.
- The `batch >= 8` gate keeps the E094/E095 decode-tuned bs=4 paths stable.

Evidence:
- `cargo fmt --all --check` passed.
- `glslc` compiled both new shaders with the same flags as `build.rs`.
- `cargo test -p kiln-vulkan-kernel --test gdn_parity mlp_decode -- --nocapture` passed.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed.
- Fresh release-server no-profile same-binary checks on the same four-prompt all-miss `/v1/completions/batch` fixture as E105-E121, `temperature=0`, `top_p=1`, 88 prompt tokens:
  - E122 default at `max_tokens=1`: 4.900s wall, server 4.885s.
  - E122 same-binary rollback `KILN_DISABLE_VULKAN_PREFILL_ROW_PAIR_MATMUL=1` at `max_tokens=1`: 6.011s wall, server 5.993s.
  - E122 default at `max_tokens=16`: 8.323s wall, server 8.305s, 64 completion tokens, 7.69 tok/s.
  - E122 same-binary rollback `KILN_DISABLE_VULKAN_PREFILL_ROW_PAIR_MATMUL=1` at `max_tokens=16`: 9.442s wall, server 9.424s, 64 completion tokens, 6.78 tok/s.
- Post-E122 prefill profile on the same `max_tokens=1` fixture:
  - `mlp.fused` moved down to about 241-265ms per row across 32 layers.
  - `gdn.in_proj` is now the largest row bucket at about 498-630ms per row across 24 layers.
  - Full-attention layers are about 271-303ms per row across 8 layers.

Verdict:
- Keep.
- This improves the best confirmed all-miss batch fixture from E121's 9.497s / 6.739 tok/s to 8.323s / 7.69 tok/s.
- The next prefill target should be GDN input projection, likely with the same row-pair weight-reuse idea or another multi-row matvec shape, before returning to full-attention prefill.

### E123: Test Row-Pair Vulkan Matvec For Prefill GDN Input Projection

Change:
- Temporarily added a row-pair variant of `gdn_in_proj_decode_batched.comp` for flattened prefill batches with `batch >= 8`.
- Added a GDN-specific rollback guard for the trial: `KILN_DISABLE_VULKAN_GDN_PREFILL_ROW_PAIR_IN_PROJ=1`.
- Extended the GDN input projection batched parity test to include batch 9.

Reasoning:
- E122's post-profile showed `gdn.in_proj` became the largest prefill bucket at about 498-630ms per row across 24 layers.
- The E122 row-pair idea worked for MLP because adjacent flattened token rows can share gate/up/down projection weight loads.
- GDN input projection has the same broad shape, so the first direct follow-up was to try the same two-row workgroup idea there.

Evidence:
- `cargo fmt --all --check` passed after formatting.
- `glslc` compiled the temporary shader with the same flags as `build.rs`.
- `cargo test -p kiln-vulkan-kernel --test gdn_parity gdn_in_proj -- --nocapture` passed.
- `cargo test -p kiln-vulkan-kernel --test gdn_parity mlp_decode -- --nocapture` passed.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed.
- Fresh release-server same-binary checks on the same four-prompt all-miss `/v1/completions/batch` fixture:
  - E123 default at `max_tokens=1`: 4.815s wall, server 4.797s.
  - E123 GDN-row-pair rollback `KILN_DISABLE_VULKAN_GDN_PREFILL_ROW_PAIR_IN_PROJ=1` at `max_tokens=1`: 4.864s wall, server 4.846s.
  - E123 default at `max_tokens=16`: 8.438s wall, server 8.421s.
  - E123 GDN-row-pair rollback at `max_tokens=16`: 8.455s wall, server 8.435s.

Verdict:
- Reject and revert.
- The tiny `max_tokens=1` default advantage did not become a material full-fixture win, and both `max_tokens=16` runs were slower than E122's 8.323s reference.
- Do not repeat this direct row-pair GDN input-projection shape; future GDN input-projection work needs a different tiling or a reduction in transfer/dispatch boundaries.

### E124: Add Full-Attention Prefill Breakdown Profiling

Change:
- Added prefill-only profile buckets inside the paged full-attention path for QKV projection, split/reshape, QK norm, RoPE, KV write/read, GQA expansion, scores/mask/softmax/weighted sum, output reshape/gate/projection, and full-layer residual/norm work.
- When prefill profiling is active, replaced the coarse `layer.full` row with the detailed buckets to avoid double-counting.
- Kept the helper gated by `KILN_PROFILE_PREFILL=1` / `KILN_PROFILE_VULKAN_BATCH_PREFILL=1`, so no-profile requests skip the decode-profile TLS path.

Reasoning:
- E122/E123 left full-attention at about 271-303ms per row across 8 layers, but that coarse bucket did not show whether the remaining cost was SDPA math, KV movement, projection matmuls, or block-local residual/norm work.
- The Vulkan flash-attn prefill hook remains disabled for Qwen3.5-4B's `head_dim=256`, so the next full-attention experiment needed a measured target before attempting a larger kernel.
...
- `cargo test -p kiln-model native_batch_greedy -- --nocapture` passed.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed.
- Fresh release-server no-profile smoke check on the same four-prompt all-miss `max_tokens=1` fixture measured 4.819s wall, server 4.803s, 88 prompt tokens / 4 completion tokens.
- Fresh profiled release-server `max_tokens=1` fixture:
  - Row totals were 1260.0ms, 1258.2ms, 1540.9ms, and 1623.2ms profiled.
  - Full-attention SDPA math was tiny: `full.scores`, `full.mask`, `full.softmax`, and `full.weighted_sum` were each below 1ms per row across 8 layers.
  - Full-attention projection buckets dominated the full-attention cost: `full.qkv_proj` was about 121.7-180.7ms/8 and `full.o_proj` was about 44.2-73.0ms/8.
  - GDN projection buckets still dominated whole-row prefill: `gdn.in_proj` was about 445.7-688.9ms/24 and `gdn.out_proj` was about 131.7-211.6ms/24.

Verdict:
- Keep the profiler.
- The next broad prefill target should be generic projection reuse, not SDPA/softmax. In particular, widening Vulkan `linear_decode` to prefill rows should cover full-attention Q/K/V fallback projections, full-attention `o_proj`, and GDN `out_proj` before attempting a larger fused QKV or flash-attn kernel.

### E125: Use Vulkan Generic Linear Decode For Prefill Rows

Change:
- Widened the Vulkan backend `linear_decode` wrapper to accept `seq_len > 1` tensors by flattening `[batch, seq_len, hidden]` to `[batch * seq_len, 1, hidden]`, dispatching the existing cached Vulkan generic linear kernel, and reshaping back.
- Kept the existing rollback guard: `KILN_DISABLE_VULKAN_LINEAR_DECODE=1`.

Reasoning:
- E124 showed that the remaining full-attention prefill cost was mostly projection work, not the attention score/softmax/weighted-sum path.
- The existing generic Vulkan linear kernel already supports multiple rows as `[batch, 1, hidden]`; the backend wrapper was still rejecting `seq_len > 1`, so prefill full-attention Q/K/V fallback projections, full-attention `o_proj`, and GDN `out_proj` fell back to the slower portable path.
- This follows the E121 MLP prefill widening pattern, but uses the generic linear path for shared projection sites instead of adding a new fused shader first.

Evidence:
- `cargo fmt --all --check` passed.
- `git diff --check` passed.
- `cargo check -p kiln-model --features vulkan` passed.
- `cargo test -p kiln-vulkan-kernel --test gdn_parity linear_decode -- --nocapture` passed.
- `cargo test -p kiln-model native_batch_greedy -- --nocapture` passed.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed.
- Fresh release-server no-profile same-request checks on the same four-prompt all-miss `/v1/completions/batch` fixture:
  - E125 default at `max_tokens=1`: 4.324s wall, server 4.309s, 88 prompt tokens / 4 completion tokens.
  - E125 broad rollback `KILN_DISABLE_VULKAN_LINEAR_DECODE=1` at `max_tokens=1`: 6.618s wall, server 6.602s. This rollback disables decode-side generic linear paths too, so it is not a prefill-only isolation.
  - E125 default at `max_tokens=16`: 7.856s wall, server 7.839s, 64 completion tokens, about 8.17 tok/s.
- E125 profiled `max_tokens=1` fixture:
  - Row totals moved down to 1124.1ms, 1066.7ms, 1046.8ms, and 1030.9ms profiled.
  - `full.qkv_proj` moved to about 87.7-104.9ms/8.
  - `full.o_proj` moved to about 18.2-19.6ms/8.
  - `gdn.out_proj` moved to about 60.4-65.1ms/24.
  - `gdn.in_proj` remained the largest bucket at about 405.3-462.4ms/24.

Verdict:
- Keep.
- This improves the best confirmed all-miss `max_tokens=16` fixture from E122's 8.323s / 7.69 tok/s to 7.856s / 8.17 tok/s.
- The next remaining prefill target is still GDN input projection, but E123 showed direct row-pair sharing was too weak; the next attempt should reduce GDN input-projection dispatch/transfer boundaries or try a different larger-row tiling instead of repeating the two-row shape.

### E126: Use Vulkan GDN Input Projection For Prefill Rows

Change:
- Widened the Vulkan backend `gdn_in_proj_decode` wrapper to accept `seq_len > 1` tensors by flattening `[batch, seq_len, hidden]` to `[batch * seq_len, 1, hidden]`, dispatching the existing cached 80x3 batched GDN input-projection kernel, and reshaping `qkv`, `z`, `a`, and `b` back to `[batch, seq_len, dim]`.
- Added a prefill-only rollback guard: `KILN_DISABLE_VULKAN_GDN_PREFILL_IN_PROJ=1`.
- Decode-side `seq_len == 1` behavior is unchanged by the new guard.

Reasoning:
- E125 left `gdn.in_proj` as the largest remaining prefill bucket at about 405.3-462.4ms/24 layers.
- The existing batched GDN input-projection kernel already accepts multiple rows as `[batch, 1, hidden]`; the backend wrapper still rejected prefill tensors before reaching it.
- This avoids reintroducing E123's rejected row-pair shader and instead first removes the wrapper-level fallback to four portable matmuls.
...
- `cargo test -p kiln-model native_batch_greedy -- --nocapture` passed.
- `cargo build --release --features vulkan --bin kiln --bin kiln-bench` passed.
- Fresh release-server no-profile same-binary checks on the same four-prompt all-miss `/v1/completions/batch` fixture:
  - E126 default at `max_tokens=1`: 4.129s wall, server 4.122s, 88 prompt tokens / 4 completion tokens.
  - E126 prefill-only rollback `KILN_DISABLE_VULKAN_GDN_PREFILL_IN_PROJ=1` at `max_tokens=1`: 4.761s wall, server 4.744s.
  - E126 default at `max_tokens=16`: 7.526s wall, server 7.507s, 64 completion tokens, about 8.50 tok/s.
  - E126 prefill-only rollback at `max_tokens=16`: 8.257s wall, server 8.241s, 64 completion tokens, about 7.75 tok/s.
- E126 profiled `max_tokens=1` fixture:
  - Row totals were 989.4ms, 1078.4ms, 1018.0ms, and 1015.5ms profiled.
  - `gdn.in_proj` moved down to about 333.4-375.3ms/24.
  - `gdn.out_proj` stayed about 56.1-64.1ms/24.
  - `full.qkv_proj` stayed about 89.9-104.4ms/8 and `full.o_proj` about 19.4-20.8ms/8.

Verdict:
- Keep.
- This improves the best confirmed all-miss `max_tokens=16` fixture from E125's 7.856s / 8.17 tok/s to 7.526s / 8.50 tok/s, with a prefill-only rollback showing the widened GDN input-projection route is material.
- `gdn.in_proj` remains the largest prefill bucket, but the next step likely needs fewer GDN prefill transfer/dispatch boundaries or a different prefill-specific tiling rather than the direct row-pair shape E123 already rejected.

### E127: Retest Opt-In Vulkan Fused Conv1d After E126

Change:
- No code change.
- Retested the existing opt-in `KILN_ENABLE_VULKAN_FUSED_CONV1D=1` path against the post-E126 source.

Reasoning:
- E126 moved `gdn.in_proj` down, leaving `gdn.conv` as a visible but smaller bucket at about 61.1-74.7ms/24 in the profiled all-miss prefill rows.
- The fused conv1d implementation is already present behind an opt-in gate, so this was a cheap A/B before adding another GDN input-projection shader variant.

Evidence:
- Fresh release-server opt-in checks on the same four-prompt all-miss `/v1/completions/batch` fixture:
  - E127 opt-in `KILN_ENABLE_VULKAN_FUSED_CONV1D=1` at `max_tokens=1`: 4.115s wall, server 4.097s.
  - E126 default reference at `max_tokens=1`: 4.129s wall, server 4.122s.
  - E127 opt-in at `max_tokens=16`: 8.161s wall, server 8.144s, 64 completion tokens, about 7.84 tok/s.
  - E126 default reference at `max_tokens=16`: 7.526s wall, server 7.507s, 64 completion tokens, about 8.50 tok/s.

Verdict:
- Reject as a default promotion.
- The `max_tokens=1` movement is below noise, while the full 64-token fixture regresses materially.
- Keep fused conv1d opt-in only via `KILN_ENABLE_VULKAN_FUSED_CONV1D=1`.

### E128: Test Equal-Length Native Batch Prefill Grouping

Change:
- Temporarily grouped all-miss rows with the same uncached prompt length through a new same-length paged batch prefill path.
- Added a trial rollback guard: `KILN_DISABLE_VULKAN_BATCH_EQUAL_LENGTH_PREFILL=1`.
- The trial path kept full-attention row-serial through independent block tables while running GDN/MLP layers over `[batch, seq_len, hidden]`, then split the stacked linear-attention state back into per-row states.

Reasoning:
- E117 and later prefill profiling showed the four-prompt all-miss fixture still spends most wall time in independent serial prefill rows.
- The standard fixture has two rows with the same prompt length, so grouping them was a direct way to test whether removing an entire per-row prefill pass helps before attempting more invasive variable-length batch prefill.

Evidence:
- Trial code validation passed:
  - `cargo check -p kiln-model --features vulkan`
  - `cargo fmt --all --check`
  - `git diff --check`
  - `cargo test -p kiln-model native_batch_greedy -- --nocapture`
  - `cargo build --release --features vulkan --bin kiln --bin kiln-bench`
- Fresh release-server same-request checks on the same four-prompt all-miss `/v1/completions/batch` fixture:
  - E128 default at `max_tokens=1`: 4.189s wall, server 4.171s.
  - Same-binary rollback `KILN_DISABLE_VULKAN_BATCH_EQUAL_LENGTH_PREFILL=1` at `max_tokens=1`: 4.186s wall, server 4.168s.
  - The grouped equal-length rows `[2, 3]` took 2195.8ms, while the rollback path processed those rows individually at 1041.3ms + 1034.8ms.
  - E128 default at `max_tokens=16`: 7.617s wall, server 7.602s.
  - E126 default reference at `max_tokens=16`: 7.526s wall, server 7.507s, about 8.50 tok/s.

Verdict:
- Reject and revert.
- Batched same-length prefill did not remove enough work because full-attention stayed row-serial and state splitting plus stack/unstack overhead erased the intended win.
- Do not restart from this same whole-prefill grouping shape after the pause; future prefill work should target a specific remaining bucket instead.

## Next Candidate

After E126-E128, the current best confirmed four-prompt all-miss `max_tokens=16` fixture remains 7.526s / 8.50 tok/s, with `KILN_DISABLE_VULKAN_GDN_PREFILL_IN_PROJ=1` at 8.257s / 7.75 tok/s. E127 retested existing fused conv1d and rejected it as a default promotion because `max_tokens=16` regressed to 8.161s / 7.84 tok/s. E128 tried same-length whole-prefill grouping and rejected it because the grouped equal-length rows `[2, 3]` took 2195.8ms versus 1041.3ms + 1034.8ms on the same-binary rollback path, and the full `max_tokens=16` fixture regressed to 7.617s. The remaining prefill work should still prioritize GDN input projection, but the next useful attempt needs to reduce GDN prefill transfer/dispatch boundaries or try a materially different prefill-specific tiling; E123 already rejected the direct row-pair shape, E126 only removes the wrapper-level portable matmul fallback, and E128 shows row-serial full-attention makes whole-prefill grouping unattractive.

With E108 removing repeated row RoPE table construction and E107 removing repeated row-position tensor construction in the full-attention batch path, the best observed bs=1 short-source anchor remains E084's 129.1ms mean ITL, with a second E084 no-profile run at 130.2ms. The current E087 default measured 130.4ms and 130.7ms while its rollback guard measured 132.3ms and 133.7ms, so the retiled full-attention QKV path is kept as a current-source micro-win and is now correctly gated to batch=1. E089 restored native-batch GDN input projection after the E088 misplaced guard, moving the four-prompt `max_tokens=16` fixture from the broken 46.581s / 1.374 tok/s back to 25.457s / 2.514 tok/s. E091 promoted batch fused-GDN resident state and moved the same fixture to 15.498s / 4.130 tok/s, with rollback `KILN_DISABLE_VULKAN_GDN_DECODE_FUSED_RESIDENT_STATE=1` at 25.917s / 2.469 tok/s. E092 retiled batched MLP gate/up to 64x4 and moved the fixture to 14.550s / 4.399 tok/s. E093 retiled batched GDN input projection to 64x4 and moved that fixture to 13.981s / 4.578 tok/s. E094 retiled the generic batched linear shader to 32x8 and improved a same-fixture 72-prompt-token four-prompt run from 13.204s / 4.847 tok/s with 16x16 rollback to 12.942s / 4.945 tok/s. E095 retiled batched MLP gate/up to 128x2 and moved that same 72-prompt-token fixture to 11.880s / 5.387 tok/s. E096 tried batched GDN input projection at 128x2 and regressed to 12.542s / 5.103. E097 tried skipping final-step paged K/V writes and measured 11.902s / 5.377 tok/s, effectively neutral versus E095's 11.880s / 5.387, so that special cache path was rejected. E098 tried batched MLP gate/up at 256x1 and regressed to 12.081s / 5.298, E105 tried 96x3 and regressed to 12.490s / 5.124 versus a same-request kept 128x2 baseline at 12.301s / 5.203, E110 tried 160x2 and regressed to 12.669s / 5.052 versus E108's 11.988s / 5.339 same-request baseline, and E119 tried the midpoint 144x2 and regressed to 12.043s / 5.314 then 11.944s / 5.358, so E095's 128x2 MLP shape remains best for decode. E121 then removed the prefill-only `seq_len == 1` restriction in the Vulkan MLP backend wrapper by flattening prefill rows into the existing batched kernels; the same all-miss fixture moved from E115's 11.858s / 5.397 and 11.729s / 5.456 to 10.512s / 6.088 and 9.497s / 6.739, while same-binary `KILN_DISABLE_VULKAN_MLP_DECODE=1` measured 13.671s / 4.681. E122 added row-pair MLP gate/up and down-projection shaders for flattened prefill batches with `batch >= 8`, inspired by llama.cpp's multi-row Vulkan matvec weight reuse; the fixture improved again to 8.323s / 7.69 tok/s versus same-binary `KILN_DISABLE_VULKAN_PREFILL_ROW_PAIR_MATMUL=1` at 9.442s / 6.78 tok/s. E123 tried applying the same row-pair shape to GDN input projection and was rejected: `max_tokens=1` barely moved at 4.815s default versus 4.864s rollback, while `max_tokens=16` was 8.438s default versus 8.455s rollback and did not beat E122's 8.323s reference. E124 showed full-attention prefill was projection-bound, not SDPA-bound: `full.qkv_proj` was about 121.7-180.7ms/8, `full.o_proj` about 44.2-73.0ms/8, and the score/mask/softmax/weighted-sum buckets were each below 1ms per row. E125 widened generic Vulkan `linear_decode` to flattened prefill rows, moving the all-miss fixture to 4.324s at `max_tokens=1` and 7.856s / 8.17 tok/s at `max_tokens=16`; its broad rollback `KILN_DISABLE_VULKAN_LINEAR_DECODE=1` measured 6.618s at `max_tokens=1`, and the profile moved `full.qkv_proj` to about 87.7-104.9ms/8, `full.o_proj` to about 18.2-19.6ms/8, and `gdn.out_proj` to about 60.4-65.1ms/24. E099 retiled batched GDN input projection to 80x3 and confirmed 11.715s / 5.463 then 11.789s / 5.429, so 80x3 remains the best decode-side batched GDN input-projection shape. E100 tried 96x3 and regressed to 11.984s / 5.341, E109 tried 80x4 and regressed to 12.366s / 5.175 versus E108's 11.988s / 5.339 same-request baseline, and E118 tried the midpoint 88x3 and regressed to 13.281s / 4.819 then 12.068s / 5.303, confirming the E099 shape is the useful middle point between 64x4, wider groups, and extra reduction lanes. E101 tried generic batched linear at 48x6 and regressed to 11.964s / 5.349; E111 tried 40x8 and regressed to 13.135s / 4.872; E113 tried a llama.cpp-inspired subgroup-clustered 8x32 reduction and regressed to 12.493s / 5.123 then 12.293s / 5.206 versus E108's 11.988s / 5.339 same-request baseline, so E094's 32x8 shared-memory shape remains best. E112 lowered the batch-local common-prefix threshold from 8 blocks to 1 block and measured 11.983s / 5.341 versus E108's 11.988s / 5.339 same-request baseline, effectively neutral and below noise, so the 8-block threshold remains best. E114 keeps a fused batched LM-head argmax that preserves coalesced matvec loads and avoids materialized-logit readback; fixed-gate same-binary default measured 12.349s / 5.183 then 11.757s / 5.444 versus rollback at 12.846s / 4.982 then 12.290s / 5.208. E115 retiled that argmax block shader from 32x8 to 64x4 and measured 11.858s / 5.397 then 11.729s / 5.456, making it the best pre-E121 short all-miss result on this fixture. E116 tried 128x2 for the same argmax block shader and measured 11.726s / 5.458 then 11.905s / 5.376, so it was rejected in favor of E115's more stable 64x4 shape. E120 tried shrinking the batched argmax reduce shader from 256 lanes to 128 lanes and regressed to 12.479s / 5.129, so E115's 256-lane reducer remains best. E117 refreshed the batch greedy profile after the LM-head changes: short all-miss batch latency is dominated by serial independent prefill rows at 2.1-2.8s each, while later decode steps are now balanced across `linear.gdn` (~86-88ms/24), `full.block` (~54ms/8), `lm_head` (~43ms), and `linear.mlp` (~41ms/24). E121's prefill profiler showed those row-prefill totals were mostly unfused MLP projections, full-attention layers, and GDN input projection; E122 moved `mlp.fused` down to about 241-265ms per row, E124/E125 removed the main generic-projection prefill fallback, and `gdn.in_proj` is still the largest prefill bucket at about 405.3-462.4ms/24. E102 tried bs=1 MLP gate/up at 80x3 and produced one 128.9ms run but regressed to 134.8ms on confirmation, so E084's 64x4 shape remains best. E103 attempted to refresh the stale mixed cache-hit/cache-miss anchor but all reconstructed hit rows missed the prefix cache, so those timings are not valid mixed anchors; E104 now protects the exact model-level mixed hit/miss path but does not replace the stale performance anchor. E106 showed that simply fanning LM-head argmax out to per-row dispatches is effectively neutral, E113 showed that a direct subgroup-clustered generic linear reduction loses too much memory coalescing, E114 lands the coalesced fused batched-reduction version, E115 reduces its block-reduce overhead, E116 shows 128x2 block shape goes too far, and E120 shows shrinking the final reduce shader also regresses. The next useful LM-head work would need to keep the final token path resident with surrounding decode work. E079 showed that the opposite 8x32 MLP shape and two-kernel single-submit recording do not improve whole-token latency; E083/E084 show that increasing output-column parallelism is the useful direction for the single-token MLP gate/up shader, with 64x4 currently best. E085 confirmed 128x2 goes too far and regresses the bs=1 MLP bucket, so E095 is deliberately batch-only. E086 confirmed that applying 32x8 to bs=1 generic `linear_decode` regresses MLP down, full-attention, and GDN output projections, so E094 is also batch-only. The next useful experiment should prioritize GDN input projection by reducing dispatch/transfer boundaries or trying a different prefill-specific tiling; the direct two-row E123 shape was below noise, but the E125 profile now makes `gdn.in_proj` the clear remaining prefill target. Speculative decode can consider resident-state scopes later, now that explicit materialization exists for resident recurrent state.

For bs>1, duplicate deterministic work is eliminated, batched GDN input projection is on Vulkan, E059 promoted native batch with batch-default fused GDN decode, E061 stops decoding rows after EOS, E062 routes single-row tails back to the tuned bs=1 decode path, E069 removes duplicate prefill for long in-batch common prefixes, E072 reuses those common prefixes across later batches, E091 keeps fused-GDN recurrent state resident through lockstep batch decode, E094 reduces generic batched projection workgroup count, E095 reduces batched MLP gate/up workgroup count further, E099 lands the best current batched GDN input-projection workgroup balance, and E114 avoids native-batch LM-head logits materialization for greedy decode. The remaining throughput work is now true multi-sequence paged attention and scheduling: batched KV write/read and SDPA for full-attention layers, continuous batching beyond the batch endpoint, and reducing remaining MLP/full-attention transfer boundaries.
