...
After E126/E127, the current best confirmed four-prompt all-miss `max_tokens=16` fixture remains 7.526s / 8.50 tok/s, with `KILN_DISABLE_VULKAN_GDN_PREFILL_IN_PROJ=1` at 8.257s / 7.75 tok/s. E127 retested existing fused conv1d and rejected it as a default promotion because `max_tokens=16` regressed to 8.161s / 7.84 tok/s. The remaining prefill work should still prioritize GDN input projection, but the next useful attempt needs to reduce GDN prefill transfer/dispatch boundaries or try a materially different prefill-specific tiling; E123 already rejected the direct row-pair shape, and E126 only removes the wrapper-level portable matmul fallback.
