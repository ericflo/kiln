...
After E126, the current best confirmed four-prompt all-miss `max_tokens=16` fixture is 7.526s / 8.50 tok/s, with `KILN_DISABLE_VULKAN_GDN_PREFILL_IN_PROJ=1` at 8.257s / 7.75 tok/s. The remaining prefill work should still prioritize GDN input projection, but the next useful attempt needs to reduce GDN prefill transfer/dispatch boundaries or try a materially different prefill-specific tiling; E123 already rejected the direct row-pair shape, and E126 only removes the wrapper-level portable matmul fallback.
