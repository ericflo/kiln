...
2. **Disk-fill (see also §8).** Each unique `(name, scale)` permutation is a distinct cache entry. A loop of `chat_completions` with random scales creates an unbounded set of `.composed/<hash>/` directories. *Resolved by the LRU eviction added for item 8 in branch `ce/phase9-composed-cache-lru`; both byte and entry caps fire in `synthesize_composed_adapter`.*
...
**Resolution (total-size cap, item 9).** `adapters.max_disk_bytes` (default 100 GiB, override via `KILN_ADAPTERS_MAX_DISK_BYTES`, set to `0` to opt out) added in branch `ce/phase9-adapter-disk-cap`. The upload handler in `crates/kiln-server/src/api/adapters.rs` now sums the on-disk size of finalized adapter directories under `adapter_dir/` (skipping `.upload-tmp-*/` staging dirs and the `.composed/` cache, both of which are bounded separately) right before the staging→target rename. If the sum plus the just-extracted byte count would exceed the cap, the upload is rejected with HTTP 507 `adapter_disk_quota_exceeded` and no partial adapter is left behind.

**Resolution (composed-cache LRU, item 8).** `adapters.composed_cache_max_bytes` (default 10 GiB, env override `KILN_ADAPTERS_COMPOSED_CACHE_MAX_BYTES`, `0` to disable) and `adapters.composed_cache_max_entries` (default 64, env override `KILN_ADAPTERS_COMPOSED_CACHE_MAX_ENTRIES`, `0` to disable) added in branch `ce/phase9-composed-cache-lru`. After each successful synthesize, `synthesize_composed_adapter` (`crates/kiln-server/src/api/completions.rs`) runs `evict_composed_cache_lru` over `adapter_dir/.composed/`, removing oldest-mtime entries until both caps are satisfied. Cache hits refresh the entry's mtime via `filetime::set_file_mtime` so reuse counts as recency. Eviction runs inside the existing `spawn_blocking` block so it does not block the runtime, and individual `remove_dir_all` failures are logged without failing the request.
...
8. ~~**Cap composed-adapter cache size** (§6, §8). LRU-evict `.composed/<hash>/` entries above N total or M GiB.~~ **Fixed** in branch `ce/phase9-composed-cache-lru` — `adapters.composed_cache_max_bytes` (default 10 GiB, env override `KILN_ADAPTERS_COMPOSED_CACHE_MAX_BYTES`, `0` to disable) and `adapters.composed_cache_max_entries` (default 64, env override `KILN_ADAPTERS_COMPOSED_CACHE_MAX_ENTRIES`, `0` to disable) added; LRU eviction by directory mtime runs after each successful synthesize in `synthesize_composed_adapter` (`crates/kiln-server/src/api/completions.rs`). Cache hits refresh mtime via `filetime::set_file_mtime` so reuse counts as recency. Eviction is best-effort and runs inside the existing `spawn_blocking` so it does not block the runtime.
9. ~~**Cap total adapter-dir disk usage** (§8). Reject uploads that would push total adapter-dir size above `adapters.max_disk_bytes`.~~ **Fixed** in branch `ce/phase9-adapter-disk-cap` — `adapters.max_disk_bytes` (default 100 GiB, env override `KILN_ADAPTERS_MAX_DISK_BYTES`, `0` to disable) checked pre-rename in `upload_adapter`; rejects with HTTP 507 `adapter_disk_quota_exceeded`.
