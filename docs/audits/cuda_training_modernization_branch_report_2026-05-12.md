...
the per-token last-dim broadcast multiply and a single-token recurrent GDN step, but these pieces
are not yet threaded through a recurrent GDN layer or full-model training step.
