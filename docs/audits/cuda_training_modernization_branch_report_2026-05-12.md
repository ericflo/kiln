...
per-`TensorId` gradient store for a future CUDA autograd graph. CUDA add and multiply ops prove that
the tape can propagate and accumulate gradients through real CUDA tensor ops, including a product
rule that uses saved inputs. It still does **not** have native CUDA Qwen forward/backward ops or an
allocator/arena equivalent to the Vulkan native training path.
