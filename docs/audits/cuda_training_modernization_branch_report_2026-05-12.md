...
repeat-KV-heads, last-dim narrow, and rank-2 row index-select ops prove that the tape can propagate
and accumulate gradients through real CUDA tensor ops, including product and quotient rules,
mixed-precision passthrough casts, scalar-loss reductions, shape/view movement,
projection/attention-shaped matmul, softmax Jacobian-vector products, sigmoid gates, SwiGLU
activation derivatives, GQA head-repeat gradient summation, slice-gradient scattering, and row
gather scatter-add
