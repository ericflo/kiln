...
per-`TensorId` gradient store for a future CUDA autograd graph. CUDA add, multiply, and sum
reduction ops prove that the tape can propagate and accumulate gradients through real CUDA tensor
ops, including a product rule that uses saved inputs and scalar-loss reductions. It still does
**not** have native CUDA Qwen forward/backward ops or an allocator/arena equivalent to the Vulkan
...
| Native CUDA training stack | `crates/kiln-model/src/cuda_train.rs` provides an initial CUDA-only tensor shell over candle CUDA storage with op IDs, parameter `TensorId`, `requires_grad`, detach semantics, a backward-op trait, reverse-topology traversal, per-parameter grad storage, CUDA add/mul/sum backward ops, and resident optimizer-kernel delegation. There is still no CUDA equivalent of `vk_train.rs`, native Qwen forward/backward ops, or a training arena. | Initial tensor/autograd boundary present; full native stack missing. |
