...
proves loss/backward reaches Q/K/V parameters; causal masking is still a missing CUDA native train
op. Native SGD and AdamW helpers now apply those gradients through resident optimizer kernels and
...
| Native CUDA training stack | `crates/kiln-model/src/cuda_train.rs` provides an initial CUDA-only tensor shell over candle CUDA storage with op IDs, parameter `TensorId`, `requires_grad`, detach semantics, a backward-op trait, reverse-topology traversal, per-parameter grad storage, CUDA add/sub/mul/div/scale/cast/sum/mean/reshape/transpose/last-two-transpose/matmul/batched-matmul/softmax/sigmoid/SiLU/repeat-KV/narrow/index-select/attention-permute backward ops, an unmasked SDPA composition helper, resident SGD/AdamW optimizer delegation, tiny optimizer loss-decrease proofs, and conservative arena allocation accounting. `crates/kiln-train/src/cuda_train.rs` adds a minimal linear AdamW train-step bridge with caller-owned arena support plus a safetensors save-boundary helper for named CUDA training tensors. There is still no CUDA equivalent of `vk_train.rs`, native Qwen forward/backward ops, causal-mask train op, or a custom pooled training allocator. | Initial tensor/autograd/optimizer/arena/save boundary and train-crate bridge present; full native stack missing. |
