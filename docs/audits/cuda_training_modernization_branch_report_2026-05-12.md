...
matmul, exact-batch batched matmul, F32 softmax-last-dim, F32 sigmoid, and F32 SiLU ops prove that
the tape can propagate and accumulate gradients through real CUDA tensor ops, including product and
quotient rules, mixed-precision passthrough casts, scalar-loss reductions, shape/view movement,
projection/attention-shaped matmul, softmax Jacobian-vector products, sigmoid gates, and SwiGLU
