...
| `6ca7fb75` | CUDA native masked CE SFT objective | Adds a CUDA shifted-label cross-entropy autograd seed and threads tokenizer `label_mask` through the CUDA-native SFT route; validated on A6000. |
| This commit | CUDA native chunked linear CE SFT loss | Replaces native SFT full-logits CE with a chunked hidden-by-LM-head linear CE autograd seed that avoids materializing `[tokens, vocab]` logits. |
...
- RunPod A6000 validation on pod `pdauarcdn9k62l`, release mode with `KILN_CUDA_ARCHS=86`, passed:
  ...
  - The native CUDA SFT loss was then moved from full-logits CE to chunked linear CE. `cargo test --release -p kiln-model --features cuda cuda_shifted_linear_cross_entropy_loss_matches_full_logits_reference --lib --quiet` passed for chunked-vocab value/backward parity, `cargo test --release -p kiln-train --features cuda cuda_lora_model_step_with_gdn_state_threads_gdn_state --lib --quiet` passed for the mixed GDN-state model step, and `KILN_CUDA_ARCHS=86 scripts/cuda_qwen_sft_smoke.sh --model-path /workspace/qwen3.5-4b --native-cuda` passed with loss `7.271030`, `51.86s/step`, `36424 MB` peak VRAM, and non-null training JSON.
...
model-step helpers now use shifted next-token linear cross-entropy over the supervised label mask
when called from SFT, chunking the LM-head projection over vocab and returning the analytic gradient
to the post-final-norm hidden state without materializing `[tokens, vocab]` logits. The older
synthetic squared-logits loss remains only for token-sequence smoke helpers.
...
| Native CUDA training stack | `crates/kiln-model/src/cuda_train.rs` provides an initial CUDA-only tensor shell over candle CUDA storage with op IDs, parameter `TensorId`, `requires_grad`, detach semantics, a backward-op trait, reverse-topology traversal, per-parameter grad storage, CUDA add/sub/mul/div/scale/cast/sum/mean/reshape/transpose/last-two-transpose/matmul/batched-matmul/softmax/shifted-cross-entropy/shifted-linear-cross-entropy/sigmoid/SiLU/repeat-KV/narrow/index-select/embedding-lookup/attention-permute/causal-mask/RMSNorm/RoPE backward ops, last-dim broadcast ops, causal conv state helpers, single-token/one-head/multi-head GDN recurrence, unmasked and causal SDPA plus SwiGLU MLP and synthetic FullAttention layer composition helpers, resident SGD/AdamW optimizer delegation, tiny optimizer loss-decrease proofs, conservative arena allocation accounting, and typed `CudaModelWeights` import from existing CUDA `GpuWeights`. `crates/kiln-train/src/cuda_train.rs` adds CUDA LoRA pair/layer holders, deterministic LoRA initialization from `CudaModelWeights`, LoRA AdamW state allocation for FullAttention and reserved GDN slots, native CUDA LoRA linear, FullAttention-layer, GDN-layer, FullAttention-only and mixed FullAttention/GDN model-step helpers, FullAttention-only and mixed GDN-state token-sequence train loops, FullAttention-only and mixed GDN-state adapter-producing token-train wrappers, a CUDA-native SFT entry point that selects the mixed GDN-state path for hybrid models, plus minimal linear and synthetic FullAttention AdamW bridges with caller-owned arena support, PEFT-keyed LoRA safetensors, and final adapter directory/config save helpers. The SFT entrypoint imports existing CUDA `GpuWeights`, tokenizes examples, trains resident CUDA LoRA tensors with masked chunked shifted-label linear CE, reports progress, checkpoints, saves at the adapter boundary, and is routable from `KILN_CUDA_NATIVE_TRAINING=1` for SFT. There is still no custom pooled training allocator. | Initial tensor/autograd/optimizer/arena/model-import/save boundary, chunked masked CE SFT objective, train-crate SFT entrypoint, server SFT routing, GDN layer bridge, mixed GDN state-threading model step, mixed GDN-state token train loop, mixed adapter save wrapper, hybrid SFT routing, and one-step real-model validation present; allocator parity still missing. |
