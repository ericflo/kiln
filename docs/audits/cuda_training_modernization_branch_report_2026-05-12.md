...
`CudaModelWeights`, tokenize SFT examples, initialize LoRA, keep AdamW state resident, report
progress, checkpoint adapters, and save the final adapter. It is still FullAttention-only because
the native recurrent GDN layer route is not implemented.
The GDN train-crate bridge now covers LoRA q/k/v/z input projections, gate composition, stateful
causal conv q/k/v, gated RMSNorm, and LoRA output projection in isolation. The train-shell also has
the per-token last-dim broadcast multiply, single-token recurrent GDN step, and one-head sequence
plus flattened multi-head sequence recurrence, but these pieces are not yet threaded through a full
GDN layer or full-model training step.
...
| Native CUDA training stack | `crates/kiln-model/src/cuda_train.rs` provides an initial CUDA-only tensor shell over candle CUDA storage with op IDs, parameter `TensorId`, `requires_grad`, detach semantics, a backward-op trait, reverse-topology traversal, per-parameter grad storage, CUDA add/sub/mul/div/scale/cast/sum/mean/reshape/transpose/last-two-transpose/matmul/batched-matmul/softmax/sigmoid/SiLU/repeat-KV/narrow/index-select/embedding-lookup/attention-permute/causal-mask/RMSNorm/RoPE backward ops, last-dim broadcast ops, causal conv state helpers, single-token/one-head/multi-head GDN recurrence, unmasked and causal SDPA plus SwiGLU MLP and synthetic FullAttention layer composition helpers, resident SGD/AdamW optimizer delegation, tiny optimizer loss-decrease proofs, conservative arena allocation accounting, and typed `CudaModelWeights` import from existing CUDA `GpuWeights`. `crates/kiln-train/src/cuda_train.rs` adds CUDA LoRA pair/layer holders, deterministic LoRA initialization from `CudaModelWeights`, LoRA AdamW state allocation for FullAttention and reserved GDN slots, native CUDA LoRA linear, FullAttention-layer, FullAttention-only model-step, token-sequence train-loop, adapter-producing token-train, FullAttention-only native SFT entry-point helpers, and isolated GDN projection/gate/conv/output-projection bridge pieces, plus minimal linear and synthetic FullAttention AdamW bridges with caller-owned arena support, PEFT-keyed LoRA safetensors, and final adapter directory/config save helpers. The SFT entrypoint imports existing CUDA `GpuWeights`, tokenizes examples, trains resident CUDA LoRA tensors for FullAttention-only stacks, reports progress, checkpoints, saves at the adapter boundary, and is routable from `KILN_CUDA_NATIVE_TRAINING=1` for SFT. There is still no native full Qwen/GDN forward-backward route or custom pooled training allocator. | Initial tensor/autograd/optimizer/arena/model-import/save boundary, train-crate SFT entrypoint, server SFT routing, and isolated GDN bridge pieces present; full native stack missing. |
