...
| Phase 1 | Introduce focused backend traits | `gap` | `landed` | `none` | no | `crates/kiln-model/src/backend/mod.rs`, `scripts/generate_backend_capability_report.py`, `crates/kiln-model/tests/backend_capability_contract.rs` | focused traits still forward through BackendRuntime and BackendRuntime remains above the method-count gate |
