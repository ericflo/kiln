...
The accepted TOML surface contains 16 top-level sections and 112 fixed leaf
...
The migration index separately tracks all 82 former public spellings; entries
marked removed are inert and intentionally have no replacement field.
...
`kiln.vulkan-kernel-policy.v6`, derived once from the selected physical
device's reported Vulkan limits and memory types before model routes are
projected. There is no TOML, CLI, request, environment, device-name, vendor-ID,
...
dispatch-grid and buffer-range checks remain at dispatch. A coherent
host-visible recurrent-state route additionally requires a host-visible,
device-local Vulkan memory type; cached staging memory remains a separate
choice. A device that cannot create the complete route set skips full prewarm
and lazily creates only compatible selected pipelines.

Changing a requirement or workload crossover requires a reviewed source
change, a new policy version, backend/oracle parity, performance evidence, and
cross-device qualification.

The capability policy owns these decisions:

| Family | Capability-derived selection |
|---|---|
| Model route availability | GDN, linear and batched argmax, full-attention QKV, SDPA prefill, paged decode with GPU gather, fused MLP, packed-weight projections, resident decode, and final-state readback elision are admitted only when every shader requirement for that route fits. |
| Kernel scheduling | Row grouping, single-submit, chained dispatch/transfer, parallel reduction, fused recurrence, and batched transfer/upload routes are independently limited by their workgroup, shared-memory, binding, and push-constant requirements. |
| Memory topology | Recurrent zero-copy host visibility requires a coherent host-visible device-local memory type. Discrete-only memory keeps recurrent state device-local. Prompt recurrent-state residency remains disabled. |
| Explicit fallback | `VulkanKernelPolicy::portable_fallback()` is the pre-device and device-unavailable baseline. Unsupported optional routes retain their narrow established fallback instead of disabling unrelated Vulkan routes. |
...
no second direct-stream decode scheduler. This invariant gives cancellation,
backpressure, admission, adapter barriers, KV ownership, and shutdown one
state machine. Mock mode keeps the deterministic scheduler-backed test engine
and does not construct a production actor.
...
| Backend | Backend decode width before server constraints | `prefill_admission_quantum = "auto"` | `burst_prefill_admission` diagnostic | `actor_prefill_tile_alignment_required` diagnostic |
|---|---:|---:|---:|---:|
| CUDA | 8 | effective decode width | true | false |
| ROCm | 8 | 4, clamped to effective width | false | true |
| Vulkan | 64 | effective decode width | false | false |
| Metal | 8 | 4, clamped to effective width | false | false |
| CPU/other | 8 | 4, clamped to effective width | false | false |
...
because local deterministic parity proved that changing a prompt partition can
change greedy output. Startup therefore requires actor prompt chunks and the
qualified streaming-prefill tile policy to preserve one numerical partition.
...
`config_file`, `environment`, or `effective_decode_width`. The admission quantum uses
...
`decode_runtime.batching_engine` snapshot. The trusted debug response keeps the
`batching_engine.enabled` actor-activity flag and the immutable object at
`batching_engine.configuration`. A real backend always reports the actor live;
mock mode reports no production actor snapshot.
...
`kiln config --file <path>` validates and prints all four `[batching]` startup
values: rowwise decode, prefix-aware admission, prefill quantum, and
actor-cycle idle. It also prints the combined actor-cycle budget, prompt-token and layer
...
the alignment requirement, decode width, streaming
...
`/v1/config` after restart for actual selected-device and actor state.
...
This section governs server-owned OpenEnv runs launched from the dashboard or
`/v1/openenv/*`. The `kiln openenv` CLI uses the same protocol client and
artifacts but is not disabled or origin-restricted by this server control-plane
policy.
...
| `openenv.max_active_runs` | unsigned integer; `4` | `KILN_OPENENV_MAX_ACTIVE_RUNS` (implemented) | none | Must be greater than zero and no greater than `max_tracked_runs`. Bounds admitted complete workflows across collection, queued or running GRPO, static post-evaluation, and paired held-out environment evaluation. Additional accepted runs wait in a cancellable FIFO; underlying executors retain their own bounds. |
| `openenv.max_tracked_runs` | unsigned integer; `128` | `KILN_OPENENV_MAX_TRACKED_RUNS` (implemented) | none | Must be greater than zero and at least `max_active_runs`. This is the hard admission bound for active plus FIFO-queued work and retained terminal history. A retained record also owns its optional idempotency-key binding. |
| `openenv.tracked_run_ttl_secs` | unsigned integer; `604800` | `KILN_OPENENV_TRACKED_RUN_TTL_SECS` (implemented) | none | Must be greater than zero. Terminal status and idempotency bindings age out after this window; content-addressed rollout and paired-evaluation artifacts remain under `<adapter_dir>/.openenv/runs/`. |
...
| `openenv.credentials.<id>` | map of origin-scoped credential objects; empty | none | none | Config-file-only handles. IDs use 1..=64 ASCII letters, digits, `_` or `-`, except that `-` alone is reserved for a public CLI/dashboard slot; each object requires an exact canonical `origin` and `bearer_token_env`. Remote origins require HTTPS. The named secret must be set and non-empty at startup; its value and variable name are redacted from effective configuration. |

Protected environments use server-owned, exact-origin credentials:
...
selected device satisfies the complete resident shader limit set. Their former
enable and disable variables were deleted without aliases, so process
environment cannot compose an unreported route mixture. Changing either
decision requires a reviewed source policy version and the full evidence
sequence above.
...
map keyed by canonical TOML paths. It always contains all 112 fixed typed
...
The canonical-only environment contract is a breaking cleanup. The former
