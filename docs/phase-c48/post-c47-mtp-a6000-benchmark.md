...
- `docs/phase-c45/c45-layer1-row-normalization-bisect.md` contains the C47 stop condition: do not change production inference math at the C45 broadcast boundary unless a fresh current-main reproducer again fails the current mask.
- Prior comparison baseline is `docs/phase-c40f/summary.json`.
...
for seed in $(seq 0 19); do
  KILN_SPEC_METHOD=mtp KILN_BENCH_FORCE_MTP=1 \
    ...
    --json-output docs/phase-c48/seed-${seed}.json \
    > docs/phase-c48/seed-${seed}.log 2>&1
  echo "$?" > docs/phase-c48/seed-${seed}.exit
done
```
