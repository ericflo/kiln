...
```bash
mkdir -p docs/phase-c56 /tmp/kiln-c56
...
$NSYS profile --force-overwrite=true --trace=cuda,nvtx,osrt --sample=none --cpuctxsw=none --cuda-memory-usage=false --output /tmp/kiln-c56/c56-post-476-mtp \
  ...
  2>&1 | tee docs/phase-c56/nsys-profile.log
$NSYS stats --report nvtx_sum,nvtx_pushpop_trace,cuda_gpu_kern_sum,cuda_api_sum,cuda_kern_exec_sum --format csv --output docs/phase-c56/c56-failed-prefill /tmp/kiln-c56/c56-post-476-mtp.nsys-rep \
  2>&1 | tee docs/phase-c56/nsys-stats-failed-prefill.log
```
