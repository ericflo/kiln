...
WEB_SOURCES: list[WebSource] = [
    WebSource(
        ...
        "https://rlm.md/",
        "2026-02",
        "Independent explainer and resource hub for RLM fundamentals, techniques, research results, applications, and ecosystem links.",
        "No explicit reuse license found during corpus generation; indexed with metadata only.",
    ),
    WebSource(
        "huggingface-rlm-qwen3-8b",
        "RLM-Qwen3-8B model card",
        "https://huggingface.co/a1zhang/rlm-qwen3-8b",
        "2026",
        "Model card for the small post-trained Qwen3-8B model from the RLM paper experiments.",
        "Model card advertises MIT in front matter, but this corpus indexes it as metadata only to avoid mixing model-card license handling into the fetcher.",
    ),
    WebSource(
        "dspy-rlm-docs",
        "dspy.RLM documentation",
        "https://github.com/stanfordnlp/dspy/blob/main/docs/docs/api/modules/RLM.md",
        "2026",
        "Primary DSPy documentation for the RLM module: sandboxed Python REPL, recursive sub-LLM calls, and large-context exploration through a DSPy signature.",
        "No explicit per-page reuse license checked during corpus generation; indexed with metadata only.",
    ),
    WebSource(
        "predict-rlm-docs",
        "Predict-RLM documentation",
        "https://predict-rlm-docs.docsalot.dev/",
        "2026",
        "Production-focused RLM runtime on top of DSPy with file mounting, skills/tools, sandboxed REPL control flow, and PredictRLM interfaces.",
        "No explicit reuse license found during corpus generation; indexed with metadata only.",
    ),
    WebSource(
        "fleet-rlm-pypi",
        "fleet-rlm PyPI project",
        "https://pypi.org/project/fleet-rlm/",
        "2026",
        "Secure cloud-sandboxed RLM package using DSPy plus Modal/Daytona-style execution and recursive delegation.",
        "PyPI metadata advertises MIT, but this corpus indexes it as metadata only to avoid package-content mirroring.",
    ),
    WebSource(
        "dsprrr-how-rlm-works",
        "How the Recursive Language Model Works",
        "https://jameshwade.github.io/dsprrr/articles/how-rlm-works.html",
        "2026-04-28",
        "R/dsprrr explanation and implementation comparison covering DSPy, official RLM, Google ADK, and R-based REPL execution.",
        "No explicit reuse license found during corpus generation; indexed with metadata only.",
    ),
    WebSource(
        "fullstackwebdev-rlm-repl",
        "fullstackwebdev/rlm_repl",
        "https://github.com/fullstackwebdev/rlm_repl",
        "2026",
        "Proof-of-concept RLM REPL implementation with local LLM server support, prompt-as-variable context, tracing, and cost tracking.",
        "No license file was found in the quick source check; indexed with metadata only.",
    ),
    WebSource(
        "supergok-adk-rlm",
        "Recursive Language Models in ADK for Enterprise AI Agents",
        "https://supergok.com/recursive-language-models-adk-enterprise-ai/",
        "2026",
        "Article about implementing RLM-like recursive delegation in Google's Agent Development Kit for enterprise agents.",
        "No explicit reuse license found during corpus generation; indexed with metadata only.",
    ),
    WebSource(
