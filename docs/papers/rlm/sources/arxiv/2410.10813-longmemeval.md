...
authors:
  ...
  - "Yu, Dong"
date: "2024/10/14"
online_date: "2025/03/04"
license_url: "http://creativecommons.org/licenses/by/4.0/"
source_url: "https://arxiv.org/abs/2410.10813v2"
...
- RLM relevance: LongMemEval is a long-term interactive memory benchmark; useful for comparing RLM source-addressability with indexed/retrieved chat memory systems.
- Related links:
...
Large language models (LLMs) have exhibited impressive capabilities in solving diverse tasks through natural language, leading to numerous successful chat assistant applications (OpenAI, [2022](https://arxiv.org/html/2410.10813v2#bib.bib29); Microsoft, [2023](https://arxiv.org/html/2410.10813v2#bib.bib25)). Nevertheless, LLMs face limitations on tasks relying heavily on personal knowledge accumulated through long-term user-AI interactions, such as psychological counseling or secretarial duties (Zhong et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib52)). Failing to incorporate user background and preferences into responses can diminish the response’s accuracy as well as user satisfaction. To personalize LLM-based assistants, long-term memory, the ability to memorize, recall, and reason with a long interaction history, is indispensable. Recently, several commercial (OpenAI, [2024](https://arxiv.org/html/2410.10813v2#bib.bib30); Coze, [2024](https://arxiv.org/html/2410.10813v2#bib.bib9)) and open-source assistant systems with memory (Zhong et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib52); Zhang et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib49)) have been introduced. These systems leverage techniques like compressing, indexing, and retrieving from chat histories to generate more accurate and personalized responses.

Despite these advances, there has been limited progress in holistically evaluating the memory capability in long-term interactions. While several benchmarks evaluate LLMs on understanding long chat histories (Xu et al., [2022a](https://arxiv.org/html/2410.10813v2#bib.bib44); [b](https://arxiv.org/html/2410.10813v2#bib.bib45); Zhong et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib52); Maharana et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib24); Du et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib11); Kim et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib19)), they have two major shortcomings. First, they do not accurately reflect user-AI interactions: many focus solely on human-human conversations (Xu et al., [2022a](https://arxiv.org/html/2410.10813v2#bib.bib44); Maharana et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib24); Kim et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib19)), while others omit task-oriented dialogues, which represent a significant portion of chat assistant usage and challenge memorization with the long-context inputs and long-form responses. Their interactive histories also typically have a non-configurable length spanning only a few thousand tokens, limiting the difficulty as current systems continue to improve. Second, current benchmarks’ questions only offer a limited coverage of the memory abilities required in dynamic long-term interactions. For instance, MemoryBank (Zhong et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib52)) and PerLTQA (Du et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib11)) insufficiently evaluate the ability to synthesize information across numerous sessions or to reason with temporal metadata or time references. All long-term memory benchmarks including recent ones such as LoCoMo (Maharana et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib24)) also fail to evaluate recall of information provided by the assistant or reasoning with updated user information.

We introduce LongMemEval, a comprehensive benchmark for assessing the long-term memory capabilities of chat assistants. LongMemEval consists of 500 manually created questions to test five core memory abilities: information extraction, multi-session reasoning, temporal reasoning, knowledge updates, and abstention. Each question requires recalling information hidden within one or more task-oriented dialogues between a user and an assistant. Inspired by the “needle-in-a-haystack” test (Kamradt, [2023](https://arxiv.org/html/2410.10813v2#bib.bib18)), we design a pipeline to compile a coherent and length-configurable chat history for each question. A chat system, then, is required to parse the dynamic interactions online for memorization, and answer the question after all the interaction sessions. While the length of the history is freely extensible, we provide two standard settings for consistent comparison: LongMemEvalS with approximately 115k tokens per problem and LongMemEvalM with 500 sessions (around 1.5 million tokens). Preliminary evaluations highlight the difficulty of LongMemEval, as long-context LLMs show a 30%∼similar-to\sim∼60% performance drop on LongMemEvalS, and manual evaluations reveal that state-of-the-art commercial systems only achieved 30%∼similar-to\sim∼70% accuracy in a setting much simpler than LongMemEvalS ([section 3.4](https://arxiv.org/html/2410.10813v2#S3.SS4 "3.4 LongMemEval represents a significant challenge ‣ 3 LongMemEval ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory")).
...
- •
  ...
  ([Section 5.2](https://arxiv.org/html/2410.10813v2#S5.SS2 "5.2 Value: Decomposition improves RAG performance ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory")) Instead of sessions, round is the more optimal granularity for storing and utilizing the interactive history. While further compression into individual user facts harms overall performance due to information loss, it improves the multi-session reasoning accuracy.
- •

  ([Section 5.3](https://arxiv.org/html/2410.10813v2#S5.SS3 "5.3 Key: Multi-key indexing improves retrieval and RAG ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory")) While using a flat index with the memory values themselves as the keys is a strong baseline, further expanding the keys with extracted user facts improves both memory recall (9.4% higher recall@k𝑘kitalic\_k) and downstream question answering (5.4% higher accuracy).
- •

  ([Section 5.4](https://arxiv.org/html/2410.10813v2#S5.SS4 "5.4 Query: Time-aware query expansion improves temporal reasoning ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory")) Naive time-agnostic memory designs perform poorly on temporal reasoning questions. We propose a simple indexing and query expansion strategy to explicitly associate timestamps with facts and narrow down the search range, improving the memory recall for temporal reasoning by 6.8%∼similar-to\sim∼11.3% when a strong LLM is employed for query expansion.
- •

  ([Section 5.5](https://arxiv.org/html/2410.10813v2#S5.SS5 "5.5 Improving reading with chain-of-note and structured format ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory")) Even with perfect memory recall, accurately utilizing retrieved items is non-trivial. Applying Chain-of-Note (Yu et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib47)) and structured data format (Yin et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib46)) improves question answering accuracy by as much as 10 absolute points across three LLMs.

## 2 Related Work

#### Long-Term Dialogue Benchmarks

As the ability of dialogue systems improve, research starts to focus on long-term dialogue understanding beyond traditional dialogue modeling benchmarks (Budzianowski et al., [2018](https://arxiv.org/html/2410.10813v2#bib.bib5); Wei et al., [2018](https://arxiv.org/html/2410.10813v2#bib.bib39)). Early works focused on language modeling evaluation on generating personalized responses from human-human (Xu et al., [2022a](https://arxiv.org/html/2410.10813v2#bib.bib44)) or human-AI (Xu et al., [2022b](https://arxiv.org/html/2410.10813v2#bib.bib45)) chat histories. To more precisely evaluate memory accuracy, subsequent benchmarks shifted toward question answering (QA, Reddy et al. ([2019](https://arxiv.org/html/2410.10813v2#bib.bib31)); Zhang & Choi ([2021](https://arxiv.org/html/2410.10813v2#bib.bib50))). For example, MemoryBank (Zhong et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib52)) features multi-day chat histories from 15 users with 194 human-written probing questions. LoCoMo (Maharana et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib24)) includes 50 long-term chat histories and questions testing single-hop, multi-hop, temporal, commonsense, world knowledge, and adversarial reasoning. PerLTQA (Du et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib11)) scales the evaluation to 3,409 dialogues and 8,593 questions, covering world knowledge, personal profiles, social relationships, events, and dialogue history. DialSim (Kim et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib19)) evaluates models’ memory ability by roleplaying TV show characters and introduces a time constraint that penalizes slow system responses. Despite these advancements, existing QA-based benchmarks overlook several memory capabilities critical to long-term user-assistant interactions: synthesizing information across numerous sessions, recalling assistant side information, and reasoning about updated user details or complex temporal references. Additionally, the chat histories are often too brief and do not reflect the nature of task-oriented interactions. [Table 1](https://arxiv.org/html/2410.10813v2#S2.T1 "In Long-Term Dialogue Benchmarks ‣ 2 Related Work ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory") compares between LongMemEval and previous works, highlighting its advantages in both (1) featuring a long and freely extensible iterative history and (2) holistically covering critical memory abilities in a uniquely challenging way (further examples in [Figure 1](https://arxiv.org/html/2410.10813v2#S2.F1 "In Long-Term Dialogue Benchmarks ‣ 2 Related Work ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory")).
...
| IE | MR | KU | TR | ABS |
| MSC (Xu et al., [2022a](https://arxiv.org/html/2410.10813v2#bib.bib44)) | Open-Domain | 5k | - | 1k | ✗ | ✗ | ✗ | ✗ | ✗ |
| DuLeMon (Xu et al., [2022b](https://arxiv.org/html/2410.10813v2#bib.bib45)) | Open-Domain | 30k | - | 1k | ✗ | ✗ | ✗ | ✗ | ✗ |
| MemoryBank (Zhong et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib52)) | Personal | 300 | 194 | 5k | ✓ | ✗ | ✗ | ✓ | ✗ |
| PerLTQA (Du et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib11)) | Personal | 4k | 8593 | 1M∗ | ✓ | ✗ | ✗ | ✗ | ✓ |
| LoCoMo (Maharana et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib24)) | Personal | 1k | 7512 | 10k | ✓ | ✓ | ✗ | ✓ | ✓ |
| DialSim (Kim et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib19)) | TV Shows | 1k–2k | 1M | 350k | ✓ | ✓∗∗ | ✗ | ✓ | ✓ |
...
To equip chat assistants with long-term memory capabilities, three major techniques are commonly explored. The first approach involves directly adapting LLMs to process extensive history information as long-context inputs (Beltagy et al., [2020](https://arxiv.org/html/2410.10813v2#bib.bib4); Kitaev et al., [2020](https://arxiv.org/html/2410.10813v2#bib.bib20); Fu et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib14); An et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib3)). While this method avoids the need for complex architectures, it is inefficient and susceptible to the “lost-in-the-middle” phenomenon, where the ability of LLMs to utilize contextual information weakens as the input length grows (Shi et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib34); Liu et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib22)). A second line of research integrates differentiable memory modules into language models, proposing specialized architectural designs and training strategies to enhance memory capabilities (Weston et al., [2014](https://arxiv.org/html/2410.10813v2#bib.bib40); Wu et al., [2022](https://arxiv.org/html/2410.10813v2#bib.bib41); Zhong et al., [2022](https://arxiv.org/html/2410.10813v2#bib.bib53); Wang et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib38)). Lastly, several studies approach long-term memory from the perspective of context compression, developing techniques to condense lengthy histories into compact representations, whether in the form of LLM internal representations (Mu et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib27); Chevalier et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib8)), discrete tokens (Jiang et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib17); Xu et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib43)), or retrievable text segments via retrieval-augmented generation (RAG, Shi et al. ([2024](https://arxiv.org/html/2410.10813v2#bib.bib35)); Wang et al. ([2023](https://arxiv.org/html/2410.10813v2#bib.bib38)); Sarthi et al. ([2024](https://arxiv.org/html/2410.10813v2#bib.bib33)); Chen et al. ([2023a](https://arxiv.org/html/2410.10813v2#bib.bib6)); Gutiérrez et al. ([2024](https://arxiv.org/html/2410.10813v2#bib.bib15))). Although LongMemEval can evaluate any memory system, we will take an online context compression perspective, where each history interaction session is sequentially processed, stored, and accessed on-demand through indexing and retrieval mechanisms ([Section 4](https://arxiv.org/html/2410.10813v2#S4 "4 A Unified View of Long-Term Memory Assistants ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory")). This formulation aligns with current literature (Zhong et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib52); Gutiérrez et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib15)) and commercial systems (OpenAI, [2024](https://arxiv.org/html/2410.10813v2#bib.bib30); Coze, [2024](https://arxiv.org/html/2410.10813v2#bib.bib9)). Its plug-and-play nature also facilitates the integration into existing chat assistant systems.

## 3 LongMemEval

### 3.1 Problem Formulation

The evaluation of LongMemEval requires an instance of 4-tuple (𝐒,q,tq,a)𝐒𝑞subscript𝑡𝑞𝑎(\mathbf{S},q,t\_{q},a)( bold\_S , italic\_q , italic\_t start\_POSTSUBSCRIPT italic\_q end\_POSTSUBSCRIPT , italic\_a ). 𝐒≡[(t1,S1),(t2,S2),…,(tN,SN)]𝐒

subscript𝑡1subscript𝑆1subscript𝑡2subscript𝑆2…subscript𝑡𝑁subscript𝑆𝑁\mathbf{S}\equiv[(t\_{1},S\_{1}),(t\_{2},S\_{2}),...,(t\_{N},S\_{N})]bold\_S ≡ [ ( italic\_t start\_POSTSUBSCRIPT 1 end\_POSTSUBSCRIPT , italic\_S start\_POSTSUBSCRIPT 1 end\_POSTSUBSCRIPT ) , ( italic\_t start\_POSTSUBSCRIPT 2 end\_POSTSUBSCRIPT , italic\_S start\_POSTSUBSCRIPT 2 end\_POSTSUBSCRIPT ) , … , ( italic\_t start\_POSTSUBSCRIPT italic\_N end\_POSTSUBSCRIPT , italic\_S start\_POSTSUBSCRIPT italic\_N end\_POSTSUBSCRIPT ) ] is a sequence of N𝑁Nitalic\_N history chat sessions ordered from the earliest to the latest, where Sisubscript𝑆𝑖S\_{i}italic\_S start\_POSTSUBSCRIPT italic\_i end\_POSTSUBSCRIPT is a multi-turn interaction between the user and a chat assistant and tisubscript𝑡𝑖t\_{i}italic\_t start\_POSTSUBSCRIPT italic\_i end\_POSTSUBSCRIPT is the session’s timestamp. Each session can be further decomposed into rounds: one user message followed by one assistant response. During test time, 𝐒𝐒\mathbf{S}bold\_S is provided to the system one by one. q𝑞qitalic\_q and tq>tNsubscript𝑡𝑞subscript𝑡𝑁t\_{q}>t\_{N}italic\_t start\_POSTSUBSCRIPT italic\_q end\_POSTSUBSCRIPT > italic\_t start\_POSTSUBSCRIPT italic\_N end\_POSTSUBSCRIPT represent the question from the user and its date. a𝑎aitalic\_a is a short phrase indicating the answer, or a natural language rubric describing the preferred answer in the case where q𝑞qitalic\_q is open-ended.
...
For each question, LongMemEval compiles a coherent user-AI chat history ([Figure 2](https://arxiv.org/html/2410.10813v2#S3.F2 "In Evidence Session Construction ‣ 3.2 LongMemEval: Benchmark Curation ‣ 3 LongMemEval ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory")c). Our approach is analogous to the needle-in-a-haystack test (Kamradt, [2023](https://arxiv.org/html/2410.10813v2#bib.bib18)), which asks a model to retrieve brief information (the “needle”) embedded in a long document (the “haystack”). In comparison, LongMemEval is more challenging and realistic as it involves retrieving and synthesizing information from multiple extended evidence sessions. Specifically, we sample a number of unrelated user-AI chat sessions, randomly insert the evidence sessions in the middle, and assign a plausible timestamp to all sessions. We draw the irrelevant sessions from two sources: (1) self-chat sessions simulated based on other non-conflicting attributes and (2) publicly released user-AI style chat data including ShareGPT (Zheng et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib51)) and UltraChat (Ding et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib10)). This design creates extensible realistic chat histories with minimal conflicts. While the pipeline allows us to compile chat histories of arbitrary length, we provide two standard settings: LongMemEvalS (∼similar-to\sim∼115k tokens/question) and LongMemEvalM (500 sessions, ∼similar-to\sim∼1.5M tokens).

### 3.3 Evaluation Metric

#### Question Answering

As the correct answers can take flexible forms, an exact matching strategy as in previous works can result in inaccurate evaluations. To address this, LongMemEval employs a LLM to assess response quality (Liu et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib23)). Specifically, we prompt-engineer the gpt-4o-2024-08-06 model via the OpenAI API. Our meta-evaluation study demonstrates that the evaluator achieves more than 97% agreement with human experts. The prompts for each problem type as well as the human meta-evaluation details are presented in [Section A.4](https://arxiv.org/html/2410.10813v2#A1.SS4 "A.4 Evaluation Metric Building ‣ Appendix A Supplemental Details for LongMemEval ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory").

#### Memory Recall

As LongMemEval contains human-annotated answer location labels, intermediate retrieval metrics can be easily calculated if the chat system exposes its retrieval results. We report Recall@k𝑘kitalic\_k and NDCG@k𝑘kitalic\_k, where k𝑘kitalic\_k is the number of top items retrieved by the system.
...
We evaluate two commercial systems that maintain a set of memorized user facts as the user chats with the assistant: ChatGPT (OpenAI, [2024](https://arxiv.org/html/2410.10813v2#bib.bib30)) and Coze (Coze, [2024](https://arxiv.org/html/2410.10813v2#bib.bib9)). Since these systems only support memory features via their web interfaces, we randomly selected 97 questions and created a short chat history of 3-6 sessions (approximately 10x shorter than LongMemEvalS). Human annotators interacted with the chat assistants session-by-session and turn-by-turn, and finally ask the question in a new session222All evaluations were conducted in the first two weeks of August 2024.. In [Figure 3(a)](https://arxiv.org/html/2410.10813v2#S3.F3.sf1 "In Figure 3 ‣ Commercial systems ‣ 3.4 LongMemEval represents a significant challenge ‣ 3 LongMemEval ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we compare this online memory evaluation with offline reading, where GPT-4o is prompted to answer with the complete history provided as context. Both ChatGPT and Coze exhibited significant performance drops compared to offline reading, underscoring the challenging nature of LongMemEval. We found ChatGPT tended to overwrite crucial information as the chat continues, while Coze often failed to record indirectly provided user information. We provide analyses in [Appendix B](https://arxiv.org/html/2410.10813v2#A2 "Appendix B A Human Study on Commercial Memory Chatbots ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"). Overall, this result highlights the gap between building a seemingly personalized chat assistant by recalling isolated facts and demonstrating a genuinely strong memory ability.
...
While LongMemEval poses a significant challenge to online memory systems, is the benchmark easily tackled with offline reading over the entire history? In [Figure 3(b)](https://arxiv.org/html/2410.10813v2#S3.F3.sf2 "In Figure 3 ‣ Commercial systems ‣ 3.4 LongMemEval represents a significant challenge ‣ 3 LongMemEval ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we evaluated four advanced long-context LLMs on LongMemEvalS (with a history length of approximately 115k tokens): GPT-4o, Llama 3.1 Instruct (Dubey et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib12)), and Phi-3 (Abdin et al., [2024a](https://arxiv.org/html/2410.10813v2#bib.bib1)). Compared to the oracle retrieval setting (answering with only the evidence sessions as the context), these LLMs showed a 30% to 60% performance decline when tasked with reading the entire LongMemEvalS history, regardless of whether the chain-of-note technique (Yu et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib47)) was applied (discussed further in [Section 4.2](https://arxiv.org/html/2410.10813v2#S4.SS2 "4.2 Long-Term Memory System: Design Choices ‣ 4 A Unified View of Long-Term Memory Assistants ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory")). As the histories in LongMemEvalS is still short (∼similar-to\sim∼50 sessions), the performance is likely to further degrade as the interaction history expands. Overall, these findings suggest that even the most capable current long-context LLMs struggle to manage an ever-growing interaction history without an effective memory mechanism.
...
We formulate long-term memory as a massive key-value datastore [(k1,v1),(k2,v2),…]

subscript𝑘1subscript𝑣1subscript𝑘2subscript𝑣2…[(k\_{1},v\_{1}),(k\_{2},v\_{2}),...][ ( italic\_k start\_POSTSUBSCRIPT 1 end\_POSTSUBSCRIPT , italic\_v start\_POSTSUBSCRIPT 1 end\_POSTSUBSCRIPT ) , ( italic\_k start\_POSTSUBSCRIPT 2 end\_POSTSUBSCRIPT , italic\_v start\_POSTSUBSCRIPT 2 end\_POSTSUBSCRIPT ) , … ]. The keys kisubscript𝑘𝑖k\_{i}italic\_k start\_POSTSUBSCRIPT italic\_i end\_POSTSUBSCRIPT can be heterogeneous, and could be discrete or continuous. In the discrete case, the key could be a sentence, a paragraph, a fact, or an entity, etc. In the continuous case, the key could be e.g., the model’s internal representation under some inputs. The values visubscript𝑣𝑖v\_{i}italic\_v start\_POSTSUBSCRIPT italic\_i end\_POSTSUBSCRIPT might repeat. As shown in [Figure 4](https://arxiv.org/html/2410.10813v2#S4.F4 "In 4.2 Long-Term Memory System: Design Choices ‣ 4 A Unified View of Long-Term Memory Assistants ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we formulate three stages for a memory-augmented assistant: (1) indexing, converting each history session (ti,Si)subscript𝑡𝑖subscript𝑆𝑖(t\_{i},S\_{i})( italic\_t start\_POSTSUBSCRIPT italic\_i end\_POSTSUBSCRIPT , italic\_S start\_POSTSUBSCRIPT italic\_i end\_POSTSUBSCRIPT ) into one or more key-value items, (2) retrieval, formulating a retrieval query and collecting k𝑘kitalic\_k most relevant items, and (3) reading, an LLM ℳℳ\mathcal{M}caligraphic\_M reads the retrieval result and generates a response. In [Table 2](https://arxiv.org/html/2410.10813v2#S4.T2 "In CP 4: Reading Strategy ‣ 4.2 Long-Term Memory System: Design Choices ‣ 4 A Unified View of Long-Term Memory Assistants ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we show how nine memory-augmented chat assistant systems can be viewed as instantiations of this framework. An alternative mathematical formulation is presented in [Appendix C](https://arxiv.org/html/2410.10813v2#A3 "Appendix C Unified Memory View ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"). For its conciseness, the rest of this paper follows this section’s formulation.

### 4.2 Long-Term Memory System: Design Choices

We identify four crucial control points for long-term memory of chat assistants, as illustrated in [Figure 4](https://arxiv.org/html/2410.10813v2#S4.F4 "In 4.2 Long-Term Memory System: Design Choices ‣ 4 A Unified View of Long-Term Memory Assistants ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"). We analyze design choices from existing works and their limitations, and propose our optimizations. Due to space constraints, we present these designs at a high level here, with detailed designs further described in [Section 5](https://arxiv.org/html/2410.10813v2#S5 "5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory") and [Appendix D](https://arxiv.org/html/2410.10813v2#A4 "Appendix D Memory Optimizations: Implementation Details ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory").

![Refer to caption](x3.png)

Figure 4: A unified view of a chat assistant with long-term memory in operation. We formulate three stages and four control points (CP). We provide further examples in [Table 2](https://arxiv.org/html/2410.10813v2#S4.T2 "In CP 4: Reading Strategy ‣ 4.2 Long-Term Memory System: Design Choices ‣ 4 A Unified View of Long-Term Memory Assistants ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory") and [Appendix C](https://arxiv.org/html/2410.10813v2#A3 "Appendix C Unified Memory View ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory").

#### CP 1: Value

The value represents the format and granularity of each session stored in memory. Given that user-AI chat sessions are often lengthy and cover multiple topics, storing each session as a single item can hinder effective retrieval and reading. Conversely, compressing sessions into summaries or user-specific facts, as seen in prior work such as Zhong et al. ([2024](https://arxiv.org/html/2410.10813v2#bib.bib52)) and Du et al. ([2024](https://arxiv.org/html/2410.10813v2#bib.bib11)), can lead to information loss, harming the performance of the system to answer detailed questions. In [Section 5.2](https://arxiv.org/html/2410.10813v2#S5.SS2 "5.2 Value: Decomposition improves RAG performance ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we compare three value representation strategies: storing entire sessions, decomposing sessions into individual rounds, and further applying summary/fact extraction.

#### CP 2: Key

Even when sessions are decomposed and compressed, each item still contains substantial information, with only a fraction relevant to the user’s query. Therefore, using the value itself as the key, a common practice in prior works (Zhong et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib52); Maharana et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib24)), may be suboptimal. We introduce a key expansion approach in [Section 5.3](https://arxiv.org/html/2410.10813v2#S5.SS3 "5.3 Key: Multi-key indexing improves retrieval and RAG ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), where summaries, keyphrases, user facts, and timestamped events are extracted from the values to augment the index. This optimization highlights the key information and enables effective retrieval with multiple pathways.

#### CP 3: Query

For straightforward user queries, the aforementioned key-value optimizations may yield high retrieval accuracy. However, when queries involve temporal references (e.g., “Which restaurant did you recommend last weekend?”), naive similarity search proves insufficient. We address this with a time-aware indexing and query expansion strategy, where values are indexed with timestamped events, and retrieval is restricted to items within the relevant time range ([Section 5.4](https://arxiv.org/html/2410.10813v2#S5.SS4 "5.4 Query: Time-aware query expansion improves temporal reasoning ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory")).

#### CP 4: Reading Strategy

Answering complex queries may require recalling numerous memory items. Although the retrieval accuracy can be enhanced through the preceding designs, it does not guarantee that the LLM can effectively reason over the extensive context (Shi et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib34); Liu et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib22)). In [Section 5.5](https://arxiv.org/html/2410.10813v2#S5.SS5 "5.5 Improving reading with chain-of-note and structured format ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we explore reading strategies and demonstrate that optimizations such as extracting key information before answering (Chain-of-Note, (Yu et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib47))) and using structured format prompting (Yin et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib46)) are crucial for achieving high reading performance.

Table 2: A comparison of nine memory-augmented frameworks through the lens of the proposed unified framework. We provide detailed references and discussions of each work in [Appendix C](https://arxiv.org/html/2410.10813v2#A3 "Appendix C Unified Memory View ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"). For ChatGPT and Coze, we skip several designs as they are unknown.

| Method | Value | Key | Query | Retrieval | Time-aware | Reading |
| --- | --- | --- | --- | --- | --- | --- |
| In-context RAG (Shi et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib35)) | round/session | K = V | question | flat | No | direct |
| MemoryBank (Zhong et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib52)) | summary + round | K = V | question | flat | Yes | direct |
| LD-Agent (Li et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib21)) | summary + fact | K = V | keyphrase | flat | Yes | direct |
| CoN (Yu et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib47)) | round/session | K = V | question | flat | No | CoN |
| ChatGPT | fact | - | - | - | No | - |
| Coze | fact | - | - | - | No | - |
| RAPTOR (Sarthi et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib33)) | round/session | node summary | question | flat/interactive | No | - |
| MemWalker (Chen et al., [2023a](https://arxiv.org/html/2410.10813v2#bib.bib6)) | round/session | node summary | question | interactive | No | interactive |
| HippoRAG (Gutiérrez et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib15)) | round/session | entity | entity | PPR | No | direct |
...
We mainly study three LLMs: GPT-4o, Llama 3.1 70B Instruct, and Llama 3.1 8B Instruct333More results on five additional LLMs are reported in [Section E.1](https://arxiv.org/html/2410.10813v2#A5.SS1 "E.1 Results on More LLMs ‣ Appendix E Extended Analyses ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory").. For the retriever, we choose dense retrieval with the 1.5B Stella V5 model (Zhang, [2023](https://arxiv.org/html/2410.10813v2#bib.bib48)), given its high performance on MTEB (Muennighoff et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib28)). Extensive comparisons between Stella V5 and alternative retrievers are provided in [Section E.2](https://arxiv.org/html/2410.10813v2#A5.SS2 "E.2 Ablations on Retriever Selection ‣ Appendix E Extended Analyses ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"). For the indexing stage, we employ Llama 3.1 8B Instruct to extract summaries, keyphrases, user facts, and timestamped events. When sessions or rounds are used as the key, we only keep the user-side utterances. In the reading stage, the retrieved items are always sorted by their timestamp to help the reader model maintain temporal consistency. Throughout [Section 5.2](https://arxiv.org/html/2410.10813v2#S5.SS2 "5.2 Value: Decomposition improves RAG performance ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory") to [Section 5.4](https://arxiv.org/html/2410.10813v2#S5.SS4 "5.4 Query: Time-aware query expansion improves temporal reasoning ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we apply Chain-of-Note and json format (discussed in [Section 5.5](https://arxiv.org/html/2410.10813v2#S5.SS5 "5.5 Improving reading with chain-of-note and structured format ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory")) by default.
...
Using LongMemEvalM, we compare different value choices in a budget-aware manner. As shown in [Figure 5](https://arxiv.org/html/2410.10813v2#S5.F5 "In 5.1 Experimental Setup ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), decomposing sessions into rounds significantly enhances reading performance with GPT-4o as the reader, while performing similarly to non-decomposed sessions when using Llama 3.1 8B Instruct as the reader. However, despite their efficiency in token usage, replacing sessions or rounds with extracted summaries or facts negatively impacts QA performance due to information loss. The only exception is with multi-session reasoning questions, where fact decomposition consistently improves performance. We hypothesize this is because fact decomposition extracts the same type of information across all sessions in a more uniform and simplified format, aiding retrieval and reading (Chen et al., [2023b](https://arxiv.org/html/2410.10813v2#bib.bib7)). Finally, we observe that the optimal token budget varies by the reader’s capability: while Llama 3.1 8B Instruct’s performance drops sharply beyond 3k retrieved tokens, GPT-4o continues to improve even with over 20k retrieved tokens.

### 5.3 Key: Multi-key indexing improves retrieval and RAG

In [Table 3](https://arxiv.org/html/2410.10813v2#S5.T3 "In 5.3 Key: Multi-key indexing improves retrieval and RAG ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we explore whether summaries, keyphrases, or user facts condensed from the value can serve as better keys than the value itself. Interestingly, despite their more focused semantics, using these condensed forms alone does not enhance the memory recall performance. We hypothesize that this is due to the retriever’s ability to already effectively handle long-text semantics.

To leverage both the highlighted information from compression and the completeness of the original value, we applied a simple document expansion technique (Tao et al., [2006](https://arxiv.org/html/2410.10813v2#bib.bib36); Efron et al., [2012](https://arxiv.org/html/2410.10813v2#bib.bib13)), where the compressed information is concatenated with the original value to form the key during indexing444We also experimented with merging at the retrieval stage by combining the ranks from different pathways, but it underperformed compared to indexing-stage merging. See [Section E.3](https://arxiv.org/html/2410.10813v2#A5.SS3 "E.3 Post-retrieval rank merging for index expansion ‣ Appendix E Extended Analyses ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory") for details.. This approach, particularly when using user facts, yielded an average improvement of 9.4% in recall@k𝑘kitalic\_k and 5.4% in final accuracy across all models. In [Section E.2](https://arxiv.org/html/2410.10813v2#A5.SS2 "E.2 Ablations on Retriever Selection ‣ Appendix E Extended Analyses ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we further analyze different retrievers and find with alternative retrievers, key expansion with summary and keyphrases could improve Recall@5 when session is used as the value granularity. These results suggest that multi-pathway retrieval can significantly enhance memory recall performance. In the following section, we will investigate the time constraint as another pathway to leverage.
...
A key challenge highlighted by LongMemEval in building real-world assistant systems is the need to utilize temporal information present in both metadata and user utterances to correctly answer time-sensitive queries. To address this need, we introduce a simple yet effective time-aware indexing and query expansion scheme. Specifically, values are additionally indexed by the dates of the events they contain. During retrieval, an LLM ℳTsubscriptℳ𝑇\mathcal{M}\_{T}caligraphic\_M start\_POSTSUBSCRIPT italic\_T end\_POSTSUBSCRIPT extracts a time range for time-sensitive queries, which is used to filter out a large number of irrelevant values.

As shown in [Table 4](https://arxiv.org/html/2410.10813v2#S5.T4 "In 5.4 Query: Time-aware query expansion improves temporal reasoning ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), this simple design improves recall by an average of 11.3% when using rounds as the value and by 6.8% when using sessions as the value. This improvement remains consistent when key expansion is applied during indexing. We also find that the effectiveness of this method depends on using a strong LLM for ℳTsubscriptℳ𝑇\mathcal{M}\_{T}caligraphic\_M start\_POSTSUBSCRIPT italic\_T end\_POSTSUBSCRIPT to accurately infer time ranges from queries. Llama 8B, on the other hand, struggles to generate accurate time ranges, often hallucinating or missing temporal cues even with numerous in-context examples. Further analysis is provided in [Section E.4](https://arxiv.org/html/2410.10813v2#A5.SS4 "E.4 Strong and weak LLMs for extracting time ranges from queries ‣ Appendix E Extended Analyses ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory").
...
| (1) w/ Query Expansion (ℳTsubscriptℳ𝑇\mathcal{M}\_{T}caligraphic\_M start\_POSTSUBSCRIPT italic\_T end\_POSTSUBSCRIPT = GPT-4o) | 0.654 | 0.660 | 0.707 | 0.679 | 0.451 | 0.565 | 0.495 | 0.538 |
| (1) w/ Query Expansion (ℳTsubscriptℳ𝑇\mathcal{M}\_{T}caligraphic\_M start\_POSTSUBSCRIPT italic\_T end\_POSTSUBSCRIPT = Llama 3.1 8B Instruct) | 0.624 | 0.627 | 0.647 | 0.692 | 0.384 | 0.448 | 0.489 | 0.488 |
| (2) K = V + fact | 0.684 | 0.688 | 0.721 | 0.782 | 0.489 | 0.500 | 0.550 | 0.598 |
| (2) w/ Query Expansion (ℳTsubscriptℳ𝑇\mathcal{M}\_{T}caligraphic\_M start\_POSTSUBSCRIPT italic\_T end\_POSTSUBSCRIPT = GPT-4o) | 0.722 | 0.732 | 0.797 | 0.758 | 0.526 | 0.548 | 0.722 | 0.669 |
| (2) w/ Query Expansion (ℳTsubscriptℳ𝑇\mathcal{M}\_{T}caligraphic\_M start\_POSTSUBSCRIPT italic\_T end\_POSTSUBSCRIPT = Llama 3.1 8B Instruct) | 0.677 | 0.688 | 0.711 | 0.744 | 0.481 | 0.532 | 0.570 | 0.647 |

### 5.5 Improving reading with chain-of-note and structured format

As LongMemEval requires syntheses across multiple sessions, even an optimal memory retrieval mechanism needs to return a long context to capture all relevant information. To enhance the model’s ability to handle long retrieved contexts, we apply two key optimizations. First, we present retrieved items in a structured JSON format (Yin et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib46)), which helps the model clearly recognize memory items as the data for reading. Additionally, we apply the Chain-of-Note (CoN) reading approach (Yu et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib47)), instructing the LLM to first extract information from each memory item and then reason based on these notes. This effectively decomposes long-context reading into two simpler subtasks: copying important details and reasoning with more concise notes.

In [Figure 6](https://arxiv.org/html/2410.10813v2#S5.F6 "In 5.5 Improving reading with chain-of-note and structured format ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we evaluate the reading designs under the oracle retrieval setting, where only evidence sessions are provided. Surprisingly, even with perfect retrieval, a suboptimal reading strategy results in up to a 10-point absolute performance drop compared to the best approach for GPT-4o. Notably, when CoN is not applied, JSON format does not consistently outperform the natural language format. However, with CoN, JSON format consistently benefits reader LLMs of various capabilities. [Section E.5](https://arxiv.org/html/2410.10813v2#A5.SS5 "E.5 Error Analysis ‣ Appendix E Extended Analyses ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory") further analyzes error patterns of LLMs with the enhanced reading strategy.
...
In [Table 5](https://arxiv.org/html/2410.10813v2#A1.T5 "In Attribute Ontology ‣ A.1 Dataset Construction ‣ Appendix A Supplemental Details for LongMemEval ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we provide the full attribute ontology used in LongMemEval. This attribute ontology is constructed manually to reflect commonly mentioned topics in user-assistant chats. Five major categories are included: demographic information, lifestyle, situational context, life events, and belongings.
...
In [Figure 9](https://arxiv.org/html/2410.10813v2#A1.F9 "In A.3 Basic Statistics ‣ Appendix A Supplemental Details for LongMemEval ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we present the basic statistics of LongMemEval, revealing that most questions require evidence from multiple sessions (up to six) and that evidence statements are positioned diversely within sessions, increasing the challenge to the memory design.
...
We formulate long-term memory as a massive key-value datastore D=[(k1,v1),(k2,v2),…]𝐷

subscript𝑘1subscript𝑣1subscript𝑘2subscript𝑣2…D=[(k\_{1},v\_{1}),(k\_{2},v\_{2}),...]italic\_D = [ ( italic\_k start\_POSTSUBSCRIPT 1 end\_POSTSUBSCRIPT , italic\_v start\_POSTSUBSCRIPT 1 end\_POSTSUBSCRIPT ) , ( italic\_k start\_POSTSUBSCRIPT 2 end\_POSTSUBSCRIPT , italic\_v start\_POSTSUBSCRIPT 2 end\_POSTSUBSCRIPT ) , … ], where the keys kisubscript𝑘𝑖k\_{i}italic\_k start\_POSTSUBSCRIPT italic\_i end\_POSTSUBSCRIPT can be heterogeneous and the values visubscript𝑣𝑖v\_{i}italic\_v start\_POSTSUBSCRIPT italic\_i end\_POSTSUBSCRIPT might repeat. More concretely, a key could be discrete or continuous. In the discrete case, the key could be a sentence, a paragraph, a fact, or an entity, etc. In the continuous case, the key could be e.g., the model’s internal representation under some inputs. For the three stages, we formulate them with four functions: ℐℐ\mathcal{I}caligraphic\_I, 𝒬𝒬\mathcal{Q}caligraphic\_Q, 𝒮𝒮\mathcal{S}caligraphic\_S, ℛℛ\mathcal{R}caligraphic\_R. The offline index function ℐ⁢(S,D)ℐ𝑆𝐷\mathcal{I}(S,D)caligraphic\_I ( italic\_S , italic\_D ) converts a session S𝑆Sitalic\_S into a series of key-value tuples. Alternatively, the online index function ℐo⁢n⁢l⁢i⁢n⁢e⁢(S,D)subscriptℐ𝑜𝑛𝑙𝑖𝑛𝑒𝑆𝐷\mathcal{I}\_{online}(S,D)caligraphic\_I start\_POSTSUBSCRIPT italic\_o italic\_n italic\_l italic\_i italic\_n italic\_e end\_POSTSUBSCRIPT ( italic\_S , italic\_D ) updates D with the key-value tuples extracted from S, potentially removing or editing the existing items in D. The query formulation function 𝒬⁢(q)𝒬𝑞\mathcal{Q}(q)caligraphic\_Q ( italic\_q ) converts a natural language user query q𝑞qitalic\_q into a representation q⁢’𝑞’q’italic\_q ’ that function 𝒮𝒮\mathcal{S}caligraphic\_S could leverage. The salience scoring function 𝒮⁢(q⁢’,D)𝒮𝑞’𝐷\mathcal{S}(q’,D)caligraphic\_S ( italic\_q ’ , italic\_D ) orders D by their relevance to q⁢’𝑞’q’italic\_q ’. Potential initializations of 𝒮𝒮\mathcal{S}caligraphic\_S include ranking dense text embedding, graph-based ranking, and so on. Finally, the reading function ℛ⁢(M,D⁢’)ℛ𝑀𝐷’\mathcal{R}(M,D’)caligraphic\_R ( italic\_M , italic\_D ’ ) combines the model M with D’, the most relevant part as ranked by the salience scoring function 𝒮𝒮\mathcal{S}caligraphic\_S and generates a response. The methods outlined in the columns of [Table 2](https://arxiv.org/html/2410.10813v2#S4.T2 "In CP 4: Reading Strategy ‣ 4.2 Long-Term Memory System: Design Choices ‣ 4 A Unified View of Long-Term Memory Assistants ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory") could be seen as different ways to instantiate the four functions ℐℐ\mathcal{I}caligraphic\_I, 𝒬𝒬\mathcal{Q}caligraphic\_Q, 𝒮𝒮\mathcal{S}caligraphic\_S, ℛℛ\mathcal{R}caligraphic\_R.

### C.2 Existing Memory Systems from the Unified View

In [Section 4](https://arxiv.org/html/2410.10813v2#S4 "4 A Unified View of Long-Term Memory Assistants ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we have presented a unified view of the memory systems with three stages and four control points. In this section, we provide a closer look at nine memory-augmented chat systems and view them as specific instances of the unified framework. Specifically, we consider in-context RAG (Shi et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib35)), Memorybank (Zhong et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib52)), LD-Agent (Li et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib21)), CoN (Yu et al., [2023](https://arxiv.org/html/2410.10813v2#bib.bib47)), ChatGPT, Coze, RAPTOR (Sarthi et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib33)), MemWalker (Chen et al., [2023a](https://arxiv.org/html/2410.10813v2#bib.bib6)), and HippoRAG (Gutiérrez et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib15)). [Table 2](https://arxiv.org/html/2410.10813v2#S4.T2 "In CP 4: Reading Strategy ‣ 4.2 Long-Term Memory System: Design Choices ‣ 4 A Unified View of Long-Term Memory Assistants ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory") provides an overall comparison between the systems, among which we also situate the recommended design identified through the paper’s empirical study.
...
In [Table 8](https://arxiv.org/html/2410.10813v2#A5.T8 "In E.1 Results on More LLMs ‣ Appendix E Extended Analyses ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we evaluate five more models on LongMemEval: the 1B and 3B models of Llama-3.2 Instruct (Dubey et al., [2024](https://arxiv.org/html/2410.10813v2#bib.bib12)), Qwen-2.5-7B (Team, [2024](https://arxiv.org/html/2410.10813v2#bib.bib37)), Phi-3.5-mini-instruct (Abdin et al., [2024b](https://arxiv.org/html/2410.10813v2#bib.bib2)), and Mistral-Nemo-Instruct-2407 (Mistral AI Team, [2024](https://arxiv.org/html/2410.10813v2#bib.bib26)). Overall, we observe that the performance is consistent with the results reported in the paper. For instance, long-context performance on LongMemEvalS is significantly lower than the oracle retrieval performance. In addition, the fact-based key expansion and CoN consistently improves the performance.
...
In [Table 9](https://arxiv.org/html/2410.10813v2#A5.T9 "In E.2 Ablations on Retriever Selection ‣ Appendix E Extended Analyses ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we compare the performance of Stella V5 with two alternative popular retrievers: BM25 (Robertson & Zaragoza, [2009](https://arxiv.org/html/2410.10813v2#bib.bib32)) and Contriever (Izacard et al., [2022](https://arxiv.org/html/2410.10813v2#bib.bib16)). We do not report the results of retrievers that use larger embedding models, as their latency is generally unrealistic for real applications. The retrievers are compared under two settings: (1) vanilla key design where the values themselves are used as the key and (2) the key expansion strategies introduced in the main text. We list the key observations below:
...
In [Section 5.3](https://arxiv.org/html/2410.10813v2#S5.SS3 "5.3 Key: Multi-key indexing improves retrieval and RAG ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we have introduced and evaluated the key expansion strategy for multi-path retrieval, where important information is extracted from the value items and then used to augment the key. However, an alternative strategy is to directly create a separate key with the retrieved information and place it in parallel as the original keys. At the retrieval stage, the query could be used to retrieve from pools defined multiple types of keys and the values from different sequences could be merged subsequently according to their rank. Effectively, this implements the true “multi-path retrieval. We call this strategy “rank merging”. In [Table 10](https://arxiv.org/html/2410.10813v2#A5.T10 "In E.3 Post-retrieval rank merging for index expansion ‣ Appendix E Extended Analyses ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we thoroughly evaluate rank merging verse key merging, the strategy we recommended in the main text. We find that rank merging has much lower performance than key merging. One potential reason is that rank merging increases the index size by m+1𝑚1m+1italic\_m + 1 times, where m𝑚mitalic\_m is the number of information pieces extracted from each (key, value) pair. By comparison, key merging highlights the important information while avoiding explording the size of the index.
...
In [Section 5.4](https://arxiv.org/html/2410.10813v2#S5.SS4 "5.4 Query: Time-aware query expansion improves temporal reasoning ‣ 5 Experiment Results ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we concluded that a strong model is required to accurately extract the time ranges from the questions, even if in-context examples (with balanced labels) are provided. In this section, we further illustrate this finding with examples in [Table 11](https://arxiv.org/html/2410.10813v2#A5.T11 "In E.4 Strong and weak LLMs for extracting time ranges from queries ‣ Appendix E Extended Analyses ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"). We find that the Llama 3.1 8B Instruct model can generate a correct temporal range when the time references are clearly defined (example 2). However, in many of the cases where the question does not have any temporal reference (example 1, 2, 4), the model often mistakenly extracts a time range, which erroneously prunes out the search space, leading to a low memory recall. By contrast, GPT-4o is able to refuse to generate a time range when the question does not have a time reference.
...
| Predicted time range (Llama 3.1 8B Instruct): 2023/05/01∼similar-to\sim∼2023/05/28 [false positive] |
| Example 2 |
| Question Date: 2023/04/27 |
| Question: Which airline did I fly with the most in March and April? |
| Predicted time range (GPT-4o): 2023/03/01∼similar-to\sim∼2023/04/30 [correct] |
| Predicted time range (Llama 3.1 8B Instruct): 2023/03/01∼similar-to\sim∼2023/04/30 [correct] |
| Example 3 |
| Question Date: 2023/06/28 |
| Question: How many days before the ’Rack Fest’ did I participate in the ’Turbocharged Tuesdays’ event? |
| Predicted time range (GPT-4o): No date extracted [correct] |
| Predicted time range (Llama 3.1 8B Instruct): 2023/06/21∼similar-to\sim∼2023/06/28 [false positive] |
| Example 4 |
| Question Date: 2023/03/10 |
| Question: Which seeds were started first, the tomatoes or the marigolds? |
| Predicted time range (GPT-4o): No date extracted [correct] |
| Predicted time range (Llama 3.1 8B Instruct): 2023/03/01∼similar-to\sim∼2023/03/10 [false positive] |

### E.5 Error Analysis

In this section, we analyze the source of error of various LLMs with our best memory design. Specifically, we use round as the value granularity, expand the key with extracted user facts, and apply CoN as the reading strategy. Stella v5 1.5B is used as the retriever, and top-10 items are provided to the model with a JSON structured prompt. In [Figure 14](https://arxiv.org/html/2410.10813v2#A5.F14 "In E.5 Error Analysis ‣ Appendix E Extended Analyses ‣ LongMemEval: Benchmarking Chat Assist- ants on Long-Term Interactive Memory"), we analyze the distribution of the correct and error cases for three different reader LLMs. First, we observe that a substantial proportion of errors corresponds to correct retrieval yet wrong generation (15%∼similar-to\sim∼19% of all instances, and 40%∼similar-to\sim∼50% among the error instances). The proportion is higher when a weaker reader LLM is used. This indicates that the reading strategy still has a large room of improvement. In addition, we observe that for the reader LLM to generate a correct answer, performing correct retrieval is necessary in ∼similar-to\sim∼90% of the time. We observe that the rest 10% instances are mostly of the question type knowledge-update and the retriever was only able to identify the updated knowledge but failed to retrieved the previous information before update. For these cases, our strict retrieval evaluation criteria will deem that the retrieval has failed. Overall, this result also highlights the high quality of LongMemEval, as the reader LLM cannot take any shortcut to answer the question correctly without a correct memory recall.
