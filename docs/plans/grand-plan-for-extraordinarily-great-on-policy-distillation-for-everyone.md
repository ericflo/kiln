...
4. **The same logic applies to vk-native training improvements** — the GDN backward kernels, gradient checkpointing for hybrid models, the solve_tri shader replacement (Phase 7 follow-ups in `docs/archive/vk-harmonization/vk_native_training.md`) — all of them flow directly into OPD speed on Vulkan as they land.
...
- Reproduce Lu's recovery experiment: a kiln 4B mid-trained on internal docs degrades IF-eval; `distill/refresh` recovers it from <50% to ≥80% with no new internal-QA loss. _The full recipe (real two-phase SFT→OPD pipeline with the dual eval enqueue) runs end-to-end on real GPU hardware — validated on an A100 80GB pod at commit `c40ecf89` (see `bench-results/baselines/opd-phase0-validation-2026-05-16.json`): 461 unit tests across 6 modules pass on real CUDA, the kernel hits or beats the A6000 baseline on every shape on its own GPU class, and the `kiln-opd-loss-kernel` parity tests pass against the candle reference. The specific numerical claim — `<50% → ≥80%` on IFEval — requires a real IFEval scorer + an internal-QA test set + multiple training cycles; that's filed as a separately-scheduled multi-hour validation pass, not slipped into a coding session._
- **`kiln-opd-loss-kernel` ships on CUDA, Vulkan, and Metal**, bit-equivalent within 1e-5, all three meeting the per-engine speed gates in §9.7's "today" column. **No engine ships before its gate is met.** A >5% perf regression on any engine fails CI thereafter. _CUDA-side validated: 6.6× speedup BF16 K=32 forward + 4.7× forward+backward on A6000 at commit `60db09ff` (`bench-results/opd-a6000-baseline.json`); +40% to +348% throughput on A100 80GB at commit `c40ecf89` (`bench-results/opd-a100-baseline.json`); §9.9 CI gate enforces the 5% regression threshold against the GPU-matched baseline. Vulkan + Metal are scoped out of this branch per the user's instruction — their gates ship alongside those kernels._
