...
- [ ] Hold each prefix-cache hit lease until decode cleanup and finish-time
  registrations are complete; snapshot failures must not leak a permanent pin.
- [ ] Make eviction, clear, and adapter purge honor active prefix-cache leases,
  and rank exact hits by next-token compatibility before pinning an entry.
