...
- OPD periodic snapshots remain PEFT-only and lack optimizer/cursor/RNG/
  reference state. SFT exact resume is public, while inline and streamed GRPO
  exact loops now have server/API/CLI/browser admission and complete docs but
  still need completed public-route hardware qualification:
