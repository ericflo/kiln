...
| 2026-07-09 | ROCm graph-owner lifecycle observability | `sha256:c70fe7a869ab` | this commit | portable + ROCm | model counters, health serialization, lifecycle-contract, and ROCm-feature server checks | passed | Health reports live tracked owners plus monotonic owner and graph-release counts; bounded debug events bind owner start, recycled block identity, cleanup, and direct receiver loss to one row ID |
