...
kiln passes the BASE model's PRE-final-norm hidden state into MTP as `h_prev`; the upstream contract (vLLM Qwen3-Next, vLLM Qwen3.5, SGLang Qwen3.5, HF transformers `Qwen3NextModel`) consumes the BASE model's POST-final-norm `last_hidden_state`. The MTP head's `pre_fc_norm_hidden` is therefore being applied to a tensor in a different basis from what it was trained against, and the resulting `fc(concat(norm_emb, norm_h))` lives in a downstream-of-`fc` distribution the MTP transformer block was never trained on. This is the root cause of α-collapse. Full reference-frame analysis (every cross-check, every snippet) is in the companion document `docs/phase-c17/c17-reference-frame.md`.
...
Numerical corroboration: Phase C14 (`docs/phase-c14/c14-splice-verdict.md`) measured `c14__logits` cos = 0.9999736 against an FP32 HF reference that uses the SAME tied tensor. A wrong tying or wrong transpose would have shown cos ≪ 1 at this tap; it didn't. CLEAN.
...
Full chain documented in `docs/phase-c17/c17-reference-frame.md`. Summary:
...
From `docs/phase-c15/c15-h-main-drift-verdict.md` (FP32 reference column):
...
- This document.
- Companion: `docs/phase-c17/c17-reference-frame.md` (full reference-frame cross-check, numerical fingerprint, upstream snippet pins).
...
- Predecessor verdicts: `docs/phase-c14/c14-splice-verdict.md`, `docs/phase-c15/c15-h-main-drift-verdict.md`, `docs/phase-c16/c16-accept-reject-verdict.md`.
