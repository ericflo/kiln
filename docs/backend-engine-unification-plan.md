...
- ROCm's qualified typed kernel profile includes head-major prefill; the
  complete 75-leaf model/tensor policy also owns model loading, W8 decode, long
  attention, tape forward, bounded training geometry, paged-attention splitting
  and GQA specialization, safe concat, finite checks, RMSNorm row tiling, and
  native/online/materialized flash-attention forward and backward selection.
  Portable fallback declines all 45 accelerated routes without weakening the
  thirty correctness and bounded-work leaves.
