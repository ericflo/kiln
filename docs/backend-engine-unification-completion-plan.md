...
4. **A behavioral test proves the seam.** A test using a CPU backend or an
   ...
   but at least one default-CI (CPU) test must exercise the dispatch seam for X.
...
- **W0.3 — Wire `--check` into CI.** Add a CPU-only job to a `.github/workflows`
  file that runs `python3 scripts/generate_backend_capability_report.py --check` and
  the `backend_capability_contract` test. Currently `--check` is in **no** workflow,
  so a hand-edited green report cannot be caught in CI.
- **W0.4 — Close the latency-gate trust holes** (review defects #1, #3):
  - Bind result artifacts to provenance that a human cannot self-declare: require a
    CI-run attestation (e.g., the artifact's committing identity must be the CI bot,
    or a CI-only signature file checked by `check_backend_latency_fixtures.py`).
    Document the trust boundary explicitly in `backend-latency-result-schema.md`.
  ...
  - Add `bench-results/backend-latency/**` to `pull_request.paths` in
    `perf-regression-nightly.yml` so edits/deletions of committed results trip the
    gate.
  - Fix the stale dropdown: `perf-regression-nightly.yml:75`
    `vulkan_rtx6000_decode_microbench` → `vulkan_strix_halo_decode_microbench`.
- **DoD:** no `"status": "covered"` literal remains for a migration-bearing phase;
  every `genuine: true` is computed; `--check` runs in CI; the latency gate rejects a
  fabricated artifact (add a self-test case proving it).
...
- **DoD:** `ops/matmul.rs` has zero `Device::` dispatch branches (grep-guard); a
  default-CI parity test compares request-routed matmul against the CPU oracle for
  rank-2, rank-3 batched, transposed, and mixed-dtype; `supports_matmul_request`
...
- **W5.3 — Real parity tests.** Replace the no-op scaffold tests with eager-vs-replay
  ...
  contract test that runs in default CI.
...
- **W8.2** Add CPU-backend conformance coverage so default CI exercises *behavior*
...
- **W8.3** (Folds into W0.4) Latency-gate spoofing + path filter + dropdown.

## Quick-win defects (independent, do anytime)

- Stale dropdown (`perf-regression-nightly.yml:75`) — also in W0.4.
...
5. A default-CI behavioral test exists for each migrated dispatch seam; real-hardware
   parity tests pass on available hardware.
6. The latency gate rejects a fabricated artifact; `--check` runs in CI; the report
