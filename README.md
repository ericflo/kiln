...
| `server.max_prefill_tokens_per_cycle` | `KILN_SERVER_MAX_PREFILL_TOKENS_PER_CYCLE` | 64 | Prompt-only ceiling inside the combined actor-cycle budget (1–65536). Decode reserves its rows first, then partial prefills advance round-robin by at most this many tokens before the next decode cohort. Lower values favor ITL; higher values favor prefill throughput. The checked Strix Halo Vulkan development profile uses 128; concurrent Vulkan serving at 256 is not qualified. Invalid values stop startup; health/debug report the effective value and source |
